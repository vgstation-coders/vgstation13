...
#define PLATING_PLANE 			(-20 + FLOAT_PLANE)

#define ABOVE_PLATING_PLANE		(-19 + FLOAT_PLANE)
...
#define FLOOR_PLANE 			(-18 + FLOAT_PLANE)

#define BELOW_TURF_PLANE 		(-17 + FLOAT_PLANE)		// objects that are below turfs and darkness but above platings. Useful for asteroid smoothing or other such magic.
	#define CORNER_LAYER 				2
	#define SIDE_LAYER					3

#define TURF_PLANE				(-16 + FLOAT_PLANE)
	#define MAPPING_TURF_LAYER			-999

#define ABOVE_TURF_PLANE 		(-15 + FLOAT_PLANE)			// For items which should appear above turfs but below other objects and hiding mobs, eg: wires & pipes
...
#define NOIR_BLOOD_PLANE 		(-14 + FLOAT_PLANE)		 	// Contains BLOOD, (ALSO) will appear to people under the influence of the noir colour matrix. -if changing this, make sure that the blood layer changes too.

#define HIDING_MOB_PLANE 		(-13 + FLOAT_PLANE)			// for hiding mobs like MoMMIs or spiders or whatever, under most objects but over pipes & such.

#define OBJ_PLANE 				(-12 + FLOAT_PLANE)			// For objects which appear below humans.
...
#define LYING_MOB_PLANE			(-11 + FLOAT_PLANE)			// other mobs that are lying down.

#define LYING_HUMAN_PLANE 		(-10 + FLOAT_PLANE)			// humans that are lying down

#define ABOVE_OBJ_PLANE			(-9 + FLOAT_PLANE)			// for objects that are below humans when they are standing but above them when they are not. - eg, blankets.
	#define BLANKIES_LAYER				0
	#define FACEHUGGER_LAYER			1

#define HUMAN_PLANE 			(-8 + FLOAT_PLANE)			// For Humans that are standing up.

#define MOB_PLANE 				(-7 + FLOAT_PLANE)			// For Mobs.

//	#define MOB_LAYER					4
	#define SLIME_LAYER					5

#define ABOVE_HUMAN_PLANE 		(-6 + FLOAT_PLANE)			// For things that should appear above humans.
...
#define BLOB_PLANE 				(-5 + FLOAT_PLANE)			// For Blobs, which are above humans.
...
#define EFFECTS_PLANE 			(-4 + FLOAT_PLANE)			// For special effects.
...
#define LIGHTING_PLANE 			(-2 + FLOAT_PLANE)	// Don't put anything other than lighting_overlays in there please
	#define LIGHTING_LAYER 				0

#define ABOVE_LIGHTING_PLANE	(-1 + FLOAT_PLANE)
