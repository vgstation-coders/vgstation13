...
/obj/item/weapon/melee/baton/loaded/probe
	...
	vismsg = FALSE
	attacklogverb = "zapped"
	openable = FALSE

/obj/item/weapon/melee/baton/loaded/probe/canbehonkified()
	return 0

/obj/item/weapon/melee/baton/loaded/probe/apply_baton_effect(mob/victim) // The main difference between this and a stun baton. It uses an electric shock attack, so genetics can make a player resistant
	if(iscarbon(victim))
		var/mob/living/carbon/C = victim
		var/damage = rand(5, 10)
		C.electrocute_act(damage, src, incapacitation_duration = 20 SECONDS, def_zone = LIMB_CHEST) // 20 code seconds is more like 10 real seconds, thus making the stun equal to the stun baton
		C.apply_effect(10, STUTTER)
