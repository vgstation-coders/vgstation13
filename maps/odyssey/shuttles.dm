...
// Map elements loaded via /datum/map/active/load_shuttles. Each one drops its
// DMM into a fresh VZ_PARKING vlevel and back-fills its global shuttle datum.
...
// Rendezvous destination ports between the Odyssey and the Vox freighter are
// auto-generated by generate_shuttle_docking_vlevels() now, based on the
// /obj/docking_port/shuttle/dynamic ports placed in each shuttle's DMM.
