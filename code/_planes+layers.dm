...
#define BELOW_TURF_PLANE 		-42 		// objects that are below turfs. Useful for asteroid smoothing or other such magic.
	// todo: use this for asteroids to make them be faster

#define TURF_PLANE 				-41			// For turfs.

	#define TURF_LAYER_MEME_NAME_BECAUSE_CELT_IS_A_FUCKING_RETARD	-999

#define ABOVE_TURF_PLANE 		-40			// For items which should appear above turfs but below other objects and hiding mobs, eg: wires & pipes
...
#define NOIR_BLOOD_PLANE 		-39		 	// For blood which is red, will appear to people under the influence of the noir colour matrix. -if changing this, make sure that the blood layer changes too.

#define HIDING_MOB_PLANE 		-38			// for hiding mobs like MoMMIs or spiders or whatever, under most objects but over pipes & such.

#define OBJ_PLANE 				-37			// For objects which appear below humans.
...
#define LYING_MOB_PLANE			-36			// other mobs that are lying down.

#define LYING_HUMAN_PLANE 		-35			// humans that are lying down

#define ABOVE_OBJ_PLANE			-34			// for objects that are below humans when they are standing but above them when they are not. - eg, blankets.
	#define BLANKIES_LAYER			0

#define HUMAN_PLANE 			-33			// For Humans that are standing up.

#define CUSTOM_HUD_PLANE_START	-32

#define CUSTOM_HUD_PLANE_END	-11
