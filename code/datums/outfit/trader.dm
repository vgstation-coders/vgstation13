...
/datum/outfit/trader
	...
	items_to_collect = list(
		/obj/item/storage/wallet/trader = SURVIVAL_BOX,
	)

	pda_type = null
	associated_job = /datum/job/trader
	id_type = /obj/item/card/id/vox
...
/datum/outfit/trader/pre_equip(var/mob/living/carbon/human/H)
	...
	switch(H.mind.role_alt_title) // Add to the survival box in case we don't have a backbag.
		if("Trader") //Traders get snacks and a coin
			items_to_collect[/obj/item/storage/box/donkpockets/random_amount] = SURVIVAL_BOX
			items_to_collect[/obj/item/reagent_containers/food/drinks/thermos/full] = SURVIVAL_BOX
			items_to_collect[/obj/item/coin/trader] = SURVIVAL_BOX

		if("Merchant") //Merchants get an implant
			implant_types += /obj/item/implant/loyalty

		if("Salvage Broker")
			items_to_collect[/obj/item/device/telepad_beacon] = SURVIVAL_BOX
			items_to_collect[/obj/item/rcs/salvage] = SURVIVAL_BOX
	return ..()
