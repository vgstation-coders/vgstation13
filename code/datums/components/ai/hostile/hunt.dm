	...
	switch(controller.getState())
		...
		if(HOSTILE_STANCE_ATTACK)
			...
			if(!isnull(target))
				...
				SendSignal(/datum/component/ai, COMSIG_ATTACKING, list("target"=target)) // We're telling the attack modules that we have attack intention.  They then individually decide whether to fire.
				if(T)
