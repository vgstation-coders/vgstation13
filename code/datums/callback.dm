/*
	...
	PROC TYPEPATH SHORTCUTS (these operate on paths, not types, so to these shortcuts, datum is NOT a parent of atom, etc...)
		...
		Otherwise you will have to do the full typepath of the proc (/type/of/thing/proc/procname)
...
/datum/callback/New(var/thingtocall, var/proctocall, ...)
	if (thingtocall)
		object = thingtocall
	delegate = proctocall
	if (length(args) > 2)
...
/proc/ImmediateInvokeAsync(var/thingtocall, var/proctocall, ...)
	set waitfor = FALSE
...
/datum/callback/proc/Invoke(...)
	if(!usr)
		...
		if(W)
			...
			if(M)
				if (length(args))
					return PushUsr(arglist(list(M, src) + args))
				return PushUsr(M, src)
...
// Use sparingly
/proc/PushUsr(var/mob/M, var/datum/callback/CB, ...)
	var/temp = usr
	usr = M
	if (length(args) > 2)
