	...
	//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
		...
		switch(filter)
			...
			if ("color")
				...
				while (color_matrix != null)
					...
					if (color_matrix == null)
						return
					if(alert(src,"Modify matrix further?", "New Filter Effect (Color Matrix)","Yes","No") == "No")
						break
				...
				A.filters += filter(type="color", color=color_matrix, space=color_filter)
			...
			if ("displace")
				...
				if (choice == "icon")
					A.filters += filter(type="displace", x=displace_x, y=displace_y, size=displace_size, icon=displace_icon, flags=added_flag)
				else
					A.filters += filter(type="displace", x=displace_x, y=displace_y, size=displace_size, render_source=displace_target, flags=added_flag)
			...
			if ("layer")
				...
				switch (choice_color)
					...
					if ("color_matrix")
						...
						while (color_layer != null)
							color_layer = modify_matrix_menu(color_layer)
							if (color_layer == null)
								return
				...
				if (choice == "icon")
					A.filters += filter(type="displace", x=layer_x, y=layer_y, icon=layer_icon, flags=added_flag, color=color_layer, transform=transform_layer, blend_mode=layer_blend)
				else
					A.filters += filter(type="displace", x=layer_x, y=layer_y, render_source=layer_target, flags=added_flag, color=color_layer, transform=transform_layer, blend_mode=layer_blend)
			...
			if ("outline")
				...
				A.filters += filter(type="outline", size=outline_size , color=outline_color, flags=added_flag)
			...
			if ("radial_blur")
				...
				A.filters += filter(type="radial_blur", x=blur_x, y=blur_y, size=blur_size, offset=blur_offset)
			...
			if ("rays")
				...
				A.filters += filter(type="rays", x=rays_x, y=rays_y, size=rays_size, color=rays_color, offset=rays_offset, density=rays_density, threshold=rays_threshold, factor=rays_factor, flags=added_flag)
			...
			if ("ripple")
				...
				A.filters += filter(type="ripple", x=ripple_x, y=ripple_y, size=ripple_size, repeat=ripple_repeat, radius=ripple_radius, falloff=ripple_falloff, flags=added_flag)
			...
			if ("wave")
				...
				A.filters += filter(type="wave", x=wave_x, y=wave_y, size=wave_size, offset=wave_offset, flags=added_flag)
