...
*/
			...
			if (prob(9))
				affected_mob << "<span class=\"rose\">You feel angry.</span>"
		if(2)
			if(restcure)
/*
...
*/
				if(affected_mob.lying && prob(20))  //changed FROM prob(10) until sleeping is fixed
					...
					return
			if (prob(8))
				affected_mob << "<span class=\"rose\">Your skin feels loose.</span>"
			if (prob(10))
			...
			if (prob(4))
				affected_mob << "<span class=\"rose\">Your stomach churns.</span>"
		if(3)
			if(restcure)
/*
				if(affected_mob.sleeping && prob(20))  //removed until sleeping is fixed
...
*/
				if(affected_mob.lying && prob(20))  //changed FROM prob(10) until sleeping is fixed
					...
					return
			if (prob(10))
				affected_mob << "<span class=\"rose\">Your entire body vibrates.</span>"
