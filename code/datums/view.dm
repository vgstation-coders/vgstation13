...
/datum/view_data/proc/resetFormat()//Cuck
	zoom = chief?.prefs.get_pref(/datum/preference_setting/numerical/pixel_size)
...
/datum/view_data/proc/setZoomMode()
	winset(chief, "mapwindow.map", "zoom-mode=[chief?.prefs.get_pref(/datum/preference_setting/enum/scaling_method)]")

/datum/view_data/proc/isZooming()
	return (width || height)

/datum/view_data/proc/getScreenSize()
	if(chief.prefs.get_pref(/datum/preference_setting/toggle/widescreen))
...
/datum/view_data/proc/addToHeight(toAdd)
	height += toAdd
	apply()
