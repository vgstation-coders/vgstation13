...
/datum/objective/escape/IsFulfilled()
	...
	if(emergency_shuttle.shuttle == S || emergency_shuttle.escape_pods.Find(S))
		if(istype(location, /turf/simulated/floor/shuttle4)) // Fails traitors if they are in the shuttle brig
...
/datum/objective/escape_prisoner/IsFulfilled()
	...
	if(istype(T, /turf/simulated/floor/shuttle4)) //red shuttle floors grant redtext
		return FALSE
