...
/datum/dynamic_ruleset/midround/from_ghosts/faction_based/blob_storm/finish_setup(var/mob/new_character, var/index)
	var/chosen_dir = meteor_wave(rand(20, 40), types = thing_storm_types["blob storm"], offset_origin = 215, offset_dest = 75) // 215 offset : ensure that it actually hits the station (else, the round ends abruptly.)
