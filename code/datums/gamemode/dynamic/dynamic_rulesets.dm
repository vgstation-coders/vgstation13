...
/datum/dynamic_ruleset/proc/check_enemy_jobs(var/dead_dont_count = FALSE, var/midround = FALSE)
	...
	if (enemies_count < required_enemies[threat] && !map.ignore_enemy_requirement(src))
		...
		//world.log << "Dynamic Mode: There are not enough enemy jobs ready for [name]. ([enemies_count] out of [required_enemies[threat]])"
		return FALSE
	...
	if (!dead_dont_count)//roundstart check only
		...
		//world.log << "Dynamic Mode: Despite [name] having enough candidates, there are not enough enemy jobs and pop ready ([enemies_count] and [mode.roundstart_pop_ready] out of [required_pop[threat]])"
	return FALSE
