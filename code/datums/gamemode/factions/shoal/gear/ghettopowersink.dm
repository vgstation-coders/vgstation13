...
/obj/machinery/ghettopowersink/wrenchAnchor(var/mob/user, var/obj/item/I, var/time_to_wrench = 15 SECONDS)
	if(active)
		to_chat(user, "<span class='warning'>[src] needs to be disconnected first!</span>")
		return FALSE
	. = ..()

/obj/machinery/ghettopowersink/malfhack_valid(var/mob/living/silicon/malf)
	if(..())
...
/obj/machinery/ghettopowersink/proc/activate(mob/user)
	if(power_drained >= max_power)
		...
		return
	var/turf/T = loc
	if(isturf(T) && !T.intact && locate(/obj/structure/cable) in T && T.get_cable_node())
		playsound(src, 'sound/weapons/cablecuff.ogg', 50, 1)
		if(do_after(user, src, 5 SECONDS))
			if(!active)
				playsound(src,'sound/misc/click.ogg',40,0)
				power_connection.connect()
				active = TRUE
				spark(src, rand(2,4), FALSE)
				last_drain = 0
		...
		user.visible_message("<span class='notice'>[user] stares at [src] absentmindedly. </span>", \
			"<span class='warning'>There's no exposed cable node here to attach to.</span>", \
...
/obj/machinery/ghettopowersink/attack_hand(mob/user, ignore_brain_damage)
	...
	if(state == 1)
		if(active && power_connection)
			electrocute_mob(user, power_connection.get_powernet(), src)		// Not very safe.
...
/obj/machinery/ghettopowersink/process()
	if((power_drained/max_power > 0.66) && prob(5))
		spark(src, rand(2,4), FALSE)
	if(active && power_connection.connected)
		var/datum/powernet/PN = power_connection.get_powernet()
		if(PN)
			set_light(4,1, "#c5c203")
...
/obj/machinery/ghettopowersink/update_icon()
	icon_state = "ghettosink_[active ? "charging_" : ""][floor(((power_drained/max_power)*100) / 25) * 25]"
