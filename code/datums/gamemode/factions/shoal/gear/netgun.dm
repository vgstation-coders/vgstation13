...
/obj/structure/net/proc/insert_content(var/atom/movable/AM)
	...
	if(istype(AM, /mob/living))
		...
		density = AM.density
	AM.forceMove(src)
	AM.visible_message("<span class='warning'>\The [AM] is caught by \the [src]!</span>", \
...
/obj/item/projectile/hookshot/net/proc/create_net(var/atom/A)
	...
	if(to_net && ismob(to_net) && to_net.locked_to)
		if(!unlock_atom(to_net.locked_to))
			to_net.visible_message("<span class='warning'>The [src] bounces right off [to_net]</span>")
			to_net = null
	if(to_net)
