...
/obj/item/weapon/tome/suicide_act(var/mob/living/user)
	...
	if (C)
		...
		if (ishuman(user))
			...
			H.bloody_hands(H)
		sleep(10)
		C.get_devotion(500, DEVOTION_TIER_4, "suicide_tome", user)
...
/obj/item/weapon/tome/attack(var/mob/living/M, var/mob/living/user)
	...
	if (!M.isDead())
		...
		if (C)
			if (M.mind)
				C.get_devotion(30, DEVOTION_TIER_3, "attack_tome", M)
			else
				C.get_devotion(30, DEVOTION_TIER_2, "attack_tome_nomind", M)
...
/obj/item/weapon/melee/cultblade/attack(var/mob/living/target, var/mob/living/carbon/human/user)
	...
	if (iscultist(user))
		if (!iscultist(target) && !target.isDead())
			var/datum/role/cultist/C = user.mind.GetRole(CULTIST)
			if (target.mind)
				C.get_devotion(30, DEVOTION_TIER_3, "attack_cultblade", target)
			else
				C.get_devotion(30, DEVOTION_TIER_2, "attack_cultblade_nomind", target)
...
/obj/item/weapon/melee/soulblade/attack(var/mob/living/target, var/mob/living/carbon/human/user)
	...
	if (iscultist(user) && !iscultist(target) && !target.isDead())
		var/datum/role/cultist/C = user.mind.GetRole(CULTIST)
		if (target.mind)
			C.get_devotion(30, DEVOTION_TIER_3, "attack_soulblade", target)
		else
			C.get_devotion(30, DEVOTION_TIER_2, "attack_soulblade_nomind", target)
	if (ishuman(target) && target.resting)
		var/obj/structure/cult/altar/altar = locate() in target.loc
		if (altar)
			altar.attackby(src,user)
...
/obj/item/weapon/melee/blood_dagger/attack(var/mob/living/target, var/mob/living/carbon/human/user)
	...
	if (iscultist(user) && !iscultist(target) && !target.isDead())
		var/datum/role/cultist/C = user.mind.GetRole(CULTIST)
		if (target.mind)
			C.get_devotion(30, DEVOTION_TIER_3, "attack_blooddagger", target)
		else
			C.get_devotion(30, DEVOTION_TIER_2, "attack_blooddagger_nomind", target)
	..()
