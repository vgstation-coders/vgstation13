...
/spell/cult/trace_rune //Abstract, base for all rune systems
	name = "Trace Rune"
...
/spell/cult/trace_rune/spell_do_after(var/mob/user, var/delay, var/numticks = 3)
	...
	if (spell || tome)
		if (spell)
			if (!tome)
				to_chat(user, "<span class='warning'>Without reading the tome, you have trouble continue_drawinging the arcane words.</span>")
				return 0
		else
...
/spell/cult/trace_rune/cast(var/list/targets, var/mob/living/carbon/user)
	...
	if (write_rune_word(get_turf(user) ,data["blood"] ,word = cultwords[word]) > 1)
		continue_drawing = 1
		perform(user) //Recursion, set continue_drawing to 1 so that the spell will not be wiped
	else
		var/obj/item/weapon/tome/tome = locate() in user.held_items
