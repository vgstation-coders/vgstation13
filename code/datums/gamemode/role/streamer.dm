...
/datum/role/streamer/proc/try_add_subscription(datum/mind/new_subscriber, obj/machinery/computer/security/telescreen/entertainment/spesstv/tv)
	...
	switch(team)
		if(ESPORTS_CULTISTS)
			new /obj/item/storage/cult/sponsored(get_turf(antag.current))
			for(var/i in 1 to 3)
				new /obj/item/reagent_containers/food/drinks/soda_cans/geometer(get_turf(new_subscriber.current))
		if(ESPORTS_SECURITY)
			new /obj/item/storage/lockbox/security_sponsored(get_turf(antag.current))
			for(var/i in 1 to 4)
				new /obj/item/reagent_containers/food/snacks/donitos(get_turf(new_subscriber.current))
