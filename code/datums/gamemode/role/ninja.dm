...
/datum/role/ninja/Greet(var/greeting,var/custom)
	...
	switch(greeting)
		...
		if(GREET_WEEB)
			...
			to_chat(antag.current, "Your energy katana cannot be dropped while active, does not conduct electricity, and can teleport behind someone on attack once a minute by using the action button.")
			to_chat(antag.current, "Your energy glove can drain power from most things that use cells by using an empty hand on them.")
