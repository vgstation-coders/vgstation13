...
/datum/teleport/instant/science
	...
	teleportChecks()
		...
		if(!isemptylist(teleatom.search_contents_for(/obj/item/weapon/disk/nuclear)))
			if(istype(teleatom, /mob/living))
				var/mob/living/MM = teleatom
				MM.visible_message("<span class=\"danger\">The [MM] bounces off of the portal!</span>","<span class=\"rose\">Something you are carrying seems to be unable to pass through the portal. Better drop it if you want to go through.</span>")
			else
				teleatom.visible_message("<span class=\"danger\">The [teleatom] bounces off of the portal!</span>")
			return 0
		...
		if(istype(teleatom, /mob/living))
			...
			if(MM.locked_to_z != 0 && destination.z != MM.locked_to_z)
				MM.visible_message("<span class=\"danger\">[MM] bounces off the portal!</span>","<span class=\"rose\">You're unable to go to that destination!</span>")
				return 0
		return 1