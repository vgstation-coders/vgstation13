	...
	if(last_ambient_noise)
		...
		for(var/amb in possible_ambience)
			...
			if(last_ambient_noise == initial(A.sound))
				...
				break
		if(cancel_ambience)
			src << sound(null, 0, 0, CHANNEL_AMBIENCE)//I can't think of a sane way to have this be less abrupt. I don't think you can do anything like animating sounds and having a loop for it is silly.
