...
//The person who coded this needs a flogging. It's one of the laggiest fucking proccesses, and it isn't even working correctly
...
datum/light_source
	var/atom/owner
	var/atom/ownerTurf //If the object is inside a container, keep track of it anyways
	var/changed = 1
...
datum/light_source/proc/check()
	...
	if(ownerTurf.x != __x || ownerTurf.y != __y || ownerTurf.z != __z)
		__x = ownerTurf.x
		__y = ownerTurf.y
		__z = ownerTurf.z
		changed = 1
...
datum/light_source/proc/add_effect()
	//Only do this if the light is turned on
	ownerTurf = get_turf(owner) //Sigh
	if(ownerTurf && owner.luminosity > 0)
		readrgb(owner.l_color)
		effect = list()
		for(var/turf/T in view(owner.luminosity, ownerTurf))
...
//Below, we affect variables to other objects without asking for their permission
atom
...
//If we are setting luminosity to 0 the light will be cleaned up by the controller and garbage collected once all its queues are complete.
...
//Helps us depower things in space, remove space fire alarms, and evens out space lighting
...
#define LIGHTING_MAX_LUMINOSITY_TURF	8	//Turfs have a severely shortened range to protect from inevitable floor-lighttile spam.
