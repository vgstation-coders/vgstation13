...
/obj/machinery/genetics_scanner
	...
	receive_signal(datum/signal/signal)
		...
		if(signal.data["address_1"] != net_id || !target || signal.data["command"] != "add" || !istype(signal.data_file, /datum/computer/file/genetics_scan))
			return
	...
	examine(mob/user)
		..()
		if (occupant)
			to_chat(user, "<span class='info'>[occupant.name] is inside the scanner.</span>")
		else
	...
	verb/lock()
		...
		playsound(loc, 'click.ogg', 50, 1)
		if (locked)
			locked = 0
			usr.visible_message("<b>[usr]</b> unlocks the scanner.")
			if (occupant)
				to_chat(occupant, "<span class='warning'>You hear the scanner's lock slide out of place.</span>")
		else
			locked = 1
			usr.visible_message("<b>[usr]</b> locks the scanner.")
			if (occupant)
				to_chat(occupant, "<span class='warning'>You hear the scanner's lock click into place.</span>")
...
/datum/genetics_appearancemenu
	...
	proc
		load_mob_data(var/mob/living/carbon/human/H)
			...
			h_style = H.bioHolder.mobAppearance.h_style
			r_hair = H.bioHolder.mobAppearance.r_hair
			g_hair = H.bioHolder.mobAppearance.g_hair
			b_hair = H.bioHolder.mobAppearance.b_hair

			f_style = H.bioHolder.mobAppearance.f_style
			r_facial = H.bioHolder.mobAppearance.r_facial
			g_facial = H.bioHolder.mobAppearance.g_facial
			b_facial = H.bioHolder.mobAppearance.b_facial

			d_style = H.bioHolder.mobAppearance.d_style
			r_detail = H.bioHolder.mobAppearance.r_detail
			g_detail = H.bioHolder.mobAppearance.g_detail
			b_detail = H.bioHolder.mobAppearance.b_detail

			if(!(hair_styles[h_style] || hair_styles_gimmick[h_style]))
				h_style = "Bald"

			if(!(fhair_styles[f_style] || fhair_styles_gimmick[f_style]))
				f_style = "Shaved"

			if(!(detail_styles[d_style] || detail_styles_gimmick[d_style]))
				d_style = "None"

			r_eyes = H.bioHolder.mobAppearance.r_eyes
			g_eyes = H.bioHolder.mobAppearance.g_eyes
			b_eyes = H.bioHolder.mobAppearance.b_eyes
		...
		update_menu()
			...
			dat += "<a href='byond://?src=\ref[src];s_tone=input'><b>Skin Tone:</b></a> [-s_tone + 35]/220<br>"
			dat += "<a href='byond://?src=\ref[src];eyes=input'><b>Eye Color:</b> <font face=\"fixedsys\" size=\"3\" color=\"#[num2hex(r_eyes, 2)][num2hex(g_eyes, 2)][num2hex(b_eyes, 2)]\"><b>#</b></font></a><br>"

			dat += "<a href='byond://?src=\ref[src];h_style=input'><b>Hair:</b></a> [h_style] "
			dat += "<a href='byond://?src=\ref[src];hair=input'><font face=\"fixedsys\" size=\"3\" color=\"#[num2hex(r_hair, 2)][num2hex(g_hair, 2)][num2hex(b_hair, 2)]\"><b>#</b></font></a><br>"

			dat += "<a href='byond://?src=\ref[src];f_style=input'><b>Facial Hair:</b></a> [f_style] "
			dat += "<a href='byond://?src=\ref[src];facial=input'><font face=\"fixedsys\" size=\"3\" color=\"#[num2hex(r_facial, 2)][num2hex(g_facial, 2)][num2hex(b_facial, 2)]\"><b>#</b></font></a><br>"

			dat += "<a href='byond://?src=\ref[src];d_style=input'><b>Detail:</b></a> [d_style] "
			dat += "<a href='byond://?src=\ref[src];detail=input'><font face=\"fixedsys\" size=\"3\" color=\"#[num2hex(r_detail, 2)][num2hex(g_detail, 2)][num2hex(b_detail, 2)]\"><b>#</b></font></a><br>"
		...
		update_preview_icon()
			...
			preview_icon = new /icon('human.dmi', "body_[gender]_s")
