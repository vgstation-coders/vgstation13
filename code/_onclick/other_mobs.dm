...
/mob/living/carbon/human/UnarmedAttack(var/atom/A, var/proximity, var/params)
	...
	if(src.lying && !(isUnconscious() || stunned || paralysis) && check_crawl_ability() && istype(A, /turf/simulated) && istype(get_turf(src), /turf/simulated) && proximity && !pulledby && !locked_to && !client.move_delayer.blocked())
