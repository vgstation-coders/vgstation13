...
/atom/movable/proc/check_airflow_movable(n)
	...
	if(!isobj(src) && n < zas_settings.Get(/datum/ZAS_Setting/airflow_medium_pressure)) return 0
...
#warn make better conformity with current code here
	if(CheckSlip() < 1)
		return 0
	if(!(status_flags & CANSTUN) && !(status_flags & CANWEAKEN))
...
//		M.show_message("<span class='danger'>[src] slams into [A]!</span>",1,"<span class='danger'>You hear a loud slam!</span>",2)
	...
	apply_damage(b_loss/3, BRUTE, LIMB_HEAD, blocked, 0, "Airflow")

	blocked = run_armor_check(LIMB_CHEST,"melee")
	apply_damage(b_loss/3, BRUTE, LIMB_CHEST, blocked, 0, "Airflow")

	blocked = run_armor_check(LIMB_GROIN,"melee")
	apply_damage(b_loss/3, BRUTE, LIMB_GROIN, blocked, 0, "Airflow")
...
#warn Proper abstraction required
