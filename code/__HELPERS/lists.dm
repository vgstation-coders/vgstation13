...
/*
 * Returns a choice from an input list of types generated by a parsed filter text, calls above proc to generate list.
 * If only one thing is returned, just gives us that with no input list.
 */
/proc/filter_list_input(object, input_text, input_heading, var/list/matches)
	if(!matches.len)
		return
	if(matches.len==1)
		return matches[1]
