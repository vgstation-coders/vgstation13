...
/obj/item/clothing/suit/space/space_ninja/proc/ninjacost(C = 0,X = 0)
	...
	if( (U.stat||U.incorporeal_move)&&X!=3 )//Will not return if user is using an adrenaline booster since you can use them when stat==1.
		U << "<span class=\"rose\">You must be conscious and solid to do this.</span>"//It's not a problem of stat==2 since the ninja will explode anyway if they die.
		return 1
	else if(C&&cell.charge<C*10)
		U << "<span class=\"rose\">Not enough energy.</span>"
		return 1
	switch(X)
		if(1)
		...
		if(2)
			if(s_bombs<=0)
				U << "<span class=\"rose\">There are no more smoke bombs remaining.</span>"
				return 1
		if(3)
			if(a_boost<=0)
				U << "<span class=\"rose\">You do not have any more adrenaline boosters.</span>"
				return 1
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjajaunt()
	...
	if(!ninjacost(C,1))
		...
		if(destination&&istype(mobloc, /turf))//The turf check prevents unusual behavior. Like teleporting out of cryo pods, cloners, mechs, etc.
			...
			U << "<span class=\"rose\">The VOID-shift device is malfunctioning,</span><span class=\"danger\"> teleportation failed</span>."
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjashift(turf/T in oview())
	...
	if(!ninjacost(C,1))
		...
		if((!T.density)&&istype(mobloc, /turf))
			...
			U << "<span class=\"rose\">You cannot teleport into solid walls or from solid matter</span>"
	return
	...
	if(!ninjacost(C))
		...
		if(!kamikaze)
			if(!U.get_active_hand()&&!istype(U.get_inactive_hand(), /obj/item/weapon/melee/energy/blade))
				...
				U << "<span class=\"rose\">You can only summon one blade. Try dropping an item first.</span>"
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjastar()
	...
	if(!ninjacost(C))
		...
		if(targets.len)
			...
			U << "<span class=\"rose\">There are no targets in view.</span>"
	return
		...
		//if(M)//DEBUG
			if(!locate(/obj/effect/energy_net) in M.loc)//Check if they are already being affected by an energy net.
				...
				for(var/mob/O in viewers(U, 3))
					O.show_message(text("<span class=\"rose\">[] caught [] with an energy net!</span>", U, M), 1)
				E.affecting = M
				E.master = U
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjaboost()
	...
	*/
		...
		spawn(70)
			...
			U << "<span class=\"rose\">You are beginning to feel the after-effect of the injection.</span>"
		a_boost--
		s_coold = 3
	return
