...
/obj/item/clothing/suit/space/space_ninja/proc/killai(mob/living/silicon/ai/A = AI)
	if(A.client)
		A << "<span class=\"rose\">Self-erase protocol dete-- *bzzzzz*</span>"
...
/obj/item/clothing/suit/space/space_ninja/proc/deinit()
	...
	if(s_control&&!s_busy)
		...
		affecting << "<span class=\"rose\">The function did not trigger!</span>"
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/spideros()
	...
	if(s_control&&!s_busy&&!kamikaze)
		...
		affecting << "<span class=\"rose\">The interface is locked!</span>"
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/stealth()
	...
	if(s_control&&!s_busy)
		...
		affecting << "<span class=\"rose\">Stealth does not appear to work!</span>"
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/ninitialize(delay = s_delay, mob/living/carbon/human/U = loc)
	if(U.mind && U.mind.assigned_role=="MODE" && !s_initialized && !s_busy)//Shouldn't be busy... but anything is possible I guess.
			...
			switch(i)
				...
				if(3)
					if(U.stat==2||U.health<=0)
						U << "<span class=\"danger\">FĆAL �Rr�R</span><span class=\"rose\">: 344--93#�&&21 BR��N |/|/aV� PATT$RN</span><span class=\"danger\"> RED</span>\n<span class=\"rose\">A-A-aB�rT�NG...</span>"
						unlock_suit()
						break
					lock_suit(U,1)//Check for icons.
					U.regenerate_icons()
					U << "<span class=\"notice\">Linking neural-net interface...\nPattern \green <B>GREEN</B><span class=\"notice\">, continuing operation.</span>"
				if(4)
		...
		if(!U.mind||U.mind.assigned_role!="MODE")//Your run of the mill persons shouldn't know what it is. Or how to turn it on.
			...
			U << "<span class=\"rose\">The suit is already functioning.</span><b> Please report this bug.</b>"
		else
...
/obj/item/clothing/suit/space/space_ninja/proc/deinitialize(delay = s_delay)
	if(affecting==loc&&!s_busy)
		...
		if(!s_initialized)
			U << "<span class=\"rose\">The suit is not initialized.</span><b> Please report this bug.</b>"
			return
		...
		for(var/i = 0,i<7,i++)
			switch(i)
				...
				if(4)
					U << "<span class=\"notice\">Disconnecting neural-net interface...</span><span class=\"success\"><B>Success</B></span>."
				if(5)
					U << "<span class=\"notice\">Disengaging neural-net interface...</span><span class=\"success\">\green<B>Success</B></span>."
				if(6)
...
/obj/item/clothing/suit/space/space_ninja/Topic(href, href_list)
	...
	if(s_control)
		if(!affecting||U.stat||!s_initialized)//Check to make sure the guy is wearing the suit after clicking and it's on.
			U << "<span class=\"rose\">Your suit must be worn and active to use this function.</span>"
		...
		if(!affecting||A.stat||!s_initialized||A.loc!=src)
			A << "<span class=\"rose\">This function is not available at this time.</span>"
	...
	switch(href_list["choice"])
		...
		if("Message")
			...
			if(isnull(P)||P.toff)//So it doesn't freak out if the object no-longer exists.
				display_to << "<span class=\"rose\">Error: unable to deliver message.</span>"
				display_spideros()
				return
		...
		if("Inject")
			if( (href_list["tag"]=="radium"? (reagents.get_reagent_amount("radium"))<=(a_boost*a_transfer) : !reagents.get_reagent_amount(href_list["tag"])) )//Special case for radium. If there are only a_boost*a_transfer radium units left.
				display_to << "<span class=\"rose\">Error: the suit cannot perform this function. Out of [href_list["name"]].</span>"
			else
		...
		if("Unlock Kamikaze")
			if(input(U)=="Divine Wind")
				if( !(U.stat||U.wear_suit!=src||!s_initialized) )
					if( !(cell.charge<=1||s_busy) )
						...
						for(var/i, i<4, i++)
							switch(i)
								if(0)
									U << "<span class=\"notice\">Engaging mode...</span>\n<b>CODE NAME</b>: <span class=\"danger\"> KAMIKAZE</span>"
								if(1)
				...
				U << "<span class=\"rose\">ERROR: WRONG PASSWORD!</span>"
				k_unlock = 0
				spideros = 0
			s_busy = 0
						...
						if(AI==A)
							switch(i)
								if(0)
									...
									U << "<span class=\"danger\">WARNING</span>:<span class=\"rose\"> HACKING AT��TEMP� IN PR0GRESs!</span>"
									spideros = 0
									k_unlock = 0
									U << browse(null, "window=spideros")
								if(1)
									A << "Disconnecting neural interface..."
									U << "<span class=\"danger\">WAR�NING</span>:<span class=\"rose\"> �R�O0�Gr�--S 2&3%</span>"
								if(2)
									A << "Shutting down external protocol..."
									U << "<span class=\"danger\">WARNING</span>:<span class=\"rose\"> P����RֆGr�5S 677^%</span>"
									cancel_stealth()
								if(3)
									A << "Connecting to kernel..."
									U << "<span class=\"danger\">WARNING</span>:<span class=\"rose\"> �R�r�R_404</span>"
									A.control_disabled = 0
								if(4)
									A << "Connection established and secured. Menu updated."
									U << "<span class=\"danger\">W�r#nING</span>:<span class=\"rose\"> #%@!!WȆ|_4�54@ \nUn�B88l3 T� L�-�o-L�CaT2 ##$!�RN�0..%..</span>"
									grant_AI_verbs()
									return
							sleep(s_delay)
						else	break
...
/obj/item/clothing/suit/space/space_ninja/proc/ai_holo(var/turf/T in oview(3,affecting))//To have an internal AI display a hologram to the AI and ninja only.
	...
	if(s_initialized&&affecting&&affecting.client&&istype(affecting.loc, /turf))//If the host exists and they are playing, and their location is a turf.
		if(!hologram)//If there is not already a hologram.
			...
			AI << "<span class=\"rose\">ERROR:</span> Image feed in progress."
	else
		AI << "<span class=\"rose\">ERROR:</span> Unable to project image."
	return
	...
	if(U==affecting)//Safety, in case you try doing this without wearing the suit/being the person with the suit.
		if(istype(I, /obj/item/device/aicard))//If it's an AI card.
			if(s_control)
				...
				U << "<span class=\"danger\">ERROR</span>: Remote access channel disabled."
			...
			if(I:maxcharge>cell.maxcharge&&n_gloves&&n_gloves.candrain)
				...
				if (n_gloves&&n_gloves.candrain&&do_after(U,s_delay))
					...
					U << "<span class=\"rose\">Procedure interrupted. Protocol terminated.</span>"
			return
	...
	if(s_initialized)
		...
		if(s_control)
			...
			if(!kamikaze)
				...
				U << "<span class=\"rose\">KAMIKAZE MODE ENGAGED!</span>"
	...
	switch(target_type)
		...
		if("APC")
			...
			if(A.cell&&A.cell.charge)
				...
				U << "<span class=\"rose\">This APC has run dry of power. You must find another source.</span>"
		...
		if("SMES")
			...
			if(A.charge)
				...
				U << "<span class=\"rose\">This SMES cell has run dry of power. You must find another source.</span>"
		...
		if("CELL")
			...
			if(A.charge)
				...
				U << "<span class=\"rose\">This cell is empty and of no use.</span>"
		...
		if("MACHINERY")//Can be applied to generically to all powered machinery. I'm leaving this alone for now.
			...
			if(A.powered())//If powered.
				...
				if(B)//If APC exists. Might not if the area is unpowered like CentCom.
					...
					U << "<span class=\"rose\">Power network could not be found. Aborting.</span>"
			else
				U << "<span class=\"rose\">This recharger is not providing energy. You must find another source.</span>"
						...
						for(var/datum/tech/analyzing_data in A:files.known_tech)
							if(current_data.id==analyzing_data.id)
								if(analyzing_data.level>current_data.level)
									U << "<span class=\"notice\">Database: <b>UPDATED</b>.</span>"
		...
		if("MECHA")
			...
			A.occupant_message("<span class=\"rose\">Warning: Unauthorized access through sub-route 4, block H, detected.</span>")
			if(A.get_charge())
				while(G.candrain&&A.cell.charge>0&&!maxcapacity)
					drain = rand(G.mindrain,G.maxdrain)
					if(A.cell.charge<drain)
				...
				U << "<span class=\"rose\">The exosuit's battery has run dry. You must find another source of power.</span>"

		if("CYBORG")
			var/mob/living/silicon/robot/A = target
			A << "<span class=\"rose\">Warning: Unauthorized access through sub-route 12, block C, detected.</span>"
			G.draining = 1
			if(A.cell&&A.cell.charge)
				while(G.candrain&&A.cell.charge>0&&!maxcapacity)
					drain = rand(G.mindrain,G.maxdrain)
				...
				U << "<span class=\"rose\">Their battery has run dry of power. You must find another source.</span>"
...
/obj/effect/energy_net
	...
	process(var/mob/living/carbon/M as mob)
		...
		if(!isnull(src))//As long as both net and person exist.
			...
			M << "<span class=\"rose\">You appear in a strange place!</span>"
	...
	attack_hand()
		if (M_HULK in usr.mutations)
			...
			for(var/mob/O in oviewers(src))
				O.show_message(text("<span class=\"rose\">[] rips the energy net apart!</span>", usr), 1)
			health-=50
		healthcheck()
		return
	...
	attack_alien()
		...
		for(var/mob/O in oviewers(src))
			O.show_message(text("<span class=\"rose\">[] claws at the energy net!</span>", usr), 1)
		playsound(get_turf(src), 'sound/weapons/slash.ogg', 80, 1)
		health -= rand(10, 20)
		if(health <= 0)
			usr << text("\green You slice the energy net to pieces.")
			for(var/mob/O in oviewers(src))
				O.show_message(text("<span class=\"rose\">[] slices the energy net apart!</span>", usr), 1)
		healthcheck()
		return
