...
/proc/space_ninja_arrival(var/assign_key = null, var/assign_mission = null)
	...
	if(ninja_selection_active)
		usr << "<span class=\"rose\">Ninja selection already in progress. Please wait until it ends.</span>"
		return
...
*/
	...
	if(!candidate_mob)
		usr << "<span class=\"rose\">The randomly chosen mob was not found in the second check.</span>"
		return
	...
	spawn(1)
		if(alert(candidate_mob, "You have been selected to play as a space ninja. Would you like to play as this role? (You have 30 seconds to accept - You will spawn in 30 seconds if you accept)",,"Yes","No")!="Yes")
			usr << "<span class=\"rose\">The selected candidate for space ninja declined.</span>"
			return
	...
	spawn(300)
		if(!ninja_selection_active || (this_selection_id != ninja_selection_id ))
			...
			candidate_mob << "<span class=\"rose\">Sorry, you were too late. You only had 30 seconds to accept.</span>"
			return

		if(ninja_confirmed_selection != ninja_selection_id)
			ninja_selection_active = 0
			usr << "<span class=\"rose\">The ninja did not accept the role in time.</span>"
			return
		...
		if(assign_mission)
			new_ninja.mind.store_memory("<B>Mission:</B> <span class=\"rose\">[assign_mission].</span><br>")
			new_ninja << "<span class=\"notice\">\nYou are an elite mercenary assassin of the Spider Clan, [new_ninja.real_name]. The dreaded</span> <span class=\"danger\">SPACE NINJA</span>!<span class=\"notice\">You have a variety of abilities at your disposal, thanks to your nano-enhanced cyber armor. Remember your training! \nYour current mission is:</span> <span class=\"danger\">[assign_mission]</span>"
		else
		...
		*/
			...
			new_ninja << "<span class=\"notice\">\nYou are an elite mercenary assassin of the Spider Clan, [new_ninja.real_name]. The dreaded </span><span class=\"danger\">SPACE NINJA</span>!<span class=\"notice\"> You have a variety of abilities at your disposal, thanks to your nano-enhanced cyber armor. Remember your training (initialize your suit by right clicking on it)! \nYour current directive is:</span> <span class=\"danger\">[directive]</span>"
			new_ninja.mind.store_memory("<B>Directive:</B> <span class=\"rose\">[directive]</span><br>")
...
/obj/item/clothing/suit/space/space_ninja/proc/lock_suit(mob/living/carbon/U, X = 0)
	if(X)//If you want to check for icons.
		...
		if(U.mind.special_role!="Space Ninja")
			U << "<span class=\"danger\">fÄTaL ÈÈRRoR</span>:<span class=\"rose\"> 382200-*#00CÖDE <B>RED</B>\nUNAU†HORIZED USÈ DETÈC†††eD\nCoMMÈNCING SUB-R0U†IN3 13...\nTÈRMInATING U-U-USÈR...</span>"
			U.gib()
			return 0
		if(!istype(U:head, /obj/item/clothing/head/helmet/space/space_ninja))
			U << "<span class=\"danger\">ERROR</span>:<span class=\"rose\"> 100113</span> UNABLE TO LOCATE HEAD GEAR\nABORTING..."
			return 0
		if(!istype(U:shoes, /obj/item/clothing/shoes/space_ninja))
			U << "<span class=\"danger\">ERROR</span>: <span class=\"rose\">122011</span> UNABLE TO LOCATE FOOT GEAR\nABORTING..."
			return 0
		if(!istype(U:gloves, /obj/item/clothing/gloves/space_ninja))
			U << "<span class=\"danger\">ERROR</span>: <span class=\"rose\">110223</span> UNABLE TO LOCATE HAND GEAR\nABORTING..."
			return 0
...
/obj/item/clothing/suit/space/space_ninja/proc/grant_kamikaze(mob/living/carbon/U)
	...
	U << "<span class=\"rose\">Do or Die,</span> <span class=\"danger\">LET'S ROCK!!</span>"
...
/obj/item/clothing/gloves/space_ninja/proc/drain_wire()
	...
	if(candrain&&!draining)
		...
		if(isturf(T) && T.is_plating())
			...
			if(!attached)
				U << "<span class=\"rose\">Warning: no exposed cable available.</span>"
			else
