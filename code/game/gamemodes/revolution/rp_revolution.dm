...
/datum/game_mode/revolution/rp_revolution/process()
	...
	if(tried_to_add_revheads < world.time)
		tried_to_add_revheads = timedelay(5 SECONDS)
		var/active_revs = 0
		for(var/datum/mind/rev_mind in head_revolutionaries)
		...
		if(active_revs == 0)
			...
			if(added_heads)
				...
				tried_to_add_revheads = timedelay(10 MINUTES) // wait 10 minutes

	if(last_command_report == 0 && timedelay(-10 MINUTES) >= 0)
	...
	else if(last_command_report == 1 && timedelay(-10 MINUTES) >= 0)
	...
	else if(last_command_report == 2 && timedelay(-10 MINUTES) >= 0)
