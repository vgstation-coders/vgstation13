...
/datum/game_mode/proc/add_clockcultist(var/datum/mind/clock_mind, var/forced = 0)
	...
	if(!(clock_mind in clockcult) && is_convertable_to_cult(clock_mind, forced))
		...
		if(clock_mind.current)
			clock_mind.current.add_language(LANGUAGE_CLOCKCULT)
...
/datum/game_mode/proc/remove_clockcultist(var/datum/mind/clock_mind, var/show_message = 1, var/log = 1)
	if(clock_mind in clockcult)
		...
		clockcult -= clock_mind
		clock_mind.current << "<span class='danger'>Thoughts of the Justiciar and his eternal prison fade from your mind. The sounds of celestial clockwork fall silent, and become nothing more than fleeting memories.</span>"
		clock_mind.memory = ""

		if(clock_mind.current)
			clock_mind.current.remove_language(LANGUAGE_CLOCKCULT)

		if(show_message)
			clock_mind.current.visible_message("[clock_mind.current] looks like \he just reverted to their old faith!")
