...
/datum/game_mode/malfunction/announce()
	...
	world << "<B>The onboard AI is malfunctioning and must be destroyed.</B>"
	world << "<B>If the AI manages to take over the station, it will most likely blow it up. You have no chance to survive, make your time.</B>"
...
/datum/game_mode/proc/greet_malf(var/datum/mind/malf)
	...
	malf.current << "<B>The crew does not know about your malfunction, you might wish to keep it secret.</B>"
	malf.current << "<B>You must override the programming of the station's APCs to assume full control.</B>"
	malf.current << "The process takes one minute per APC and can only be performed on one APC at a time to avoid Powernet alerts."
	malf.current << "Remember : Only APCs on station can help you to take over the station."
	malf.current << "When you feel you have enough APCs under your control, you may begin the takeover attempt."
	malf.current << "Once done, you will be able to interface with all systems, notably the onboard nuclear fission device..."
	return
...
/datum/game_mode/malfunction/proc/capture_the_station()
	...
	world << "<B>It has fully taken control of [station_name()]'s systems.</B>"
	...
	for(var/datum/mind/AI_mind in malf_ai)
		...
		AI_mind.current << "Simply use your Explode verb to interface with the nuclear fission device."
	spawn(600)
...
/datum/game_mode/malfunction/proc/takeover()
	...
	if (!istype(ticker.mode,/datum/game_mode/malfunction))
		usr << "\red You cannot begin a takeover in this round type!"
		return
	if (ticker.mode:malf_mode_declared)
		usr << "\red You've already begun your takeover."
		return
	if (ticker.mode:apcs < 3)
		usr << "\blue You don't have enough hacked APCs to take over the station yet. You need to hack at least 3, however hacking more will make the takeover faster. You have hacked [ticker.mode:apcs] APCs so far."
		return
...
/datum/game_mode/malfunction/proc/ai_win()
	...
	if(!ticker.mode:station_captured)
		usr << "\red You are unable to access the self-destruct system as you don't control the station yet."
		return

	if(ticker.mode:explosion_in_progress || ticker.mode:station_was_nuked)
		usr << "\blue The self-destruct countdown is already triggered!"
		return

	if(!ticker.mode:to_nuke_or_not_to_nuke) //Takeover IS completed, but 60s timer passed.
		usr << "\red Cannot interface, it seems a neutralization signal was sent!"
		return

	usr << "\red Detonation signal sent!"
	...
	world << "\red <B>Self-destructing in 10...</B>"
