...
var/list/transparent_icons = list("diagonalWall","diagonalWallS","diagonalWall3","diagonalWall3S") //icon_states for which to prepare an underlay
		...
		for (var/turf/T in refined_src)
			...
			for (var/turf/B in refined_trg)
				...
				if(C_src.x_pos == C_trg.x_pos && C_src.y_pos == C_trg.y_pos)
					...
					if(direction && istype(X,/turf/simulated/wall/shuttle/diag/smooth))
