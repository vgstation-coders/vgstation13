...
/area/New()
	...
	if(type == /area)	// override defaults for space. TODO: make space areas of type /area/space rather than /area
		...
		power_equip = 0
		power_environ = 0
		music = SOUND_LIST_AMBIENCE_SPACE // area/space when?
//		lighting_state = 4
...
/area/Entered(atom/movable/Obj, atom/OldLoc)
	...
	if (M.client && M.client.prefs && M.client.prefs.toggles & SOUND_AMBIENCE)
		if (isnull(M.areaMaster.media_source) && !M.client.ambience_playing)
			M.client.ambience_playing = TRUE

			var/sound = SOUND_AMBIENCE_SHIP

			if (prob(35))
				sound = music

			M << get_sfx(sound, 0, 0, 0, 25)

			spawn (600) // Ewww - this is very very bad.
				if (M && M.client)
					M.client.ambience_playing = FALSE
