...
/obj/machinery/syndicatebomb/attackby(var/obj/item/I, var/mob/user)
	if(istype(I, /obj/item/weapon/wrench))
		...
		if(!payload)
			payload = I
			to_chat(user, "<span class='notice'>You place [payload] into \the [src].</span>")
			user.drop_item()
			payload.loc = src
		else
			to_chat(user, "<span class='notice'>[payload] is already loaded into \the [src], you'll have to remove it first.</span>")
	else
		..()
...
/obj/machinery/syndicatebomb/interact(var/mob/user)
	...
	if(!open_panel)
		if(!active)
			...
			to_chat(user, "<span class='notice'>\The [src] is bolted to the floor!</span>")
			return
...
/obj/item/weapon/bombcore
	...
	item_state = "eshield0"
	w_class = 3
	origin_tech = Tc_SYNDICATE + "=6;" + Tc_COMBAT + "=5"
	var/adminlog = null
...
/obj/item/weapon/bombcore/proc/detonate()
	if(adminlog)
		...
		log_game(adminlog)
	explosion(get_turf(src),3,7,14, flash_range = 14)
...
/obj/item/weapon/bombcore/training/defuse()
	...
	if(istype(holder))
		attempts++
		defusals++
		holder.loc.visible_message("<span class='notice'>[bicon(holder)] Alert: Bomb has been defused. Your score is now [defusals] for [attempts]! Resetting wires...</span>")
...
/obj/item/weapon/bombcore/badmin/explosion/
	var/HeavyExplosion = 3
	var/MediumExplosion = 7
	var/LightExplosion = 14
	var/Flash = 14

/obj/item/weapon/bombcore/badmin/explosion/detonate()
	explosion(get_turf(src),HeavyExplosion,MediumExplosion,LightExplosion, flash_range = Flash)
...
/obj/item/device/syndicatedetonator/attack_self(mob/user as mob)
	if(!cooldown)
		...
		if(detonated)
			...
			detonated--
			message_admins("[key_name(user)]<A HREF='?_src_=holder;adminmoreinfo=\ref[user]'>?</A> has remotely detonated [detonated ? "syndicate bombs" : "a syndicate bomb"] using a [name] at <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[T.x];Y=[T.y];Z=[T.z]'>[A.name] (JMP)</a>.")
