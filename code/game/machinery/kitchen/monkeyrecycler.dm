...
/obj/machinery/monkey_recycler/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if (istype(O, /obj/item/weapon/grab))
		...
		if(istype(grabbed, /mob/living/carbon/monkey))
			...
			if(target.stat == 0)
				user << "<span class=\"rose\">The monkey is struggling far too much to put it in the recycler.</span>"
			else
				user.drop_item()
				del(target)
			...
			user << "<span class=\"rose\">The machine only accepts monkeys!</span>"
	return
...
/obj/machinery/monkey_recycler/attack_hand(var/mob/user as mob)
	...
	if(grinded >=3)
		...
		user << "<span class=\"rose\">The machine needs at least 3 monkeys worth of material to produce a monkey cube. It only has [grinded].</span>"
	return
