...
/obj/machinery/microwave/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(src.broken > 0)
		if(src.broken == 2 && istype(O, /obj/item/weapon/screwdriver)) // If it's broken and they're using a screwdriver
			...
			user << "<span class=\"rose\">It's broken!</span>"
			return 1
		...
		if(istype(O, /obj/item/weapon/reagent_containers/spray/cleaner)) // If they're trying to clean it then let them
			...
			user << "<span class=\"rose\">It's dirty!</span>"
			return 1
		...
		if(!O.contents.len)
			...
			if (!is_type_in_list(O.contents))
				user << "<span class=\"rose\">Your [O] contains components unsuitable for cookery.</span>"
				return 1
		...
		if (contents.len>=max_n_of_items)
			user << "<span class=\"rose\">This [src] is full of ingredients, you cannot put more.</span>"
			return 1
		...
		user << "<span class=\"rose\">This is ridiculous. You can not fit \the [G.affecting] in this [src].</span>"
		return 1
	else
		user << "<span class=\"rose\">You have no idea what you can cook with this [O].</span>"
		return 1
	src.updateUsrDialog()
...
/obj/machinery/microwave/proc/muck_finish()
	playsound(get_turf(src), 'sound/machines/ding.ogg', 50, 1)
	src.visible_message("<span class=\"rose\">The microwave gets covered in muck!</span>")
...
/obj/machinery/microwave/proc/broke()
	...
	src.visible_message("<span class=\"rose\">The microwave breaks!</span>") //Let them know they're stupid
