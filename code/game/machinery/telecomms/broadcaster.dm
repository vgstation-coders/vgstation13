...
/proc/Broadcast_Message(
	...
	for (var/atom/movable/listener in listeners)
		if (listener)
			...
			var/list/nearby = hearers(speech.speaker,null) // mobs in hearing range of speaker
			if(!(listener in nearby) && listener != speech.speaker && istype(listener,/mob)) // so that they aren't the same mobs that are talking
				var/mob/L = listener
