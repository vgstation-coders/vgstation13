...
/obj/machinery/door_control/taxi/attack_hand(mob/user as mob)
	...
	spawn if(!connected_computer.callTo(destination, wait_time))
		src.visible_message("Taxi engines are on cooldown for the next [round((TAXI_SHUTTLE_COOLDOWN - realtimeat(world.time - connected_computer.lastMove)) / (1 SECONDS))] second\s. Please wait before trying again.")
