			...
			if(src.occupant)
				if(istype(occupant, /mob/living/silicon/robot))
					...
					if(R.module && R.module.modules)
						...
						for(var/obj/O in um)
							// Engineering
							if(istype(O,/obj/item/stack/sheet/metal) || istype(O,/obj/item/stack/sheet/glass/reinforced) || istype(O,/obj/item/stack/sheet/glass/basic) || istype(O,/obj/item/weapon/cable_coil))
								if(O:amount < 50)
									O:amount += 1
							// Security
