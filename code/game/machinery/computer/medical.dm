...
"}
				...
				if(6.0)
					...
					for(var/obj/machinery/bot/medbot/M in machines)
						...
						if(M.z != z)	continue	//only find medibots on the same z-level as the computer
	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(loc, /turf))) || (istype(usr, /mob/living/silicon)))
		...
		if (authenticated)
			...
			if (href_list["add_c"])
				...
				while(active2.fields[text("com_[]", counter)])
					counter++
				active2.fields[text("com_[counter]")] = text("Made by [authenticated] ([rank]) on [time2text(world.realtime, "DDD MMM DD hh:mm:ss")]<BR>[t1]")
