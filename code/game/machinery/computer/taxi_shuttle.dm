...
/obj/machinery/computer/taxi_shuttle/proc/taxi_move_to(area/destination as area, var/wait_time)
	...
	sleep(wait_time)
	var/error = shuttle.travel_to(destination)
	if(error)
		broadcast(error)
	else
		return 1
...
/obj/machinery/computer/taxi_shuttle/attack_hand(mob/user as mob)
	...
	var/dat = ""
	if(allowed(user))
		dat = {"Location: [shuttle.current_area]<br>
...
/obj/machinery/computer/taxi_shuttle/proc/callTo(var/place = "", var/wait_time)
	...
	switch(place)
		if("med_sili")
			if (taxi_move_to(shuttle.area_medical_silicon, wait_time))
				return 1
		if("engi_cargo")
			if (taxi_move_to(shuttle.area_engineering_cargo, wait_time))
				return 1
		if("sec_sci")
			if (taxi_move_to(shuttle.area_security_science, wait_time))
				return 1
		if("abandoned")
			if (taxi_move_to(shuttle.area_abandoned, wait_time))
				return 1
	return
