	...
	switch(state)
		...
		if(3)
			...
			if(istype(P, /obj/item/device/mmi) || istype(P, /obj/item/device/mmi/posibrain))
				if(!P:brainmob)
					user << "<span class=\"rose\">Sticking an empty [P] into the frame would sort of defeat the purpose.</span>"
					return
				if(P:brainmob.stat == 2)
					user << "<span class=\"rose\">Sticking a dead [P] into the frame would sort of defeat the purpose.</span>"
					return

				if(jobban_isbanned(P:brainmob, "AI"))
					user << "<span class=\"rose\">This [P] does not seem to fit.</span>"
					return
				...
				switch(interaction)
					if("AICARD")
						...
						if(C.contents.len)//If there is an AI on card.
							...
							U << "<span class=\"notice\"><b>Transfer successful</b>: [T.name] ([rand(1000,9999)].exe) removed from host terminal and stored within local memory.</span>"
					if("NINJASUIT")
