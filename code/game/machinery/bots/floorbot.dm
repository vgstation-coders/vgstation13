...
/obj/machinery/bot/floorbot/proc/fix_shit()
	...
	if(src.loc == src.target || src.loc == src.target.loc)
		...
		//else if(istype(src.target, /turf/) && emagged < 2)
			...
			visible_message("<span class=\"rose\">[src] makes an excited booping sound.</span>")
			spawn(50)
				src.amount ++
				src.anchored = 0
				src.repairing = 0
...
/obj/machinery/bot/floorbot/proc/repair(var/turf/target)
	...
	if(istype(target, /turf/space/))
		visible_message("<span class=\"rose\">[src] begins to repair the hole</span>")
		...
		if(!F.broken && !F.burnt)
			visible_message("<span class=\"rose\">[src] begins to improve the floor.</span>")
			src.repairing = 1
			spawn(50)
			...
			if(F.is_plating())
				visible_message("<span class=\"rose\">[src] begins to fix dents in the floor.</span>")
				src.repairing = 1
				spawn(20)
					src.repairing = 0
...
/obj/machinery/bot/floorbot/proc/eattile(var/obj/item/stack/tile/plasteel/T)
	if(!istype(T, /obj/item/stack/tile/plasteel))
		return
	visible_message("<span class=\"rose\">[src] begins to collect tiles.</span>")
	src.repairing = 1
	spawn(20)
		if(isnull(T))
			src.target = null
...
/obj/machinery/bot/floorbot/proc/maketile(var/obj/item/stack/sheet/metal/M)
	...
	if(M.amount > 1)
		return
	visible_message("<span class=\"rose\">[src] begins to create tiles.</span>")
	src.repairing = 1
	spawn(20)
		if(isnull(M))
			src.target = null
