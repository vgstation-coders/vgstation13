	...
	if(istype(I,/obj/item/weapon/card/emag))
		...
		if (src.health < maxhealth)
			...
			user.visible_message(
				"<span class=\"rose\">[user] repairs [src]!</span>",
		...
		if(prob(1+I.force * 2))
			unload(0)
			user.visible_message("<span class=\"rose\">[user] knocks [load] off [src] with \the [I]!</span>", "<span class=\"rose\">You knock [load] off [src] with \the [I]!</span>")
		else
...
/obj/machinery/bot/mulebot/bullet_act()
	if(prob(50) && !isnull(load))
		unload(0)
	if(prob(25))
		src.visible_message("<span class=\"rose\">Something shorts out inside [src]!</span>")
		wires.RandomCut()
	..()
		...
		switch(href_list["op"])
			...
			if("power")
				if (src.on)
					...
					if (!turn_on())
						usr << "<span class=\"rose\">You can't switch on [src].</span>"
						return
				else
					return
	...
	if(!wires.MobAvoid())		//usually just bumps, but if avoidance disabled knock over mobs
		...
		if(ismob(M))
			if(istype(M,/mob/living/silicon/robot))
				src.visible_message("<span class=\"rose\">[src] bumps into [M]!</span>")
			else
				src.visible_message("<span class=\"rose\">[src] knocks over [M]!</span>")
				M.stop_pulling()
				M.Stun(8)
				M.Weaken(5)
				M.lying = 1
...
/obj/machinery/bot/mulebot/proc/RunOver(var/mob/living/carbon/human/H)
	src.visible_message("<span class=\"rose\">[src] drives over [H]!</span>")
