...
/obj/machinery/bot/farmbot/Emag(mob/user as mob)
	..()
	if(user) user << "<span class=\"rose\">You short out [src]'s plant identifier circuits.</span>"
	spawn(0)
...
/obj/machinery/bot/farmbot/proc/water()
	...
	if ( emagged ) // warning, humans are thirsty!
		...
		src.visible_message("<span class=\"rose\">[src] splashes [target] with a bucket of water!</span>")
