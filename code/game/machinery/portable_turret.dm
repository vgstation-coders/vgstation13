...
/obj/machinery/porta_turret/Topic(href, href_list)
	...
	if ((href_list["power"]) && (src.allowed(usr)))
		if(anchored) // you can't turn a turret on/off if it's not anchored/secured
			...
			usr << "<span class=\"rose\">It has to be secured first!</span>"
...
/obj/machinery/porta_turret/attackby(obj/item/W as obj, mob/user as mob)
	...
	if ((istype(W, /obj/item/weapon/card/emag)) && (!src.emagged))
		...
		user << "<span class=\"rose\">You short out [src]'s threat assessment circuits.</span>"
		spawn(0)
			for(var/mob/O in hearers(src, null))
				O.show_message("<span class=\"rose\">[src] hums oddly...</span>", 1)
		emagged = 1
...
/obj/machinery/porta_turret_cover/attackby(obj/item/W as obj, mob/user as mob)
	...
	if ((istype(W, /obj/item/weapon/card/emag)) && (!Parent_Turret.emagged))
		user << "<span class=\"rose\">You short out [Parent_Turret]'s threat assessment circuits.</span>"
		spawn(0)
			for(var/mob/O in hearers(Parent_Turret, null))
				O.show_message("<span class=\"rose\">[Parent_Turret] hums oddly...</span>", 1)
