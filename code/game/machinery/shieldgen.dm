...
//////////////Shield Generator Access Button
/obj/machinery/access_button/shieldgen
	name = "shield generator control"
	desc = "A remote control button for a shield generator."
	command = "toggle"
	var/shield_active = 0 // Tracks the state of the linked shield generator

/obj/machinery/access_button/shieldgen/update_icon()
	if(on)
		if(shield_active)
...
/obj/machinery/access_button/shieldgen/receive_signal(datum/signal/signal)
	if(!signal || signal.encryption)
		return
	if(signal.data["tag"] != master_tag)
		return
	// Update our state based on the shield generator's status
	if("active" in signal.data)
		shield_active = signal.data["active"]
		update_icon()

/obj/machinery/access_button/shieldgen/attack_hand(mob/user)
	add_fingerprint(usr)
