...
/obj/machinery/shieldwallgen/attack_hand(mob/user as mob)
	if(state != 1)
		user << "<span class=\"rose\">The shield generator needs to be firmly secured to the floor first.</span>"
		return 1
	if(src.locked && !istype(user, /mob/living/silicon))
		user << "<span class=\"rose\">The controls are locked!</span>"
		return 1
	if(power != 1)
		user << "<span class=\"rose\">The shield generator needs to be powered by wire underneath.</span>"
		return 1
...
//		shieldload = maxshieldload
	...
	if(src.active >= 1)
		if(src.power == 0)
			src.visible_message("<span class=\"rose\">The [src.name] shuts down due to lack of power!</span>", \
				"You hear heavy droning fade out")
			icon_state = "Shield_Gen"
			src.active = 0
			spawn(1)
...
/obj/machinery/shieldwallgen/attackby(obj/item/W, mob/user)
	...
	if(istype(W, /obj/item/weapon/card/id)||istype(W, /obj/item/device/pda))
		...
		src.add_fingerprint(user)
		visible_message("<span class=\"rose\">The [src.name] has been hit with the [W.name] by [user.name]!</span>")
