...
*/
	...
	updateicon()
	visible_message("<span class=\"rose\">[src] has suddenly mutated into</span> <span class=\"notice\"> [myseed.plantname]!</span>")
...
obj/machinery/hydroponics/proc/mutateweed() // If the weeds gets the mutagent instead. Mind you, this pretty much destroys the old plant
	if ( weedlevel > 5 )
		...
		spawn(5) // Wait a while
		updateicon()
		visible_message("<span class=\"rose\">The mutated weeds in [src] spawned a</span> <span class=\"notice\"> [myseed.plantname]!</span>")
	else
...
obj/machinery/hydroponics/proc/plantdies() // OH NOES!!!!! I put this all in one function to make things easier
	...
	harvest = 0
	updateicon()
	visible_message("<span class=\"rose\">[src] is looking very unhealthy!</span>")
	return
...
obj/machinery/hydroponics/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	//		Toxicity dilutation code. The more water you put in, the lesser the toxin concentration.
			...
			user << "<span class=\"rose\">The [src] is already full.</span>"
		else
			user << "<span class=\"rose\">The bucket is not filled with water.</span>"
		updateicon()
		...
		if (planted)
			if (S.mode == 1)
				if(!S.reagents.total_volume)
					user << "<span class=\"rose\">The syringe is empty.</span>"
					return
				user << "<span class=\"rose\">You inject the [myseed.plantname] with a chemical solution.</span>"
		...
		if(!planted)
			...
			user << "<span class=\"rose\">The [src] already has seeds in it!</span>"
		...
		if(planted && myseed)
			...
			if(weedlevel > 0)
				...
				weedlevel = 0
			toxic += 4 // Oops
			visible_message("<span class=\"danger\">\The [src] has been sprayed with \the [O][(user ? " by [user]." : ".")]</span>")
		...
		//var/deweeding
		if(weedlevel > 0)
			user.visible_message("<span class=\"rose\">[user] starts uprooting the weeds.</span>", "<span class=\"rose\">You remove the weeds from the [src].</span>")
			weedlevel = 0
			updateicon()
			src.updateicon()
		else
			user << "<span class=\"rose\">This plot is completely devoid of weeds. It doesn't need uprooting.</span>"
		...
		if(planted)
			user << "<span class=\"rose\">The hydroponics tray is already occupied!</span>"
		else
			user.drop_item()
			del(O)
...
/obj/machinery/hydroponics/proc/update_tray(mob/user = usr)
	...
	if((yieldmod * myseed.yield) <= 0 || istype(myseed,/obj/item/seeds/replicapod))
		user << text("<span class=\"rose\">You fail to harvest anything useful.</span>")
	else
