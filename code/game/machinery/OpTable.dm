...
/obj/machinery/optable/attackby(obj/item/W as obj, mob/living/carbon/user as mob)
	if(W.is_wrench(user))
		...
		if(do_after(user, src, 40))
			...
			switch(rating)
				if(1)
					new /obj/item/stock_parts/scanning_module(src.loc)
				if(2)
					new /obj/item/stock_parts/scanning_module/adv(src.loc)
				if(3)
					new /obj/item/stock_parts/scanning_module/adv/phasic(src.loc)
