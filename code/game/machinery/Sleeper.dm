...
/obj/machinery/sleeper/proc/check(mob/living/user as mob)
	if(src.occupant)
		...
		user << text("[]\t Health %: [] ([])</span>", (src.occupant.health > 50 ? "<span class=\"notice\">" : "<span class=\"rose\"> "), src.occupant.health, t1)
		user << text("[]\t -Core Temperature: []&deg;C ([]&deg;F)</span><BR>", (src.occupant.bodytemperature > 50 ? "<font color='blue'>" : "<span class=\"rose\">"), src.occupant.bodytemperature-T0C, src.occupant.bodytemperature*1.8-459.67)
		user << text("[]\t -Brute Damage %: []</span>", (src.occupant.getBruteLoss() < 60 ? "<span class=\"notice\"> " : "<span class=\"rose\"> "), src.occupant.getBruteLoss())
		user << text("</span>[]\t -Respiratory Damage %</span>: []", (src.occupant.getOxyLoss() < 60 ? "<span class=\"notice\">" : "</span><span class=\"rose\"> "), src.occupant.getOxyLoss())
		user << text("</span>[]\t -Toxin Content %: []</span>", (src.occupant.getToxLoss() < 60 ? "<span class=\"notice\">" : "<span class=\"rose\"> "), src.occupant.getToxLoss())
		user << text("</span>[]\t -Burn Severity %: []</span>", (src.occupant.getFireLoss() < 60 ? "<span class=\"notice\">" : "<span class=\"rose\"> "), src.occupant.getFireLoss())
		user << "</span><span class=\"notice\">Expected time till occupant can safely awake: (note: If health is below 20% these times are inaccurate)</span>"
