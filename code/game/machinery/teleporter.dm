		...
		if(istype(L, /obj/effect/landmark/) && istype(L.loc, /turf))
			...
			if(C.data == "Clown Land")
				...
				for(var/mob/O in hearers(src, null))
					O.show_message("<span class=\"rose\">Incoming bluespace portal detected, unable to lock in.</span>", 2)
...
/obj/machinery/teleport/hub/proc/teleport(atom/movable/M as mob|obj)
	...
	if (!com.locked)
		for(var/mob/O in hearers(src, null))
			O.show_message("<span class=\"rose\">Failure: Cannot authenticate locked on coordinates. Please reinstate coordinate matrix.</span>")
		return
...
/proc/do_teleport(atom/movable/M as mob|obj, atom/destination, precision)
	...
	if (istype(M, /mob/living))
		...
		if(MM.check_contents_for(/obj/item/weapon/disk/nuclear))
			MM << "<span class=\"rose\">Something you are carrying seems to be unable to pass through the portal. Better drop it if you want to go through.</span>"
			return
	var/disky = 0
...
//Bags of Holding cause bluespace teleportation to go funky. --NeoFite
	if (istype(M, /mob/living))
		...
		if(MM.check_contents_for(/obj/item/weapon/storage/backpack/holding))
			MM << "<span class=\"rose\">The Bluespace interface on your Bag of Holding interferes with the teleport!</span>"
