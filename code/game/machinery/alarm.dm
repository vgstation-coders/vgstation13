...
/*	if (istype(W, /obj/item/weapon/wirecutters))
		...
		for(var/mob/O in viewers(user, null))
			O.show_message(text("<span class=\"rose\">[] has []activated []!</span>", user, (stat&BROKEN) ? "de" : "re", src), 1)
		update_icon()
		return
*/
...
/obj/item/alarm_frame/proc/try_build(turf/on_wall)
	...
	if (!istype(loc, /turf/simulated/floor))
		usr << "<span class=\"rose\">Air Alarm cannot be placed on this spot.</span>"
		return
		...
		switch(buildstage)
			if(2)
				if (istype(W, /obj/item/device/multitool))
					...
					if (src.detecting)
						user.visible_message("<span class=\"rose\">[user] has reconnected [src]'s detecting unit!</span>", "You have reconnected [src]'s detecting unit.")
					else
						user.visible_message("<span class=\"rose\">[user] has disconnected [src]'s detecting unit!</span>", "You have disconnected [src]'s detecting unit.")
				if(istype(W, /obj/item/weapon/wirecutters))
					if(do_after(user,50))
						buildstage=1
						user.visible_message("<span class=\"rose\">[user] has cut the wiring from \the [src]!</span>", "You have cut the last of the wiring from \the [src].")
						update_icon()
...
/obj/item/firealarm_frame/proc/try_build(turf/on_wall)
	...
	var/area/A = loc.loc
	if (!istype(loc, /turf/simulated/floor))
		usr << "<span class=\"rose\">Fire Alarm cannot be placed on this spot.</span>"
		return
	if (A.requires_power == 0 || A.name == "Space")
		usr << "<span class=\"rose\">Fire Alarm cannot be placed in this area.</span>"
		return
