...
/obj/item/weapon/circuitboard/blank/attackby(obj/item/O as obj, mob/user as mob)
	if(ismultitool(O))
		var/boardType = local_fuses.assigned_boards["[local_fuses.localbit]"]
		if(boardType)
			if(ispath(boardType))
				...
				qdel(src)
				return
			else world << "boardType was not a valid path"
		else user << "<span class='warning'>The multitool flashes red briefly.</span>"
	else if(issolder(O))
