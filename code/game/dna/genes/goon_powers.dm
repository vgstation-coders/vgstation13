...
/obj/effect/proc_holder/spell/targeted/eat/cast(list/targets)
	...
	if(ismob(the_item))
		...
		if(the_item.gender==FEMALE)
			t_his="her"
		usr.visible_message("<span class=\"rose\">[usr] begins stuffing [the_item] into [m_his] gaping maw!</span>")
		var/mob/living/carbon/human/H = the_item
		var/datum/organ/external/limb = H.get_organ(usr.zone_sel.selecting)

		if(!istype(limb))
			usr << "<span class=\"rose\">You can't eat this part of them!</span>"
			return 0
		if(!istype(limb,/datum/organ/external/head))
			// Bullshit, but prevents being unable to clone someone.
			usr << "<span class=\"rose\">You try to put \the [limb] in your mouth, but [t_his] ears tickle your throat!</span>"
			return 0
		var/oldloc = H.loc
		if(do_after(usr,EAT_MOB_DELAY))
			if(!limb || !H)
				return
			if(H.loc!=oldloc)
				usr << "<span class=\"rose\">\The [limb] moved away from your mouth!</span>"
				return
			usr.visible_message("<span class=\"rose\">[usr] [pick("chomps","bites")] off [the_item]'s [limb]!</span>")
