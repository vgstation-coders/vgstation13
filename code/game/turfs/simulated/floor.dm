...
/turf/simulated/floor/attackby(obj/item/C as obj, mob/user as mob)
	...
	if(istype(C, /obj/item/weapon/crowbar) && (!(is_plating())))
		if(broken || burnt)
			user << "<span class=\"rose\">You remove the broken plating.</span>"
		else
			if(is_wood_floor())
				user << "<span class=\"rose\">You forcefully pry off the planks, destroying them in the process.</span>"
			else
				user << "<span class=\"rose\">You remove the [floor_tile.name].</span>"
	...
	if(istype(C, /obj/item/weapon/screwdriver))
		if(is_wood_floor())
			if(broken || burnt)
				...
				if(is_wood_floor())
					user << "<span class=\"rose\">You unscrew the planks.</span>"
					new floor_tile.type(src)
	...
	if(istype(C, /obj/item/stack/rods))
		...
		if (is_plating())
			if (R.amount >= 2)
				...
				user << "<span class=\"rose\">You need more rods.</span>"
		else if (is_catwalk())
			user << "<span class=\"rose\">The entire thing is 100% rods already, it doesn't need any more.</span>"
		else
	...
	if(istype(C, /obj/item/stack/tile))
		if (is_catwalk())
			user << "<span class=\"rose\">The catwalk is too primitive to support tiling.</span>"
		if(is_plating())
			if(!broken && !burnt)
	...
	if(istype(C, /obj/item/weapon/shovel))
		if(is_grass_floor())
			...
			user << "<span class=\"rose\">You cannot shovel this.</span>"
		...
		if(welder.isOn() && (is_plating()))
			if(broken || burnt)
				if(welder.remove_fuel(0,user))
					user << "<span class=\"rose\">You fix some dents on the broken plating.</span>"
