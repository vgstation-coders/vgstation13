...
/datum/job/hobo
	...
	access = list()
	minimal_access = list()
	alt_titles = list("Vagabond")

	must_be_map_enabled = 1

	no_random_roll = 1 //Don't become a hobo randomly
	no_crew_manifest = 1
	...
	outfit_datum = /datum/outfit/hobo

/datum/job/hobo/introduce(mob/living/carbon/human/M, job_title)
	if(!job_title)
		job_title = src.title
