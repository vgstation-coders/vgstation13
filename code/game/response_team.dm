...
/client/proc/response_team()
	...
	if(!holder)
		usr << "<span class=\"rose\">Only administrators may use this command.</span>"
		return
	if(!ticker)
		usr << "<span class=\"rose\">The game hasn't started yet!</span>"
		return
	if(ticker.current_state == 1)
		usr << "<span class=\"rose\">The round hasn't started yet!</span>"
		return
	if(send_emergency_team)
		usr << "<span class=\"rose\">Central Command has already dispatched an emergency response team!</span>"
		return
	...
	if(get_security_level() != "red") // Allow admins to reconsider if the alert level isn't Red
		switch(alert("The station is not in red alert. Do you still want to dispatch a response team?",,"Yes","No"))
			if("No")
				return
	if(send_emergency_team)
		usr << "<span class=\"rose\">Looks like somebody beat you to it!</span>"
		return
...
/client/proc/create_response_team(obj/spawn_location, leader_selected = 0, commando_name)
	...
	//usr << "<span class=\"rose\">ERT has been temporarily disabled. Talk to a coder.</span>"
	//return
