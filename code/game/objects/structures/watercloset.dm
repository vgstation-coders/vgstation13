...
/obj/structure/sink/attack_hand(mob/M as mob)
	...
	if(busy)
		M << "<span class=\"rose\">Someone's already washing here.</span>"
		return
...
/obj/structure/sink/attackby(obj/item/O as obj, mob/user as mob)
	if(busy)
		user << "<span class=\"rose\">Someone's already washing here.</span>"
		return
	...
	if (istype(O, /obj/item/weapon/reagent_containers))
		...
		if (B.bcell && B.bcell.charge > 0 && B.status == 1)
			...
			user.visible_message( \
				...
				"<span class=\"rose\">You have wet \the [O], it shocks you!</span>")
			return
