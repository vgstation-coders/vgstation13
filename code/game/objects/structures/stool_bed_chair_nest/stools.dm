...
/obj/structure/stool/MouseDrop(atom/over_object)
	if (istype(over_object, /mob/living/carbon/human))
		...
		if (!H.restrained() && !H.stat && in_range(src, over_object))
			...
			src.loc = S
			H.put_in_hands(S)
			H.visible_message("<span class=\"rose\">[H] grabs [src] from the floor!</span>", "<span class=\"rose\">You grab [src] from the floor!</span>")
...
/obj/item/weapon/stool/attack(mob/M as mob, mob/user as mob)
	if (prob(5) && istype(M,/mob/living))
		user.visible_message("<span class=\"rose\">[user] breaks [src] over [M]'s back!.</span>")
		user.u_equip(src)
