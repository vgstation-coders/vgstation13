...
/obj/structure/grille/Bumped(atom/user)
	if(ismob(user))
		var/mob/M = user
		shock(M, 60) //Give the user the benifit of the doubt
		if (M.client)
			if(M.pulledby)
				...
				add_logs(M.last_bumped_by_timestamp, M, "bumped into a shocked grille", TRUE, src, get_coordinates_string(src))
			else
