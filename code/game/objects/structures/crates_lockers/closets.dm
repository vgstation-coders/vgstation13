...
/obj/structure/closet/attack_hand(mob/user as mob)
	...
	if(opened==0 && L && L.client && L.hallucinating()) //If the closet is CLOSED and user is hallucinating
		if(prob(10))
			var/client/C = L.client
			var/image/temp_overlay = image(icon, icon_state=icon_opened) //Get the closet's OPEN icon
