...
/obj/item/clothing/suit/storage/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(contents.len >= storage_slots)
		user << "<span class=\"rose\">\The [src] is full, make some space.</span>"
	...
	if(can_hold.len)
		...
		for(var/A in can_hold)
			if(istype(W, text2path(A) ))
				...
				break
		if(!ok)
			user << "<span class=\"rose\">\The [src] cannot hold \the [W].</span>"
			return
	...
	if (W.w_class > max_w_class)
		user << "<span class=\"rose\">\The [W] is too big for \the [src]</span>"
		return
	...
	if(W.w_class >= src.w_class && (istype(W, /obj/item/weapon/storage)))
		if(!istype(src, /obj/item/weapon/storage/backpack/holding))	//bohs should be able to hold backpacks again. The override for putting a boh in a boh is in backpack.dm.
			user << "<span class=\"rose\">\The [src] cannot hold \the [W] as it's a storage item of the same size.</span>"
