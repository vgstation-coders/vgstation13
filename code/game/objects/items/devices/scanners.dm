...
[(M.undergoing_hypothermia() || (M.bodytemperature > M.species.heat_level_1)) ?  "<span class='warning'>" : "<span class='notice'>"]Body Temperature: ???&deg;C (???&deg;F)</span>
...
Subject's pulse: ??? BPM"})
	...
	message += "<br>[(M.undergoing_hypothermia() || (M.bodytemperature > M.species.heat_level_1)) ?  "<span class='warning'>" : "<span class='notice'>"]Body Temperature: [round(M.bodytemperature-T0C,0.1)]&deg;C ([round(M.bodytemperature*1.8-459.67,0.1)]&deg;F)</span>"
	if(M.tod && M.isDead())
