...
/obj/item/device/modkit/afterattack(obj/O, mob/user as mob)
	...
	user.visible_message("<span class=\"rose\">[user] opens \the [src] and modifies \the [O] into \the [N].</span>","<span class=\"rose\">You open \the [src] and modify \the [O] into \the [N].</span>")
	del(O)
	parts &= ~flag
	if(!parts)
		del(src)
