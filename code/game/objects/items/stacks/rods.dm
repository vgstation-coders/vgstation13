...
/obj/item/stack/rods/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/weldingtool))
		...
		if(amount < 2)
			user << "<span class=\"rose\">You need at least two rods to do this.</span>"
			return
		...
		if(WT.remove_fuel(0,user))
			...
			for (var/mob/M in viewers(src))
				M.show_message("<span class=\"rose\">[src] is shaped into metal by [user.name] with the weldingtool.</span>", 3, "<span class=\"rose\">You hear welding.</span>", 2)
