	...
	if (href_list["make"])
		...
		if (src.amount < R.req_amount*multiplier)
			if (R.req_amount*multiplier>1)
				usr << "<span class=\"rose\">You haven't got enough [src] to build \the [R.req_amount*multiplier] [R.title]\s!</span>"
			else
				usr << "<span class=\"rose\">You haven't got enough [src] to build \the [R.title]!</span>"
			return
		if (R.one_per_turf && (locate(R.result_type) in usr.loc))
			usr << "<span class=\"rose\">There is another [R.title] here!</span>"
			return
		if (R.on_floor && !istype(usr.loc, /turf/simulated/floor))
			usr << "<span class=\"rose\">\The [R.title] must be constructed on the floor!</span>"
			return
		if (R.time)
