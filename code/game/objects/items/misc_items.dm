...
/obj/item/folded_bag/attack_self(mob/user)
	...
	var/bag = new/obj/item/storage/bag/plasticbag(user.loc)
	user.u_equip(src)
...
/obj/item/gingerbread_egg/proc/candy_spawn(var/mob/user)
	if(world.time - last_candy_time >= candy_delay)
		...
		switch(rand(1,10))
			if(1,2)
				new /obj/item/reagent_containers/food/snacks/candy_cane(get_turf(user))
			if(3,4)
				new /obj/item/reagent_containers/food/snacks/gingerbread_man(get_turf(user))
			if(5 to 9)
