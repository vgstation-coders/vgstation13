...
/obj/item/robot_spawner/update_icon()
	if(has_icon(icon, "[initial(icon_state)]-searching"))
		icon_state = "[initial(icon_state)][busy ? "-searching" : ""]"

/obj/item/robot_spawner/attack_self(mob/user)
	request_borg(user)

/obj/item/robot_spawner/check_uplink_validity()
	return charge > 0 ? TRUE : FALSE

/obj/item/robot_spawner/attack_ghost(var/mob/dead/observer/O)
...
/obj/item/robot_spawner/proc/request_borg(var/mob/user)
	if(!charge)
...
/obj/item/robot_spawner/proc/recruiter_recruiting(var/list/args)
...
/obj/item/robot_spawner/proc/recruiter_not_recruiting(var/list/args)
	var/mob/dead/observer/O = args["player"]
	var/controls = args["controls"]
	var/area/A = get_area(src)
	to_chat(O, "<span class='recruit'>\The [name] activated at \the [A.name]. ([controls])</span>")

/obj/item/robot_spawner/proc/recruiter_recruited(var/list/args)
	var/mob/dead/observer/O = args["player"]
	if(O)
		qdel(recruiter)
		recruiter = null
...
/obj/item/robot_spawner/proc/post_recruited(var/mob/living/silicon/robot/R)
	if(!R) //Knock it off.
		return
	if(faction)
		R.faction = faction
...
/obj/item/robot_spawner/syndicate
...
/obj/item/robot_spawner/syndicate/post_recruited(mob/living/silicon/robot/R)
	..()
...
/obj/item/robot_spawner/strange
	icon = 'icons/obj/assemblies.dmi'
	autoqdel = TRUE

/obj/item/robot_spawner/strange/post_recruited(mob/living/silicon/robot/R)
	..()
	investigation_log(I_ARTIFACT, "|| [key_name(R)] spawned as [R.module.name].")

/obj/item/robot_spawner/strange/ball
	name = "strange ball"
...
/obj/item/robot_spawner/strange/egg
	name = "strange egg"
...
/obj/item/robot_spawner/strange/egg/attack_hand(mob/user)
	if(ishuman(user))
...
/obj/item/robot_spawner/strange/egg/attack_paw(mob/user)
	return