...
/obj/item/robot_parts/robot_suit/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/device/mmi) || istype(W, /obj/item/device/mmi/posibrain))
		...
		if(check_completion())
			if(!istype(loc,/turf))
				user << "<span class=\"rose\">You can't put the [W] in, the frame has to be standing on the ground to be perfectly precise.</span>"
				return
			if(!M.brainmob)
				user << "<span class=\"rose\">Sticking an empty [W] into the frame would sort of defeat the purpose.</span>"
				return
			if(!M.brainmob.key)
			...
			if(M.brainmob.stat == DEAD)
				user << "<span class=\"rose\">Sticking a dead [W] into the frame would sort of defeat the purpose.</span>"
				return

			if(M.brainmob.mind in ticker.mode.head_revolutionaries)
				user << "<span class=\"rose\">The frame's firmware lets out a shrill sound, and flashes 'Abnormal Memory Engram'. It refuses to accept the [W].</span>"
				return

			if(jobban_isbanned(M.brainmob, "Cyborg"))
				user << "<span class=\"rose\">This [W] does not seem to fit.</span>"
				return
...
/obj/item/robot_parts/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/weapon/card/emag))
		if(sabotaged)
			user << "<span class=\"rose\">[src] is already sabotaged!</span>"
		else
			user << "<span class=\"rose\">You slide [W] into the dataport on [src] and short out the safeties.</span>"
			sabotaged = 1
		return
	..()