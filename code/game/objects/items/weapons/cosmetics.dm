...
/obj/item/weapon/invisible_spray/preattack(atom/movable/target, mob/user, proximity_flag, click_parameters)
	...
	if(M == user)
		...
		user.make_invisible(INVISIBLESPRAY, invisible_time, FALSE, 1, INVISIBILITY_LEVEL_ONE)
	else if (ismob(M))
		to_chat(user, "You spray [M] with \the [src].")
		M.make_invisible(INVISIBLESPRAY, invisible_time, FALSE, 1, INVISIBILITY_LEVEL_ONE)
	var/obj/O = target
	if(isobj(O))
		to_chat(user, "You spray \the [O] with \the [src].")
		O.make_invisible(INVISIBLESPRAY, invisible_time, 1, INVISIBILITY_LEVEL_ONE)
