...
/obj/item/table_parts/cultify()
	new /obj/item/table_parts/wood(loc)
	..()

/obj/item/table_parts/attackby(obj/item/W, mob/user)
	..()
	if (W.is_wrench(user))
	...
	if (istype(W, /obj/item/stack/sheet/glass/glass))
		...
		if (glass.amount >= 1)
			new /obj/item/table_parts/glass(user.loc)
	...
	if (istype(W, /obj/item/stack/sheet/glass/plasmaglass))
		...
		if (plasma.amount >= 1)
			new /obj/item/table_parts/glass/plasma(user.loc)
...
/obj/item/table_parts/preattack(atom/target, mob/user, proximity_flag, click_parameters)
	. = 1
	if(!proximity_flag)
		return
	if(!isturf(target))
...
/obj/item/table_parts/clockworkify()
	GENERIC_CLOCKWORK_CONVERSION(src, /obj/item/table_parts/clockwork, CLOCKWORK_GENERIC_GLOW)
...
/obj/item/table_parts/reinforced/attackby(obj/item/W, mob/user)
	if (W.is_wrench(user))
...
/obj/item/table_parts/wood/cultify()
	return

/obj/item/table_parts/wood/attackby(obj/item/W, mob/user)
	if (W.is_wrench(user))
		drop_stack(sheet_type, user.loc, 1, user)
		qdel(src)
		return
...
/obj/item/table_parts/glass/attackby(obj/item/W, mob/user)
	if (W.is_wrench(user))
		drop_stack(/obj/item/stack/sheet/glass/glass, loc, 1, user)
		drop_stack(sheet_type, loc, 1, user)
		qdel(src)

/obj/item/table_parts/glass/plasma
	name = "glass table parts"
...
/obj/item/table_parts/glass/plasma/attackby(obj/item/W, mob/user)
	if (W.is_wrench(user))
		drop_stack(/obj/item/stack/sheet/glass/plasmaglass, loc, 1, user)
		drop_stack(sheet_type, loc, 1, user)
		qdel(src)

/obj/item/table_parts/clockwork
...
/obj/item/table_parts/clockwork/cultify()
	return

/obj/item/table_parts/clockwork/clockworkify()
	return

/obj/item/table_parts/plastic
...
/obj/item/rack_parts/attackby(obj/item/W, mob/user)
	..()
	if (W.is_wrench(user))
		drop_stack(sheet_type, user.loc, sheet_amount, user)
		qdel(src)
	...
	if(iswelder(W))
		...
		if(WT.remove_fuel(1, user))
			...
			if(do_after(user, src, 60))
				...
				if(src.loc == user)
					user.drop_item(src, force_drop = 1)
					var/obj/item/metal_gun_stock/I = new (user.loc)
					user.put_in_hands(I)
					qdel(src)
				else
					new /obj/item/metal_gun_stock(loc)
					qdel(src)
			return
