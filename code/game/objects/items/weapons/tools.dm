...
/obj/item/weapon/tool/wrench/is_wrench(mob/user)
	return TRUE

/obj/item/weapon/tool/wrench/attackby(obj/item/weapon/W, mob/user)
	..()
	if(user.is_in_modules(src))
		return
	if(istype(W, /obj/item/weapon/handcuffs/cable) && !istype(src, /obj/item/weapon/tool/wrench/socket))
		to_chat(user, "<span class='notice'>You wrap the cable restraint around the top of the wrench.</span>")
		if(src.loc == user)
			user.drop_item(src, force_drop = 1)
			var/obj/item/weapon/tool/wrench_wired/I = new (get_turf(user))
			user.put_in_hands(I)
		else
			new /obj/item/weapon/tool/wrench_wired(get_turf(src.loc))
		qdel(src)
		qdel(W)

//we inherit a lot from wrench, so we change very little
/obj/item/weapon/tool/wrench/socket
	name = "socket wrench"
...
/obj/item/weapon/tool/screwdriver/attackby(var/obj/O)
...
/obj/item/weapon/tool/screwdriver/is_screwdriver(var/mob/user)
	return TRUE
/*
 * Wirecutters
 */
...
/obj/item/weapon/tool/weldingtool/suicide_act(mob/user)
...
/obj/item/weapon/tool/weldingtool/proc/do_weld(var/mob/user, var/atom/thing, var/time, var/fuel_cost)
...
/obj/item/weapon/tool/weldingtool/attack(mob/M as mob, mob/user as mob)
	if(hasorgans(M))
		if(can_operate(M, user))
...
/obj/item/weapon/tool/weldingtool/afterattack(atom/A, mob/user as mob, proximity)
	if(!proximity)
		return
...
/obj/item/weapon/tool/weldingtool/is_sharp()
	if(isOn())
		return sharpness
	return 0
...
/obj/item/weapon/tool/weldingtool/proc/setWelding(var/temp_welding)
...
/obj/item/weapon/tool/weldingtool/proc/check_fuel()
...
/obj/item/weapon/tool/weldingtool/proc/toggle(var/mob/user)
	if(!status)
...
/obj/item/weapon/tool/weldingtool/send_to_past(var/duration)
	..()
...
/obj/item/weapon/tool/weldingtool/hugetank/mech
	name = "welding tool"
	eye_damaging = FALSE

/obj/item/weapon/tool/weldingtool/gatling
	name = "gatling welder"
...
/obj/item/weapon/tool/weldingtool/gatling/empty
	start_fueled = 0
...
/obj/item/weapon/tool/weldingtool/experimental/empty
	start_fueled = 0

/obj/item/weapon/tool/weldingtool/experimental/process()
	..()
...
/obj/item/weapon/tool/crowbar/suicide_act(mob/user)
...
/obj/item/weapon/tool/crowbar/red/suicide_act(mob/user)
...
/obj/item/weapon/tool/solder/New()
	. = ..()
	create_reagents(max_fuel)
	//Does not come fueled up

/obj/item/weapon/tool/solder/update_icon()
	..()
...
/obj/item/weapon/tool/solder/examine(mob/user)
	..()
...
/obj/item/weapon/tool/solder/attackby(obj/item/W as obj, mob/user as mob)
...
/obj/item/weapon/tool/solder/proc/remove_fuel(var/amount, mob/user as mob)
...
/obj/item/weapon/tool/solder/pre_fueled/New()
	. = ..()
