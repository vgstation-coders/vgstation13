...
/obj/item/wrench/is_wrench(mob/user)
	return TRUE

/obj/item/wrench/attackby(obj/item/W, mob/user)
	..()
	if(user.is_in_modules(src))
		return
	if(istype(W, /obj/item/handcuffs/cable) && !istype(src, /obj/item/wrench/socket))
		to_chat(user, "<span class='notice'>You wrap the cable restraint around the top of the wrench.</span>")
		if(src.loc == user)
			user.drop_item(src, force_drop = 1)
			var/obj/item/wrench_wired/I = new (get_turf(user))
			user.put_in_hands(I)
		else
...
/obj/item/screwdriver/attackby(var/obj/O)
	if(istype(O, /obj/item/stack/cable_coil))
		...
		if(ishuman(M) && !M.restrained() && !M.stat && !M.paralysis && ! M.stunned)
			...
			if(C.amount < 10)
				...
				return
			var/obj/item/legcuffs/bolas/cable/B = new /obj/item/legcuffs/bolas/cable(usr.loc)
			qdel(src)
...
/obj/item/screwdriver/is_screwdriver(var/mob/user)
	return TRUE
/*
 * Wirecutters
 */
...
/obj/item/wirecutters/attack(mob/living/carbon/C as mob, mob/user as mob)
	if((iscarbon(C)) && (C.handcuffed) && (istype(C.handcuffed, /obj/item/handcuffs/cable)) && user.a_intent!=I_HURT)
...
/obj/item/weldingtool/attackby(obj/item/W as obj, mob/user as mob)
	...
	if((!status) && (istype(W,/obj/item/stack/rods)))
		...
		R.use(1)
		var/obj/item/gun/projectile/flamethrower/F = new/obj/item/gun/projectile/flamethrower(user.loc)
		src.forceMove(F)
		F.weldtool = src
		if (user.client)
...
/obj/item/weldingtool/proc/do_weld(var/mob/user, var/atom/thing, var/time, var/fuel_cost)
...
/obj/item/weldingtool/afterattack(atom/A, mob/user as mob, proximity)
	if(!proximity)
		return
...
/obj/item/weldingtool/proc/setWelding(var/temp_welding)
...
/obj/item/weldingtool/send_to_past(var/duration)
	..()
...
/obj/item/weldingtool/hugetank/mech
	name = "welding tool"
	eye_damaging = FALSE

/obj/item/weldingtool/gatling
	name = "gatling welder"
...
/obj/item/weldingtool/gatling/empty
	start_fueled = 0
...
/obj/item/weldingtool/experimental/empty
	start_fueled = 0
...
/obj/item/conversion_kit/attack_self(mob/user as mob)
	open = !open
...
/obj/item/solder/examine(mob/user)
	..()
...
/obj/item/solder/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/reagent_containers/) && W.flags & OPENCONTAINER)
		var/obj/item/reagent_containers/G = W
...
/obj/item/solder/proc/remove_fuel(var/amount, mob/user as mob)
...
/obj/item/solder/pre_fueled/New()
	. = ..()
...
*/
/obj/item/reagent_containers/glass/fuelcan
	name = "fuel can"
...
/obj/item/reagent_containers/glass/fuelcan/New()
	..()
	slotzero = reagents
...
/obj/item/reagent_containers/glass/fuelcan/attack_self(mob/user as mob)
	if(!slot)
		slotzero = reagents
		reagents = slotone
	else
...
/obj/item/reagent_containers/glass/fuelcan/examine(mob/user)
	..()
...
/obj/item/reagent_containers/glass/fuelcan/update_icon()
