...
/obj/item/weapon/book/manual/virology_guide
	...
	dat = {"<html>
				...
				<p>Getting infected through the <b>Air</b> occurs from breathing a pathogenic cloud, emitted by another infected individual, an open growth dish, or some other occurences.</p>
				...
				<p>If you work for a megacorporation like Nanotrasen, chances are that there are some growth dishes lying in the virology lab waiting to be analyzed. It's nice to keep you busy for a while, but you might want to work on different forms of pathogens, or ones with a completely different symptomes without having to wait for ages while incubating mutagen.</p>
				...
				<p><b>Note however that dishes that come from either Virology of Cargo tend to have relatively high strength, making them more complicated to cure for the uninitiated, and also a problem for the crew if they have dangerous symptoms, which they often have. So if you value your job (and your head) do not allow those diseases to get loose.</b></p>
				...
				<li>this information is also added to the Pathogen Database which you can edit, and also give the disease a Nickname or set it's dangerosity.
				<li>diseases analysed this way will now show up on medical HUDs, the icon will depend on the dangerosity set in the database.
