...
/obj/item/weapon/book/manual/security_antag_guide/New(turf/loc)
	...
	dat = {"<html>
				...
				<p>Rumours from less explored parts of the Galaxy talk about the so-called "masquerade" and its recent end. We don't know for sure if the vampires it refers to awakened out here in space, or if they are an old breed coming from Earth. In any case, they seem to exhibit a tendency to try and turn our space stations into their own personal fiefdoms - and blood banks. These undead lords have displayed a variety of powers, including summoning bats, hypnosis, and extensive healing capabilities even from a seemingly dead state.
