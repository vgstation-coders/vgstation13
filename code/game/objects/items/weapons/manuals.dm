...
/obj/item/weapon/book/manual/security_antag_guide/New(turf/loc)
	...
	dat = {"<html>
				...
				<p>We didn't want to believe it ourselves at first, but it has come to our attention that some employees have displayed what can only be described as "vampiric powers". We don't know for sure if they awakened those abilities here in space, or if they came from Earth and managed to fit among the migrants to our system, but they seem to exhibit a tendency to try and turn our space stations into their own personal kingdoms and blood banks, so we can't have that. Some of the powers we've observed include the ability to turn into bats, hypnotize humans into a paralysis, turn those humans into their thralls, and heal all of their own wounds even from a seemingly dead state.
				...
				<h3>Thralls</h3>
				<p>Thralls are poor crewmembers who have been subjugated by a Vampire lord's powers into servitude. They will try to accomplish the will of their master and fight to defend them. But Thralls are not beyond saving: blessed water has often been reported to be a potent antidote to a Vampire's influence.
