...
/obj/item/weapon/book/manual/security_antag_guide/New(turf/loc)
	...
	dat = {"<html>
        ...
        <ul><li>* After the blob spreads over 20% of the station, DEFCON 3 triggers and the Red alert is automatically triggered.</li><li>* After 30%, DEFCON 2 triggers allowing the crew to call a second ERT, and cyborgs to use a new module.</li><li>* After 40%, DEFCON 1 triggers, granting the crew access to most rooms, and enabling the communications computer to request supplies.</li><li>* If the blob spreads over 66.6%, Directive 7-12 is authorized, enabling the crew to self-destruct the station using its nuclear fission device. The nuclear authentication code is provided to everyone, but they still need to get a hold of the nuclear fission explosive itself and its disk.</li></ul>
