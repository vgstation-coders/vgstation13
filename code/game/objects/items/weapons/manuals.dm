...
/obj/item/weapon/book/manual/security_antag_guide/New(turf/loc)
	...
	dat = {"<html>
				...
				<h3>Challengers</h3>
				<p>The Syndicate trains its novice agents through a Battle Royale where they have to eliminate each others aboard one of our stations. On the bright side, those challengers are here to kill each other. On the downside, they don't care about how much collateral damage they cause, AND the victor may graduate to become a fully fledged Syndicate agent, enabling them to cause even more trouble. We do not want that. Challengers often grimly display the corpse of their target on Newscasters, use this opportunity to try and figure out the identity of the victor so you may deal with them.
