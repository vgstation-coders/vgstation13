...
/obj/item/weapon/stock_parts/capacitor/examine(mob/user)
	..()
	if(stored_charge)
		to_chat(user, "<span class='notice'>\The [name] is charged to [stored_charge]W.</span>")
...
/obj/item/weapon/stock_parts/capacitor/attackby(obj/item/weapon/W, mob/user)
	if(iswrench(W))
		if(!istype(loc, /turf))
			...
			return
		if(!istype(loc, /turf/simulated/floor)) //Prevent from anchoring this to shuttles / space
			to_chat(user, "<span class='notice'>You can't secure \the [src] to [istype(loc,/turf/space) ? "space" : "this"]!</span>")
			return
		to_chat(user, "You discharge \the [src] and secure it to the floor.")
		playsound(get_turf(src), 'sound/items/Ratchet.ogg', 50, 1)
		switch(type)
			if(/obj/item/weapon/stock_parts/capacitor)
				new /obj/machinery/power/secured_capacitor(get_turf(loc))
			if(/obj/item/weapon/stock_parts/capacitor/adv)
				new /obj/machinery/power/secured_capacitor/adv(get_turf(loc))
			if(/obj/item/weapon/stock_parts/capacitor/adv/super)
				new /obj/machinery/power/secured_capacitor/adv/super(get_turf(loc))
		qdel(src)
