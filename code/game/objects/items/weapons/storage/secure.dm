	...
	attackby(obj/item/weapon/W as obj, mob/user as mob)
		if(locked)
			...
			if ((istype(W, /obj/item/device/multitool)) && (src.open == 1)&& (!src.l_hacking))
				user.show_message(text("<span class=\"rose\">Now attempting to reset internal memory, please hold.</span>"), 1)
				src.l_hacking = 1
				if (do_after(usr, 100))
					if (prob(40))
						src.l_setshort = 1
						src.l_set = 0
						user.show_message(text("<span class=\"rose\">Internal memory reset.  Please give it a few seconds to reinitialize.</span>"), 1)
						sleep(80)
						src.l_setshort = 0
						src.l_hacking = 0
					else
						user.show_message(text("<span class=\"rose\">Unable to reset internal memory.</span>"), 1)
						src.l_hacking = 0
...
/obj/item/weapon/storage/secure/briefcase
	...
	attack_hand(mob/user as mob)
		if ((src.loc == user) && (src.locked == 1))
			usr << "<span class=\"rose\">[src] is locked and cannot be opened!</span>"
			...
			if(ishuman(M))
				...
				// ******* Check
					if (istype(H, /obj/item/clothing/head) && H.flags & 8 && prob(80))
						H << "<span class=\"rose\">The helmet protects you from being hit hard in the head!</span>"
						return
					var/time = rand(2, 6)
					if (prob(75))
						H.Paralyse(time)
					...
					for(var/mob/O in viewers(H, null))
						O.show_message(text("<span class=\"danger\">[] has been knocked unconscious!</span>", H), 1, "<span class=\"rose\">You hear someone fall.</span>", 2)
				else
					H << text("<span class=\"rose\">[] tried to knock you unconcious!</span>",user)
					H.eye_blurry += 3
