...
/obj/item/storage/lockbox/can_use()
	return broken || !locked

/obj/item/storage/lockbox/attack_robot(var/mob/user)
	to_chat(user, "<span class='rose'>This box was not designed for use by non-organics.</span>")
	return

/obj/item/storage/lockbox/proc/toggle(var/mob/user, var/id_name)
	if(allowed(user))
		. = TRUE
		locked = !locked
...
/obj/item/storage/lockbox/oneuse/toggle(var/mob/user, var/id_name)
	. = ..()
	if (.)
...
/obj/item/storage/lockbox/emag_act(var/mob/user)
	if (broken)
		return FALSE
	broken = 1
	locked = 0
...
/obj/item/storage/lockbox/oneuse/emag_act(var/mob/user)
	. = ..()
	if (.)
		for(var/atom/movable/A in src)
			remove_from_storage(A, get_turf(src))
...
/obj/item/storage/lockbox/bullet_act(var/obj/item/projectile/Proj)
	// WHY MUST WE DO THIS
	// WHY
...
/obj/item/storage/lockbox/ex_act(severity)
	switch(severity)
		if(1)
			qdel(src)
		if(2)
...
/obj/item/storage/lockbox/oneuse/emp_act(severity)
	. = ..()
	if (.)
		qdel(src)
...
/obj/item/storage/lockbox/exile
	name = "lockbox (exile implants)"
	req_one_access = list(access_armory)

/obj/item/storage/lockbox/exile/New()
	..()
	new /obj/item/implantcase/exile(src)
	new /obj/item/implantcase/exile(src)
	new /obj/item/implanter/exile(src)

/obj/item/storage/lockbox/tracking
	name = "lockbox (tracking implants)"
	req_one_access = list(access_security)
	storage_slots = 5

/obj/item/storage/lockbox/tracking/New()
	..()
...
/obj/item/storage/lockbox/chem
	name = "lockbox (chemical implants)"
	req_one_access = list(access_security)
	storage_slots = 5

/obj/item/storage/lockbox/chem/New()
	..()
...
/obj/item/storage/lockbox/unlockable
...
/obj/item/storage/lockbox/unlockable/attackby(obj/O as obj, mob/user as mob)
...
/obj/item/storage/lockbox/coinbox
	name = "coinbox"
	desc = "A secure container for the profits of a vending machine."
	icon_state = "coinbox+l"
	w_class = W_CLASS_SMALL
	can_only_hold = list("/obj/item/voucher","/obj/item/coin","/obj/item/reagent_containers/food/snacks/customizable/candy/coin","/obj/item/reagent_containers/food/snacks/chococoin")
...
/obj/item/storage/lockbox/lawgiver/New()
	..()
	new /obj/item/gun/lawgiver(src)

/obj/item/storage/lockbox/lawgiver/with_magazine/New()
	..()
	new /obj/item/ammo_storage/magazine/lawgiver(src)

/obj/item/storage/lockbox/oneuse
	desc = "A locked box. When unlocked, the case will fall apart."

/obj/item/storage/lockbox/AltClick()
	if(verb_togglelock())
		return
	return ..()

/obj/item/storage/lockbox/verb/verb_togglelock()
...
/obj/item/storage/lockbox/diskettebox
	name = "diskette box"
	...
	item_state = "diskbox"
	can_only_hold = list("/obj/item/disk")
	cant_hold = list("/obj/item/disk/harddiskdrive")
	fits_max_w_class = 3
...
/obj/item/storage/lockbox/diskettebox/New()
	..()
	update_icon()

/obj/item/storage/lockbox/diskettebox/open
	icon_state = "map_diskbox_open"
	locked = FALSE

/obj/item/storage/lockbox/diskettebox/large
...
/obj/item/storage/lockbox/diskettebox/large/open
	icon_state = "map_diskbox_large_open"
	locked = FALSE

//---------------------------------PRESETS---------------------------------

/obj/item/storage/lockbox/diskettebox/open/botanydisk
	name = "flora diskette box"
	desc = "A lockable box of flora data disks."

/obj/item/storage/lockbox/diskettebox/open/botanydisk/New()
	..()
	for(var/i = 1 to storage_slots)
		new /obj/item/disk/botany(src)
	update_icon()

/obj/item/storage/lockbox/diskettebox/large/open/botanydisk
	name = "large flora diskette box"
	desc = "A large lockable box of flora data disks."

/obj/item/storage/lockbox/diskettebox/large/open/botanydisk/New()
	..()
	for(var/i = 1 to storage_slots)
		new /obj/item/disk/botany(src)
	update_icon()

/obj/item/storage/lockbox/diskettebox/open/cloning
	name = "cloning diskette box"
	desc = "A lockable box of cloning data disks."

/obj/item/storage/lockbox/diskettebox/open/cloning/New()
	..()
	for(var/i = 1 to storage_slots)
		new /obj/item/disk/data(src)
	update_icon()

/obj/item/storage/lockbox/diskettebox/open/research
	name = "research diskette box"
	desc = "A lockable box of tech data disks."

/obj/item/storage/lockbox/diskettebox/open/research/New()
	..()
	new /obj/item/disk/tech_disk(src)
	new /obj/item/disk/tech_disk(src)
	new /obj/item/disk/design_disk(src)
	new /obj/item/disk/design_disk(src)
	update_icon()

/obj/item/storage/lockbox/diskettebox/open/blanks/New()
	..()
...
/obj/item/storage/lockbox/diskettebox/large/open/blanks/New()
	..()
	var/j = rand(1,round(storage_slots/2))//up to half the slots filled with useless disks
	for(var/i = 1 to j)
		new /obj/item/disk(src)
...
/obj/item/storage/lockbox/diskettebox/update_icon()
	overlays.len = 0
	...
	var/i = 0
	for (var/obj/item/disk/disk in contents)
...
/obj/item/storage/lockbox/diskettebox/attackby(obj/item/W as obj, mob/user as mob)
	. = ..()
	if (istype(W,/obj/item/card))
		playsound(src, get_sfx("card_swipe"), 60, 1, -5)
	update_icon()

/obj/item/storage/lockbox/diskettebox/handle_item_insertion(obj/item/W as obj, prevent_warning = 0)
	.=..()
	if (.)
		playsound(loc, 'sound/machines/click.ogg', 30, -5)

/obj/item/storage/lockbox/security_sponsored
	name = "sponsored Team Security lockbox"
	desc = "A sponsor-sticker-plastered lockbox."
	req_one_access = list(access_brig)
	storage_slots = 6

/obj/item/storage/lockbox/security_sponsored/New()
	..()
	for(var/i in 1 to 4)
		new /obj/item/reagent_containers/food/snacks/donitos/coolranch(src)
	new /obj/item/implanter/spesstv(src)

/obj/item/storage/lockbox/team_security_cameras
	name = "sponsored Team Security cameras lockbox"
	desc = "A sponsor-sticker-plastered lockbox."
	req_one_access = list(access_brig)
	storage_slots = 6

/obj/item/storage/lockbox/team_security_cameras/New()
	..()
	for(var/i in 1 to 6)
		new /obj/item/clothing/accessory/spesstv_tactical_camera(src)

/obj/item/storage/lockbox/advanced
...
/obj/item/storage/lockbox/advanced/ex_act()
	react()

/obj/item/storage/lockbox/advanced/emp_act()
	react()

/obj/item/storage/lockbox/advanced/emag_act()
	react()

/obj/item/storage/lockbox/advanced/bullet_act(var/obj/item/projectile/P)
...
/obj/item/storage/lockbox/advanced/proc/react()
	teleport_radius(6)
...
/obj/item/storage/lockbox/advanced/energyshotgun/New()
	..()
	req_one_access = list(access_security)
	new /obj/item/gun/energy/shotgun(src)
