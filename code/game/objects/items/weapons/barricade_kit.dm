...
/obj/item/weapon/barricade_kit/attack_self(mob/user as mob)
	...
	switch(alert("What do you want ([kit_uses] use\s left)", "Barricade Kit", "Directional", "Full Tile", "Cancel", null))
		if("Directional")
			...
			for(var/obj/structure/barricade/BC in user.loc)
			...
			if(do_after(user, src, 30))
				...
				var/obj/structure/barricade/BC = new /obj/structure/barricade(user.loc)
				BC.dir = user.dir
				kit_uses--
				if(kit_uses < 1)
					qdel(src)
		...
		if("Full Tile")
			...
			if(kit_uses < 3)
				...
				return 1
			if(locate(/obj/structure/barricade/full) in user.loc)
			...
			if(do_after(user, src, 50))
				...
				new /obj/structure/barricade/full(user.loc)
				qdel(src) //Use it up
...
/obj/item/weapon/barricade_kit/preattack(var/atom/A, mob/user as mob)
	if(istype(A, /obj/machinery/door/airlock) || istype(A, /obj/structure/window/full)) //Apply on a full window or an airlock
		...
		for(var/obj/structure/S in T)
			if(istype(S, /obj/structure/barricade/full/block))
		...
		if(do_after(user, T, 30))
			new /obj/structure/barricade/full/block(get_turf(T))
			user.visible_message("<span class='warning'>[user] barricades [A].</span>", \
			"<span class='notice'>You barricade [A].</span>")
			kit_uses--
			if(kit_uses < 1)
	...
	if(istype(A, /obj/structure/window) && !istype(A, /obj/structure/window/full)) //Windows
		...
		..() //I guess if you want to bash people with it
