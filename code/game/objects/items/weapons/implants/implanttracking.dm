...
/obj/item/device/locator_holomap/proc/update_holomap()
	...
	if (lastZ != T.z)
		...
		if(!((HOLOMAP_EXTRA_STATIONMAP+"_[T.z]") in extraMiniMaps))
			...
			if(!(holomap_bgmap in holomap_cache))
				holomap_datum.station_map = image('icons/480x480.dmi', "stationmap")
	...
	if(T.z == STATION_Z || T.z == ASTEROID_Z || T.z == DERELICT_Z)
		var/image/station_outline = image(holoMiniMaps[T.z])
		station_outline.color = "#FFDEDE"
		station_outline.alpha = 200
		var/image/station_areas = image(extraMiniMaps[HOLOMAP_EXTRA_STATIONMAPAREAS+"_[T.z]"])
		station_areas.alpha = 150
		holomap_datum.station_map.overlays += station_areas
		holomap_datum.station_map.overlays += station_outline
	if (!bogus)
		...
		for(var/obj/item/weapon/implant/tracking/implant in tracking_implants)
			...
			holomap_datum.station_map.overlays += I

	watching_mob.client.images += holomap_datum.station_map
	holomap_datum.station_map.loc = watching_mob.hud_used.holomap_obj


/obj/item/device/locator_holomap/proc/handle_marker(var/image/I,var/turf/TU)
