...
/obj/item/weapon/implanter/attack(mob/M as mob, mob/user as mob)
	...
	if (user && src.imp)
		for (var/mob/O in viewers(M, null))
			O.show_message("<span class=\"rose\">[user] is attemping to implant [M].</span>", 1)

		var/turf/T1 = get_turf(M)
		if (T1 && ((M == user) || do_after(user, 50)))
			if(user && M && (get_turf(M) == T1) && src && src.imp)
				for (var/mob/O in viewers(M, null))
					O.show_message("<span class=\"rose\">[M] has been implanted by [user].</span>", 1)
				...
				user.show_message("<span class=\"rose\">You implanted the implant into [M].</span>")
...
/obj/item/weapon/implanter/compressed/afterattack(var/obj/item/I, mob/user as mob)
	if(is_type_in_list(I,forbidden_types))
		user << "<span class=\"rose\">A red light flickers on the implanter.</span>"
		return
	if(istype(I) && imp)
		var/obj/item/weapon/implant/compressed/c = imp
		if (c.scanned)
			user << "<span class=\"rose\">Something is already scanned inside the implant!</span>"
			return
		if(user)
			user.u_equip(I)
