...
/obj/item/weapon/fireaxe/afterattack(atom/A as mob|obj|turf|area, mob/user as mob, proximity)
	...
	if(A && wielded && (istype(A,/obj/structure/window))) //destroys windows and grilles in one hit
		...
		if(istype(A,/obj/structure/window))
			...
			QDEL_NULL(A)
	else if(A && wielded && (istype(A, /turf/simulated/floor))) //removes floor plating with a delay and a chance to break
		...
		if(!do_after(user, T, breaktime, 10, custom_checks = new /callback(src, /obj/item/weapon/fireaxe/proc/on_do_after)))
			return
		...
		if(prob(breakchance))
			user.drop_item(src)
			to_chat(viewers(user), "<span class='danger'>\The [src] breaks into pieces!</span>")
