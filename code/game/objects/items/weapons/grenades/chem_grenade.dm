	...
	attackby(obj/item/weapon/W as obj, mob/user as mob)
		...
		if(istype(W,/obj/item/device/assembly_holder) && (!stage || stage==1) && path != 2)
			...
			if(istype(det.a_left,det.a_right.type) || (!isigniter(det.a_left) && !isigniter(det.a_right)))
				user << "<span class=\"rose\">Assembly must contain one igniter.</span>"
				return
			if(!det.secured)
				user << "<span class=\"rose\">Assembly must be secured with screwdriver.</span>"
				return
			path = 1
...
//					user << "<span class=\"rose\">You need to add at least one beaker before locking the assembly.</span>"
				...
				if(active && prob(95))
					user << "<span class=\"rose\">You trigger the assembly!</span>"
					prime()
					return
				else
			...
			path = 1
			if(beakers.len == 2)
				user << "<span class=\"rose\">The grenade can not hold more containers.</span>"
				return
			else
				if(W.reagents.total_volume)
					...
					user << "<span class=\"rose\">\the [W] is empty.</span>"
