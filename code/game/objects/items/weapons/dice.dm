		...
		if(istype(user,/mob/living/carbon/human)) //check that a humanoid is rolling the dice; Xenomorphs / Sillicons need not apply.
			...
			switch(result)
				...
				if(20)
					...
					user.dna.SetSEState(NOBREATHBLOCK,1)
					user.dna.SetSEState(FIREBLOCK,1)
					user.dna.SetSEState(REMOTEVIEWBLOCK,1)
					user.dna.SetSEState(REGENERATEBLOCK,1)
					user.dna.SetSEState(MORPHBLOCK,1)
					user.dna.SetSEState(NOPRINTSBLOCK,1)
					user.dna.SetSEState(SHOCKIMMUNITYBLOCK,1)
					user.dna.SetSEState(SOBERBLOCK,1)
					user.dna.SetSEState(PSYRESISTBLOCK,1)
					user.dna.SetSEState(STRONGBLOCK,1)
					user.dna.SetSEState(FARSIGHTBLOCK,1)
					user.dna.SetSEState(CHAMELEONBLOCK,1)
					user.dna.SetSEState(CRYOBLOCK,1)
					user.dna.SetSEState(EATBLOCK,1)
					user.dna.SetSEState(JUMPBLOCK,1)
					user.dna.SetSEState(MELTBLOCK,1)
					user.dna.SetSEState(EMPATHBLOCK,1)
					user.dna.SetSEState(SUPERFARTBLOCK,1)
					user.dna.SetSEState(IMMOLATEBLOCK,1) //This doesn't really let you do anything if you're heat-resistant
					user.dna.SetSEState(POLYMORPHBLOCK,1)
					genemutcheck(user,XRAYBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,TELEBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,INCREASERUNBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,SMALLSIZEBLOCK,null,MUTCHK_FORCED)
					...
					genemutcheck(user,NOBREATHBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,FIREBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,REMOTEVIEWBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,REGENERATEBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,MORPHBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,NOPRINTSBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,SHOCKIMMUNITYBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,SOBERBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,PSYRESISTBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,STRONGBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,FARSIGHTBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,CHAMELEONBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,CRYOBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,EATBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,JUMPBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,MELTBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,EMPATHBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,SUPERFARTBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,IMMOLATEBLOCK,null,MUTCHK_FORCED)
					genemutcheck(user,POLYMORPHBLOCK,null,MUTCHK_FORCED)
					user.update_mutations()
