...
/obj/item/soap/afterattack(atom/target, mob/user as mob)
...
/obj/item/bikehorn/afterattack(atom/target, mob/user as mob, proximity_flag)
...
/obj/item/bikehorn/kick_act(mob/living/H)
	if(..())
		return 1

	honk()

/obj/item/bikehorn/bite_act(mob/living/H)
	H.visible_message("<span class='danger'>[H] bites \the [src]!</span>", "<span class='danger'>You bite \the [src].</span>")

	honk()

/obj/item/bikehorn/proc/honk()
...
/obj/item/bikehorn/syndicate
	var/super_honk_delay = 50 //5 seconds
	var/last_super_honk_time

/obj/item/bikehorn/syndicate/attack_self(mob/user)
	add_fingerprint(user)
	super_honk(user)

/obj/item/bikehorn/syndicate/proc/super_honk(var/mob/user)
...
/obj/item/bikehorn/syndicate/examine(mob/user)
	..()
...
/obj/item/bikehorn/rubberducky/quantum
	desc = "A quantum quacker."
	var/teleport_range = 5
	var/last_teleport

/obj/item/bikehorn/rubberducky/quantum/New()
	..()
	processing_objects.Add(src)

/obj/item/bikehorn/rubberducky/quantum/Destroy()
	processing_objects.Remove(src)
	..()

/obj/item/bikehorn/rubberducky/quantum/process()
...
/obj/item/bikehorn/rubberducky/quantum/equipped(var/mob/user, var/slot, hand_index = 0)
...
/obj/item/glue/examine(mob/user)
	..()
	if(Adjacent(user))
...
/obj/item/glue/update_icon()
	..()
	icon_state = "glue[uses ? "1" : "0"]"

/obj/item/glue/afterattack(obj/target, mob/user, proximity_flag, click_parameters)
	if(!proximity_flag)
		return
...
/obj/item/glue/proc/apply_glue(obj/item/target)
	target.glue_act(glue_duration, glue_state_to_set)

/obj/item/glue/infinite/afterattack()
	.=..()
	uses = 1
	update_icon()
