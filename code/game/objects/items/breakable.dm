...
//not only a weapon but an item in general
//dont initialize empty values
//glances optional?
//glance text
//if the item has contents or components then also spill them?
...
/obj/item/attack_alien(mob/living/carbon/alien/humanoid/user)
	if(user.a_intent == I_HURT && breakable && breakable_flags & BREAKABLE_HIT_EMPTY)
		...
		if(take_damage(user.get_unarmed_damage()))
			user.visible_message("<span class='warning'>\The [user] [pick("slashes","claws")] \the [src]!</span>","<span class='notice'>You [pick("slash","claw")] \the [src]!</span>")
		else
			user.visible_message("<span class='warning'>\The [user] [pick("slash","claw")] \the [src], but it [pick("bounces","gleams","glances")] off!</span>","<span class='notice'>You hit \the [src], but it [pick("bounces","gleams","glances")] off!</span>")
	else
		..()
...
/obj/item/attack_animal(mob/living/simple_animal/M)
	if(M.melee_damage_upper && M.a_intent == I_HURT && breakable && breakable_flags & BREAKABLE_HIT_EMPTY)
		...
		if(take_damage(rand(M.melee_damage_lower,M.melee_damage_upper)))
			M.visible_message("<span class='warning'>\The [M] [M.attacktext] \the [src]!</span>","<span class='notice'>You hit \the [src]!</span>")
		else
			M.visible_message("<span class='warning'>\The [M] [M.attacktext] \the [src], but it [pick("bounces","gleams","glances")] off!</span>","<span class='notice'>You hit \the [src], but it [pick("bounces","gleams","glances")] off!</span>")
	else
		..()
...
/obj/item/attack_hand(mob/living/carbon/human/user)
	...
	if(user.a_intent == I_HURT && breakable && breakable_flags & BREAKABLE_HIT_EMPTY)
		...
		if(take_damage(user.get_unarmed_damage()))
			user.visible_message("<span class='warning'>\The [user] [user.species.attack_verb] \the [src]!</span>","<span class='notice'>You hit \the [src]!</span>")
		else
			user.visible_message("<span class='warning'>\The [user] [user.species.attack_verb] \the [src], but it [pick("bounces","gleams","glances")] off!</span>","<span class='notice'>You hit \the [src], but it [pick("bounces","gleams","glances")] off!</span>")
	else
		..()
...
/obj/item/attackby(obj/item/weapon/W, mob/user)
	...
	if(user.a_intent == I_HURT && breakable && breakable_flags & BREAKABLE_HIT_WEAPON)
		...
		user.delayNextAttack(1 SECONDS)
		add_fingerprint(user)
		if(take_damage(W.force))
			user.visible_message("<span class='warning'>\The [user] [pick(W.attack_verb)] \the [src] with \the [W]!</span>","<span class='notice'>You hit \the [src] with \the [W]!<span>")
		else
			user.visible_message("<span class='warning'>\The [user] [pick(W.attack_verb)] \the [src] with \the [W], but it [pick("bounces","gleams","glances")] off!</span>","<span class='notice'>You hit \the [src] with \the [W], but it [pick("bounces","gleams","glances")] off!<span>")
	else
		..()
