		...
		if ((M_HULK in user.mutations) || (prob(75 - metal*25)))
			...
			for(var/mob/O in oviewers(user))
				if ((O.client && !( O.blinded )))
					O << "<span class=\"rose\">[user] smashes through the foamed metal.</span>"
	...
	attackby(var/obj/item/I, var/mob/user)
		...
		if (istype(I, /obj/item/weapon/grab))
			...
			for(var/mob/O in viewers(src))
				if (O.client)
					O << "<span class=\"rose\">[G.assailant] smashes [G.affecting] through the foamed metal wall.</span>"
			del(I)
			del(src)
			return
	...
	start()
		if (amount <= 2)
			...
			for(var/mob/M in viewers(5, location))
				M << "<span class=\"rose\">The solution violently explodes.</span>"
			for(var/mob/M in viewers(1, location))
				if (prob (50 * amount))
					M << "<span class=\"rose\">The explosion knocks you down.</span>"
					M.Weaken(rand(1,5))
			return
		else
			var/devastation = -1
