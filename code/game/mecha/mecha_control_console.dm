...
/obj/machinery/computer/mecha/Topic(href, href_list)
	...
	if(href_list["shock"])
		switch(alert("Are you sure? This cannot be undone.","Transmit Beacon Overload Code","Yes","No"))
			if ("Yes")
				var/obj/item/mecha_parts/mecha_tracking/MT = topic_filter.getObj("shock")
				var/obj/mecha/M = MT.in_mecha()
				MT.shock()
				message_admins("[key_name_admin(usr)] overloaded \a [MT] in [M] via exosuit control console.")
				log_game("[key_name_admin(usr)] overloaded \a [MT] in [M] via exosuit control console.")
			if ("No")
				to_chat(usr, "You have second thoughts.")
