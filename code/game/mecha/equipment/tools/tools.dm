	...
	action(atom/target)
		...
		if(istype(target,/obj))
			...
			if(chassis.occupant.a_intent == "hurt")
				...
				M.updatehealth()
				occupant_message("<span class=\"rose\">You squeeze [target] with [src.name]. Something cracks.</span>")
				chassis.visible_message("<span class=\"rose\">[chassis] squeezes [target].</span>")
	...
	action(atom/target) //copypasted from extinguisher. TODO: Rewrite from scratch.
		...
		if(do_after_cooldown(target))
			if(istype(target, /obj/structure/reagent_dispensers/watertank) && get_dist(chassis,target) <= 1)
				...
				occupant_message("<span class=\"notify\">Extinguisher refilled.</span>")
...
/obj/item/mecha_parts/mecha_equipment/anticcw_armor_booster //what is that noise? A BAWWW from TK mutants.
	...
	proc/dynattackby(obj/item/weapon/W as obj, mob/user as mob)
		...
		if(prob(chassis.deflect_chance*deflect_coeff))
			user << "<span class=\"rose\">The [W] bounces off [chassis] armor.</span>"
...
/obj/item/mecha_parts/mecha_equipment/antiproj_armor_booster
	...
	proc/dynbulletdamage(var/obj/item/projectile/Proj)
		...
		if(prob(chassis.deflect_chance*deflect_coeff))
			chassis.occupant_message("<span class=\"notify\">The armor deflects incoming projectile.</span>")
	...
	proc/dynhitby(atom/movable/A)
		...
		if(prob(chassis.deflect_chance*deflect_coeff) || istype(A, /mob/living) || istype(A, /obj/item/mecha_parts/mecha_tracking))
			chassis.occupant_message("<span class=\"notify\">The [A] bounces off the armor.</span>")
...
/obj/item/mecha_parts/mecha_equipment/tool/safety_clamp
	...
	action(atom/target)
		...
		if(istype(target,/obj))
			...
			if(M.stat>1) return
			if(chassis.occupant.a_intent == "hurt")
				chassis.occupant_message("<span class=\"rose\">You obliterate [target] with [src.name], leaving blood and guts everywhere.</span>")
				chassis.visible_message("<span class=\"rose\">[chassis] destroys [target] in an unholy fury.</span>")
			if(chassis.occupant.a_intent == "disarm")
				chassis.occupant_message("<span class=\"rose\">You tear [target]'s limbs off with [src.name].</span>")
				chassis.visible_message("<span class=\"rose\">[chassis] rips [target]'s arms off.</span>")
			else
				step_away(M,chassis)
