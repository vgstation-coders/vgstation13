...
/datum/unit_test/dynamic/enemy_jobs/start()
	...
	for(var/i in 1 to 10)
		...
		for(var/datum/dynamic_ruleset/DR in rules2check)
			...
			if(result == tocheck)
				fail("[__FILE__]:[__LINE__]: enemy job test failed. expected [!tocheck], got [result] on rule [DR.name] with [enemies_count] out of [DR.required_enemies[i]] enemies[!midround ? " and [!midround ? dynamic_mode.roundstart_pop_ready : dynamic_mode.living_players.len] out of [DR.required_pop[i]] candidates" : ""]")
