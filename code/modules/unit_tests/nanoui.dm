/datum/unit_test/nanoui/start()
	...
	for(var/uifile in uis)
		...
		qdel(ui)
	ui = new(user, src, i, "nonexistantfiledonotmakethis.tmpl", "", 200, 200)
	if(!findtext(ui.get_html(),"<h2>Template error (does not exist)</h2>"))
		fail("[__FILE__]:[__LINE__]: nanoui test failed: template nonexistantfiledonotmakethis.tmpl exists.")
	qdel(ui)
	//ui = new(user, src, i, "nonexistantfiledonotmakethis.tmpl", "", 200, 200)
	//if(findtext(html,"<h2>Template error (failed to compile)</h2>"))
		//fail("[__FILE__]:[__LINE__]: nanoui test failed: template does not compile.")
	//qdel(ui)
	qdel(user)
