    ...
    for(var/list/items in lists)
        for(var/type in items)
            if(islist(items[type]))
                ...
                if(amount < abs(reagentlist[reagentlist[1]]))
                    fail("Reagent ID [reagentlist[1]] was not created to [amount] units from [items == juice_items ? "juic" : "grind"]ing [name] in [R].")
                ...
                var/amount = M.reagents.get_reagent_amount(reagentlist[1])
                if(amount < abs(reagentlist[reagentlist[1]]))
                    fail("Reagent ID [reagentlist[1]] was not created to [amount] units from [items == juice_items ? "juic" : "grind"]ing [name] in [M].")
