    ...
    for(var/itempath in subtypesof(/obj/item))
        ...
        if(O.blend_reagent || (O.grind_flags & GRIND_TRANSFER))
            ...
            if(reagent_check && !R.beaker.reagents.has_all_reagents(reagent_check))
                fail("[O.type] does not have the reagents [json_encode(reagent_check)] from being grinded in [R]. (got [json_encode(get_list_of_keys(R.beaker.reagents.amount_cache.Copy()))])")
            if(required)
        ...
        if(O.juice_reagent) //mortars prioritise this
            ...
            if(reagent_check && !M.reagents.has_all_reagents(reagent_check))
                fail("[O.type] does not have the reagents [json_encode(reagent_check)] from being grinded in [M]. (got [json_encode(get_list_of_keys(M.reagents.amount_cache.Copy()))])")
            if(required)
