		...
		if(RC.reagents)
			for(var/entry in RC.reagents_to_add)
				...
				if(!RC.reagents.has_reagent(entry, RC.reagents_to_add[entry]))
					fail("Reagent ID [entry] from reagents_to_add not found in at least [RC.reagents_to_add[entry]] units in atom [RC]]. (got [RC.get_reagent_amount(entry)] units instead)")
		qdel(RC)
