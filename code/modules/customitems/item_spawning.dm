...
//	testing("\[CustomItem\] Checking for custom items for [M.ckey] ([M.real_name])...")
	...
	to_chat(world, "Equip custom items, [M.disabilities]")
	//Gives glasses to the vision impaired
	if(M.disabilities & DISABILITY_FLAG_NEARSIGHTED)
		var/equipped = M.equip_to_slot_or_del(new /obj/item/clothing/glasses/regular(M), slot_glasses)
		if(equipped != 1)
...
//		H.update_icons()
	...
	if(!M.check_stand_ability())
		var/obj/structure/bed/chair/vehicle/wheelchair/W = new(M.loc)
		W.buckle_mob(M,M)

	if(M.disabilities & ASTHMA)
		if(M.backbag == 1)
			M.put_in_hand(GRASP_LEFT_HAND, new /obj/item/device/inhaler(M))
		else
			M.equip_or_collect(new /obj/item/device/inhaler(M), slot_in_backpack)

	if (M.client.IsByondMember())
		to_chat(M, "Thank you for supporting BYOND!")
		if(M.backbag == 1)
			M.put_in_hand(GRASP_RIGHT_HAND, new /obj/item/weapon/storage/box/byond(M))
		else
			M.equip_or_collect(new /obj/item/weapon/storage/box/byond(M), slot_in_backpack)
