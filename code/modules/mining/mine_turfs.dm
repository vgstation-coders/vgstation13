				...
				if(artifact_find)
					if(excavation_level > 0)
						...
						if(artifact_find)
							...
							B.investigation_log(I_ARTIFACT, "|| [B.artifact_find.artifact_find_type] || found by [user.name]([user.ckey]).")
					else
						artifact_debris(1)
...
/turf/unsimulated/mineral/proc/GetDrilled(var/artifact_fail = 0)
	...
	if(artifact_find && artifact_fail)
		investigation_log(I_ARTIFACT, "|| [artifact_find.artifact_find_type] || destroyed by [usr.name]([usr.ckey]).")
