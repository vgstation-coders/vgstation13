...
/turf/unsimulated/mineral/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/pickaxe))
		...
		if(do_after(user, src, max(P.toolspeed,minimum_mine_time)) && user)
			busy = 0
...
/turf/unsimulated/floor/asteroid/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/pickaxe))
		...
		if(do_after(user, src, used_digging.toolspeed) && user) //the better the drill, the faster the digging
