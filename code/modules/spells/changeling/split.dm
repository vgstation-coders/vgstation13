...
/spell/changeling/split/proc/recruiter_recruited(mob/dead/observer/player, mob/user)
	if(player)
		qdel(recruiter)
		recruiter = null
		polling_ghosts = FALSE
		
		var/turf/this_turf = get_turf(user)
		var/mob/living/carbon/human/newbody = new(this_turf)
		var/datum/role/changeling/changeling = user.mind.GetRole(CHANGELING)
		if(!changeling)
			return 0
		var/datum/dna/split_dna = changeling.GetDNA(user)
		if(!split_dna)
			return 0
