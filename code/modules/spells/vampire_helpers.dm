...
/mob/proc/vampire_power(var/required_blood = 0, var/max_stat = 0)
	...
	var/fullpower = (VAMP_JAUNT in vampire.current_powers)
...
/mob/proc/can_enthrall(var/mob/living/carbon/human/H)
	...
	if(restrained())
		...
		return 0
	if(!(VAMP_CHARISMA in V.current_powers)) //Charisma allows implanted targets to be enthralled.
