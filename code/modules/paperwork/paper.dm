...
/obj/item/paper/proc/show_text(var/mob/user, var/links = FALSE, var/starred = FALSE)
...
/obj/item/paper/attack_robot(var/mob/user as mob)
...
/obj/item/paper/proc/clearpaper()
	...
	updateinfolinks()
	update_icon()
	if(istype(loc, /obj/item/storage/bag/clipboard))
		var/obj/C = loc
		C.update_icon()

/obj/item/paper/proc/parsepencode(var/mob/user,var/obj/item/i, var/t)
...
/obj/item/paper/Topic(href, href_list)
	...
	if(href_list["write"])
		...
		spawn()
			...
			if(istype(loc, /obj/item/storage/bag/clipboard))
				var/obj/item/storage/bag/clipboard/C = loc
				C.update_icon()
...
/obj/item/paper/attackby(obj/item/P as obj, mob/user as mob)
	...
	else if(istype(P, /obj/item/photo) && !istype(src, /obj/item/paper/envelope))
		if(img)
...
var/global/list/paper_folding_results = list ( \
	"ball of paper" = /obj/item/p_folded/ball,
	"paper plane" = /obj/item/p_folded/plane,
	"paper hat" = /obj/item/p_folded/hat,
	"folded note" = /obj/item/p_folded/note_small,
	"origami crane" = /obj/item/p_folded/crane,
	"origami boat" = /obj/item/p_folded/boat,
	"origami heart" = /obj/item/p_folded/folded_heart,
	"envelope" = /obj/item/paper/envelope,
	)

/obj/item/paper/verb/fold()
	set category = "Object"
	set name = "Fold paper"
	set src in usr
	...
	var/obj/item/P
	if(ispath(foldtype, /obj/item/p_folded))
	...
	if (istype(src, /obj/item/paper/nano))
		P.color = "#9A9A9A"
		if(istype(P, /obj/item/p_folded))
			var/obj/item/p_folded/pf = P
			pf.nano = 1
...
/obj/item/paper/proc/canfold(mob/user)
	if(!user)
		return 0
...
/obj/item/paper/djstation/initialize()
	..()
...
/obj/item/paper/intercoms
	name = "paper - 'Ace Reporter Intercom manual'"

/obj/item/paper/intercoms/initialize()
	..()
...
/obj/item/paper/voxresearch/voxresearchclosure
...
/obj/item/paper/voxresearch/voxresearch1
...
/obj/item/paper/voxresearch/voxresearch2
	name = "paper- 'Two new contracts'"
...
/obj/item/paper/spectrometry_decommission
...
/obj/item/paper/anomaly
	name = "Anomaly Report"
	var/obj/machinery/artifact/artifact

/obj/item/paper/anomaly/Destroy()
	artifact = null
	..()

/obj/item/paper/merchantreport
	var/identity
	var/list/mugshots = list()

/obj/item/paper/merchantreport/New(loc,mob/living/carbon/human/merchant)
	if(merchant)
...
/obj/item/paper/merchantreport/show_text(var/mob/user, var/links = FALSE, var/starred = FALSE)
	var/index = 1
