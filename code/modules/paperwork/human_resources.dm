...
/obj/item/paper/commendation_key
...
/obj/item/demote_chip/attackby(obj/item/I as obj, mob/user as mob)
	if(istype(I, /obj/item/stamp))
		var/obj/item/stamp/S = I
		if(target_name != null)//Stamper must be able to see who he is banning
			if(istype(S, /obj/item/stamp/captain))
				if(cap == 0)
					desc = desc + "/Captain"
				cap = 1
				to_chat(user, "<span class='notice'>You stamp the demotion microchip of [target_name].</span>")
			if(istype(S, /obj/item/stamp/hop))
				if(hop == 0)
					desc = desc + "/HoP"
				hop = 1
				to_chat(user, "<span class='notice'>You stamp the demotion microchip of [target_name].</span>")
			if(istype(S, /obj/item/stamp/hos))
				if(hos == 0)
					desc = desc + "/HoS"
				hos = 1
				to_chat(user, "<span class='notice'>You stamp the demotion microchip of [target_name].</span>")
			if(istype(S, /obj/item/stamp/ce))
				if(ce == 0)
					desc = desc + "/CE"
				ce = 1
				to_chat(user, "<span class='notice'>You stamp the demotion microchip of [target_name].</span>")
			if(istype(S, /obj/item/stamp/rd))
				if(rd == 0)
					desc = desc + "/RD"
				rd = 1
				to_chat(user, "<span class='notice'>You stamp the demotion microchip of [target_name].</span>")
			if(istype(S, /obj/item/stamp/cmo))
				if(cmo == 0)
					desc = desc + "/CMO"
				cmo = 1
				to_chat(user, "<span class='notice'>You stamp the demotion microchip of [target_name].</span>")
...
/obj/item/card/id/syndicate/attackby(var/obj/item/I as obj, mob/user as mob)
...
/obj/item/card/id/attackby(var/obj/item/I as obj, mob/user as mob)
