...
/obj/item/weapon/implanter/proc/update_icon()
	overlays.len = 0

	if(held_implant)
		var/image/implant_graphic = image('icons/obj/items.dmi', src, "implanter_overlay")

		implant_graphic.icon += held_implant.color
...
/obj/item/weapon/implanter/proc/update()
	icon_state = "implanter[held_implant? 1:0]"
...
/obj/item/weapon/implanter/attack(mob/target as mob, mob/user as mob)
	...
	if(user && held_implant)
		for (var/mob/O in viewers(target, null))
			O.show_message("<span class='warning'>[user] is attempting to implant [target].</span>", 1)

		var/turf/T1 = get_turf(target)
		if(T1 && ((target == user) || do_after(user,target, 50)))
			if(user && target && (get_turf(target) == T1) && src && held_implant)
				for (var/mob/O in viewers(target, null))
					O.show_message("<span class='warning'>[target] has been implanted by [user].</span>", 1)
...
/obj/item/weapon/implanter/compressed/attack(mob/target as mob, mob/user as mob)
