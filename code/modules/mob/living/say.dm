...
/mob/living/proc/handle_inherent_channels(var/datum/speech/speech, var/message_mode)
	switch(message_mode)
		if(MODE_CHANGELING)
			if(lingcheck())
				var/turf/T = get_turf(src)
				log_say("[mind.changeling.changelingID]/[key_name(src)] (@[T.x],[T.y],[T.z]) Changeling Hivemind: [rhtml_encode(speech.message)]")
				var/themessage = text("<i><font color=#800080><b>[]:</b> []</font></i>",mind.changeling.changelingID,rhtml_encode(speech.message))
		...
		if(MODE_CULTCHAT)
			if(construct_chat_check(1)) /*sending check for humins*/
				...
				var/themessage = text("<span class='sinister'><b>[]:</b> []</span>",src.name,rhtml_encode(speech.message))
				for(var/mob/M in player_list)
		...
		if(MODE_ANCIENT)
			...
			if(stone.len)
				...
				if(commstone.commdevice)
					...
					var/themessage = text("<span class='ancient'>Ancient communication, <b>[]:</b> []</span>",src.name,rhtml_encode(speech.message))
					var/turf/T = get_turf(src)
					log_say("[key_name(src)] (@[T.x],[T.y],[T.z]) Ancient chat: [rhtml_encode(speech.message)]")
