				...
				if(distance <= 1 && !istype(src.wear_mask, /obj/item/clothing/mask))
					if(prob(25))
						for(var/mob/O in viewers(world.view,src))
							O.show_message("<span class=\"danger\">[src] has attempted to bite [src.target]!</span>", 1, "<span class=\"rose\">You hear struggling.</span>", 2)
					else
						for(var/mob/O in viewers(world.view,src))
							O.show_message("<span class=\"danger\">[src.target] has been bitten by [src]!</span>", 1, "<span class=\"rose\">You hear struggling.</span>", 2)
					...
					if(prob(25) && src.equipped())
						...
						for(var/mob/O in viewers(world.view,src))
							O.show_message("<span class=\"danger\">[src] has attempted to claw [src.target]!</span>", 1, "<span class=\"rose\">You hear struggling.</span>", 2)
					else
						for(var/mob/O in viewers(world.view,src))
							O.show_message("<span class=\"danger\">[src.target] has been clawed by [src]!</span>", 1, "<span class=\"rose\">You hear struggling.</span>", 2)
						var/mob/living/carbon/human/T = target
						T.bruteloss += rand(1,7)
						var/datum/organ/external/affecting
						if(T.organs["head"]) affecting = T.organs["head"]
...
/datum/disease/z_virus/stage_act()
	...
	switch(stage)
		if(1)
			if (prob(8))
				affected_mob << pick("<span class=\"rose\">Something about you doesnt feel right.</span>","<span class=\"rose\">Your head starts to itch.</span>")
		if(2)
			if (prob(8))
				affected_mob << "<span class=\"rose\">Your limbs feel numb.</span>"
				affected_mob.bruteloss += 1
				affected_mob.updatehealth()
			if (prob(9))
				affected_mob << "<span class=\"rose\">You feel ill...</span>"
			if (prob(9))
				affected_mob << "<span class=\"rose\">You feel a pain in your stomache...</span>"
		if(3)
			if (prob(8))
				affected_mob << text("<span class=\"rose\">[]", pick("owww...</span>","I want...</span>","Please...</span>"))
				affected_mob.bruteloss += 1
				affected_mob.updatehealth()
			if (prob(10))
				affected_mob << "<span class=\"rose\">You feel very ill.</span>"
				affected_mob.bruteloss += 5
				affected_mob.updatehealth()
			if (prob(4))
				affected_mob << "<span class=\"rose\">You feel a stabbing pain in your head.</span>"
				affected_mob.paralysis += 2
			if (prob(4))
				affected_mob << "<span class=\"rose\">Whats going to happen to me?</span>"
		if(4)
			if (prob(10))
				affected_mob << pick("<span class=\"rose\">You feel violently sick.</span>")
				affected_mob.bruteloss += 8
				affected_mob.updatehealth()
			if (prob(20))
				affected_mob.say(pick("Mmmmm.", "Hey... You look...", "Hsssshhhhh!"))
			if (prob(8))
				affected_mob << "<span class=\"rose\">You cant... feel...</span>"
		if(5)
...
//		user << "<span class=\"rose\">You stab [M] with the pen.</span>"
...
//		if(src)
//			src << "<span class=\"rose\">Wrong game mode!</span>"
//			return
