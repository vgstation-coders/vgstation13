...
/mob/living/carbon/human/handle_regular_hud_updates()
	...
	if(stat == DEAD)
		...
		if (!V || (!(VAMP_VISION in V.current_powers) && !(VAMP_MATURE in V.current_powers))) // Not a vampire, or a vampire but neither of the spells.
			change_sight(removing = SEE_MOBS)
		if (!V || !(VAMP_MATURE in V.current_powers))
