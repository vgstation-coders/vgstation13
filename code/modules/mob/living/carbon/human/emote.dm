...
/datum/emote/living/carbon/human/dab/run_emote(mob/user, params, type_override)
	var/mob/living/carbon/human/H = user
	if(H.has_organ(LIMB_LEFT_ARM) && H.has_organ(LIMB_RIGHT_ARM)) //You can't dab without arms.
		if(world.time-H.lastDab >= 100) //15 seconds.
			for(var/mob/living/M in view(0))
				if(M != H && M.loc == H.loc)
			...
			var/armtobreak = pick(LIMB_LEFT_ARM, LIMB_RIGHT_ARM) //50 50 chance to break an arm if you dab too often.
