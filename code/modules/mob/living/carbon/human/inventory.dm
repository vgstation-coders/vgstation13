/mob/living/carbon/human/verb/quick_equip()
	...
	if(ishuman(src))
		...
		if(H.equip_to_appropriate_slot(I))
			...
			H << "<span class=\"rose\">You are unable to equip that.</span>"
...
/mob/living/carbon/human/equip_to_slot(obj/item/W as obj, slot, redraw_mob = 1)
	...
	switch(slot)
		...
		if(slot_in_backpack)
			...
			src << "<span class=\"rose\">You are trying to equip this item to an unsupported inventory slot. Report this to a coder!</span>"
			return
		...
		switch(place)
			...
			if("mask")
				...
				if(target.wear_mask && !target.wear_mask.canremove)
					...
					message = "<span class=\"rose\">[source] is trying to take off \a [target.wear_mask] from [target]'s head!</span>"
			if("l_hand")
		...
		if("CPR")
			if ((target.health > config.health_threshold_dead && target.health < config.health_threshold_crit))
				...
				for(var/mob/O in viewers(source, null))
					O.show_message("<span class=\"rose\">[source] performs CPR on [target]!</span>", 1)
				target << "<span class=\"notice\"><b>You feel a breath of fresh air enter your lungs. It feels good.</b></span>"
				source << "<span class=\"rose\">Repeat at least every 7 seconds.</span>"
		if("dnainjector")
			...
			if(S)
				...
				S.s_time = world.time
				for(var/mob/O in viewers(source, null))
					O.show_message("<span class=\"rose\">[source] injects [target] with the DNA Injector!</span>", 1)
				S.inuse = 0
		if("pockets")
