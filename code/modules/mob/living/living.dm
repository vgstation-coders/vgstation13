			...
			spawn(0)
				if(C)
					...
					if(C)
						...
						sleep(7)
						if(C)
							animate(C, pixel_x = C.pixel_x - pixel_x_diff * PIXEL_MULTIPLIER, pixel_y = C.pixel_y - pixel_y_diff * PIXEL_MULTIPLIER, time=6, easing=SINE_EASING, flags=ANIMATION_PARALLEL|ANIMATION_RELATIVE)
			src = oldsrc
		if(!wasdizzy)
			dizziness = 0
