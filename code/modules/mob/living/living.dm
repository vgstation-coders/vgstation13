...
/mob/living/proc/adjustToxLoss(var/amount)
	...
	if(has_reagent(PRO_TOXIN))
		mult *= -1 //invert the multiplier, make only toxin damage heal on pro-toxin
		drowsyness = max(drowsyness - 4 * REM, 0)
		if(has_any_reagents(list(TOXIN, PLANTBGONE, INSECTICIDE, SOLANINE)))
			remove_reagents(list(TOXIN, PLANTBGONE, INSECTICIDE, SOLANINE), 4 * REM)
		if(has_any_reagents(STOXINS))
			remove_reagents(STOXINS, 4 * REM)
		if(has_reagent(PLASMA))
			remove_reagent(PLASMA, 2 * REM)
		if(has_any_reagents(SACIDS))
			remove_reagents(SACIDS, 2 * REM)
		if(has_reagent(POTASSIUM_HYDROXIDE))
			remove_reagent(POTASSIUM_HYDROXIDE, 4 * REM)
		if(has_reagent(CYANIDE))
			remove_reagent(CYANIDE, REM)
		if(has_reagent(AMATOXIN))
			remove_reagent(AMATOXIN, 4 * REM)
		if(has_reagent(CHLORALHYDRATE))
			remove_reagent(CHLORALHYDRATE, 4 * REM)
		if(has_reagent(CARPOTOXIN))
			remove_reagent(CARPOTOXIN, 2 * REM)
		if(has_reagent(ZOMBIEPOWDER))
			remove_reagent(ZOMBIEPOWDER, REM)
		if(has_reagent(MINDBREAKER))
			remove_reagent(MINDBREAKER, 4 * REM)
