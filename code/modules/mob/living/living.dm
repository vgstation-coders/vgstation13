...
/mob/living/proc/adjustToxLoss(var/amount)
	...
	if(reagents.has_reagent(PRO_TOXIN))
		...
		if (amount * tox_damage_modifier * mult < 0) //make sure we're removing
			...
			if(reagents.has_any_reagents(STOXINS))
				reagents.remove_reagents(STOXINS, 4 * REM)
			if(reagents.has_reagent(PLASMA))
				reagents.remove_reagent(PLASMA, 2 * REM)
			if(reagents.has_any_reagents(SACIDS))
				reagents.remove_reagents(SACIDS, 2 * REM)
			if(reagents.has_reagent(POTASSIUM_HYDROXIDE))
				reagents.remove_reagent(POTASSIUM_HYDROXIDE, 4 * REM)
			if(reagents.has_reagent(CYANIDE))
				reagents.remove_reagent(CYANIDE, 2 * REM)
			if(reagents.has_reagent(AMATOXIN))
				reagents.remove_reagent(AMATOXIN, 4 * REM)
			if(reagents.has_reagent(CHLORALHYDRATE))
				reagents.remove_reagent(CHLORALHYDRATE, 10 * REM)
			if(reagents.has_reagent(CARPOTOXIN))
				reagents.remove_reagent(CARPOTOXIN, 2 * REM)
			if(reagents.has_reagent(ZOMBIEPOWDER))
				reagents.remove_reagent(ZOMBIEPOWDER, REM)
			if(reagents.has_reagent(MINDBREAKER))
				reagents.remove_reagent(MINDBREAKER, 4 * REM)
			var/lucidmod = sleeping ? 3 : lying + 1 //3x as effective if they're sleeping, 2x if they're lying down
			hallucination = max(0, hallucination - 5 * REM * lucidmod)
