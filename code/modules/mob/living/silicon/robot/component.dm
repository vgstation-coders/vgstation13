...
/obj/item/device/robotanalyzer/attack(mob/living/M as mob, mob/living/user as mob)
	if(( (M_CLUMSY in user.mutations) || user.getBrainLoss() >= 60) && prob(50))
		user << text("<span class=\"rose\">You try to analyze the floor's vitals!</span>")
		for(var/mob/O in viewers(M, null))
			O.show_message(text("<span class=\"rose\">[user] has analyzed the floor's vitals!</span>"), 1)
	...
	if(!istype(M, /mob/living/silicon/robot))
		user << "<span class=\"rose\">You can't analyze non-robotic things!</span>"
		return
	...
	if(H.emagged && prob(5))
		user.show_message("<span class=\"rose\">\t ERROR: INTERNAL SYSTEMS COMPROMISED</span>",1)
