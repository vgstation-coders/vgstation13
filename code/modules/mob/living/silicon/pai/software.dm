		...
		if("doorjack")
			...
			if(href_list["cable"])
				...
				for (var/mob/M in viewers(T))
					M.show_message("<span class=\"rose\">A port on [src] opens to reveal [src.cable], which promptly falls to the floor.</span>", 3, "<span class=\"rose\">You hear the soft click of something light and hard falling to the ground.</span>", 2)
