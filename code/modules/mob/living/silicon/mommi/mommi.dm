...
/mob/living/silicon/robot/mommi/choose_icon()
	var/icontype = input("Select an icon!", "Mobile MMI", null) in list("Basic", "Hover", "Keeper", "RepairBot", "Replicator", "Gekko")
	switch(icontype)
		if("Replicator") subtype = "replicator"
		if("Keeper")	 subtype = "keeper"
		if("RepairBot")	 subtype = "repairbot"
		if("Hover")	     subtype = "hovermommi"
		if("Gekko")	     subtype = "dwarfgekko"
		else			 subtype = "mommi"
	updateicon()
...
/mob/living/silicon/robot/mommi/pick_module()
	...
	switch(modtype)
		if("MoMMI")
			...
			module_sprites["Gekko"] = "dwarfgekko"
