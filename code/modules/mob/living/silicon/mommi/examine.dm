...
/mob/living/silicon/robot/mommi/sammi/examine(mob/user)

	var/msg = "<span class='info'>*---------*\nThis is [bicon(src)] \a <EM>[src]</EM>!\n"

	msg += {"<p>This versatile assembler is controlled by a transient mind-cluster network, unfortunately it is prone to connection loss.</p><span class='warning'>"}
	if (src.getBruteLoss())
		if (src.getBruteLoss() < 75)
			msg += "It looks slightly dented.\n"
		else
			msg += "<B>It looks severely severely damaged!</B>\n"
	if (src.getFireLoss())
	...
	if(tool_state)
		var/obj/item/I = tool_state
		msg += "Its claw is gripping [bicon(I)] [I.gender==PLURAL?"some":"a"] [I.name].\n"
