...
/mob/living/simple_animal/corgi/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(inventory_head && inventory_back)
		...
		if( istype(inventory_head,/obj/item/clothing/head/helmet) && istype(inventory_back,/obj/item/clothing/suit/armor) )
			if( O.force )
				usr << "<span class=\"rose\">[src] is wearing too much armor. You can't cause \him any damage.</span>"
				for (var/mob/M in viewers(src, null))
					M.show_message("<span class=\"danger\">[user] hits [src] with [O], however [src] is too armored.</span>")
			else
				usr << "<span class=\"rose\">[src] is wearing too much armor. You can't reach \his skin.</span>"
				for (var/mob/M in viewers(src, null))
...
/mob/living/simple_animal/corgi/Topic(href, href_list)
		...
		switch(remove_from)
			if("head")
				if(inventory_head)
					...
					usr << "<span class=\"rose\">There is nothing to remove from its [remove_from].</span>"
					return
			if("back")
				if(inventory_back)
	...
	//Adding things to inventory
		...
		if(!usr.get_active_hand())
			usr << "<span class=\"rose\">You have nothing in your hand to put on its [add_to].</span>"
			return
		switch(add_to)
			if("head")
			...
			if("back")
				if(inventory_back)
					usr << "<span class=\"rose\">It's already wearing something.</span>"
					return
				else
...
/mob/living/simple_animal/corgi/proc/place_on_head(obj/item/item_to_add)
	...
	if(inventory_head)
		if(usr)	usr << "<span class=\"rose\">You can't put more than one hat on [src]!</span>"
		return
	if(!item_to_add)
...
/mob/living/simple_animal/corgi/puppy/Topic(href, href_list)
	if(href_list["remove_inv"] || href_list["add_inv"])
		usr << "<span class=\"rose\">You can't fit this on [src]</span>"
		return
	..()
...
/mob/living/simple_animal/corgi/Lisa/Topic(href, href_list)
	if(href_list["remove_inv"] || href_list["add_inv"])
		usr << "<span class=\"rose\">[src] already has a cute bow!</span>"
		return
	..()
