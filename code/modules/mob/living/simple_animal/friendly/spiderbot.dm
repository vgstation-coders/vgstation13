...
/mob/living/simple_animal/spiderbot/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(istype(O, /obj/item/device/mmi) || istype(O, /obj/item/device/mmi/posibrain))
		...
		if(src.mmi) //There's already a brain in it.
			user << "<span class=\"rose\">There's already a brain in [src]!</span>"
			return
		if(!B.brainmob)
			user << "<span class=\"rose\">Sticking an empty MMI into the frame would sort of defeat the purpose.</span>"
			return
		if(!B.brainmob.key)
		...
		if(B.brainmob.stat == DEAD)
			user << "<span class=\"rose\">[O] is dead. Sticking it into the frame would sort of defeat the purpose.</span>"
			return

		if(jobban_isbanned(B.brainmob, "Cyborg"))
			user << "<span class=\"rose\">[O] does not seem to fit.</span>"
			return
	...
	if (istype(O, /obj/item/weapon/weldingtool))
		...
		if (WT.remove_fuel(0))
			if(health < maxHealth)
				...
				for(var/mob/W in viewers(user, null))
					W.show_message(text("<span class=\"rose\">[user] has spot-welded some of the damage to [src]!</span>"), 1)
			else
		...
		if (!mmi)
			user << "<span class=\"rose\">There's no reason to swipe your ID - the spiderbot has no brain to remove.</span>"
			return 0
		...
		if(access_robotics in id_card.access)
			...
			user << "<span class=\"rose\">You swipe your card, with no effect.</span>"
			return 0
	else if (istype(O, /obj/item/weapon/card/emag))
		if (emagged)
			user << "<span class=\"rose\">[src] is already overloaded - better run.</span>"
			return 0
		else
			emagged = 1
			...
			spawn(100)	src << "<span class=\"rose\">Your cell seems to be outputting a lot of power...</span>"
			spawn(200)	src << "<span class=\"rose\">Internal heat sensors are spiking! Something is badly wrong with your cell!</span>"
		...
		if(O.force)
			...
			for(var/mob/M in viewers(src, null))
				if ((M.client && !( M.blinded )))
					M.show_message("<span class=\"rose\">[user] gently taps [src] with the [O].</span>")
...
/mob/living/simple_animal/spiderbot/proc/explode() //When emagged.
	for(var/mob/M in viewers(src, null))
		if ((M.client && !( M.blinded )))
			M.show_message("<span class=\"rose\">[src] makes an odd warbling noise, fizzles, and explodes.</span>")
...
/mob/living/simple_animal/spiderbot/verb/drop_held_item()
	...
	if(!held_item)
		usr << "<span class=\"rose\">You have nothing to drop!</span>"
		return 0

	if(istype(held_item, /obj/item/weapon/grenade))
		visible_message("<span class=\"rose\">[src] launches \the [held_item]!</span>", "<span class=\"rose\">You launch \the [held_item]!</span>", "You hear a skittering noise and a thump!")
...
/mob/living/simple_animal/spiderbot/verb/get_item()
	...
	if(held_item)
		src << "<span class=\"rose\">You are already holding \the [held_item]</span>"
		return 1
	...
	if(selection)
		for(var/obj/item/I in view(1, src))
			if(selection == I)
				...
				return held_item
		src << "<span class=\"rose\">\The [selection] is too far away.</span>"
		return 0

	src << "<span class=\"rose\">There is nothing of interest to take.</span>"
	return 0
