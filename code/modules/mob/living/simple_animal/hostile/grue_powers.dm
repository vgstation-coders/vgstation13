/datum/grue_upgrade
    var/ability_name = "Grue upgrade"
    var/ability_desc = "An upgrade for a grue's abilities"
    var/mob/living/simple_animal/hostile/grue/host
    var/condition = TRUE // So we know if we can display this in the menu
    var/upgrade_cost = 0

/datum/grue_upgrade/New(mob/living/simple_animal/hostile/grue/thisgrue)
    host = thisgrue
    update_condition_and_cost()

// Called on purchase and setup for handling in the menu
/datum/grue_upgrade/proc/update_condition_and_cost()
    if(!host)
        message_admins("Somebody set up a grue upgrade ([ability_name]) without assigning the host!")
        return

/datum/grue_upgrade/proc/on_purchase()
    if(!host)
        message_admins("Somebody set up a grue upgrade ([ability_name]) without assigning the host!")
        return FALSE
