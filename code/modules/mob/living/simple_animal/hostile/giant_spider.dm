	...
	if(isliving(target))
		...
		if(L.reagents)
			if(prob(poison_per_bite))
				src.visible_message("<span class=\"rose\">\the [src] injects a powerful toxin!</span>")
			...
			if(busy)
				if(busy == MOVING_TO_TARGET && target)
					...
					if(D.density==1)
						...
						stop_automated_movement = 1
						walk(src,0)
						D.visible_message("<span class=\"rose\">\the [D]'s motors whine as four arachnid claws begin trying to force it open!</span>")
						spawn(50)
							if(prob(25))
								D.open(1)
								D.visible_message("<span class=\"rose\">\the [src] forces \the [D] open!</span>")
					busy = 0
					stop_automated_movement = 0
						...
						if(busy == SPINNING_COCOON)
							if(cocoon_target && istype(cocoon_target.loc, /turf) && get_dist(src,cocoon_target) <= 1)
								if(istype(cocoon_target,/obj/machinery/door))
									...
									if(!W)
										src.visible_message("<span class=\"rose\">\the [src] jams \the [cocoon_target] open with web!</span>")
									...
									for(var/mob/living/M in C.loc)
										...
										large_cocoon = 1
										fed++
										src.visible_message("<span class=\"rose\">\the [src] sticks a proboscis into \the [cocoon_target] and sucks a viscous substance out.</span>")
										M.loc = C
										C.pixel_x = M.pixel_x
										C.pixel_y = M.pixel_y
										break
