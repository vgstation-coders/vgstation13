...
/mob/living/simple_animal/hostile/spacehog/Life()
	..()
	nutrition--
	if(stance == HOSTILE_STANCE_IDLE && (!food_target || get_dist(food_target, src) > 7))
		if(mood == HOG_CURIOUS)
...
/mob/living/simple_animal/hostile/spacehog/adult/UnarmedAttack(atom/target, prox)
	...
	if(!client) //automated headbutting if no client
...
/spell/headbutt/choose_targets(var/mob/user = usr)
	return list(user)
...
/mob/living/simple_animal/hostile/spacehog/adult/greased
	...
	icon_state = "hog_tusks"
	icon_living = "hog_tusks"
