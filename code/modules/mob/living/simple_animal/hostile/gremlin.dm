...
/mob/living/simple_animal/hostile/gremlin
	...
	//List of objects that we couldn't tamper with (because nobody coded an interaction for it)
	//This list starts out empty. Whenever the gremlin finds a machine that it couldn't tamper with, the machine's type is added here, and all machines of such type are ignored from then on (NOT SUBTYPES)
...
/mob/living/simple_animal/hostile/gremlin/CanAttack(atom/new_target)
	if(blacklist_types.Find(new_target.type))
		return FALSE
