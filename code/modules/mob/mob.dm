...
/mob/proc/simple_message(var/msg, var/hallucination_msg)//
...
/mob/proc/show_message(msg, type, alt, alt_type)//Message, type of message (1=visible or 2=hearable), alternative message, alt message type (1=if blind or 2=if deaf)
	...
	if(type)
		if((type & 1) && is_blind()) //Vision related //We can't see all those emotes no-one ever does !
			if(!(alt))
				return
			else
				msg = alt
				type = alt_type
		if((type & 2) && is_deaf()) //Hearing related //We can't hear what the person is saying. Too bad
			if(!(alt))
