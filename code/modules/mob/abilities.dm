...
/client/proc/test_ability()
	...
	if(!mob || !istype(mob,/mob/living) || mob.stat)
		src << "<span class=\"rose\">You must be corporeal and alive to do that.</span>"
		return 0

	//Handcuff check.
	if(mob.restrained())
		src << "<span class=\"rose\">You cannot do this while restrained.</span>"
		return 0

	if(istype(mob,/mob/living/carbon))
		var/mob/living/carbon/M = mob
		if(M.handcuffed)
			src << "<span class=\"rose\">You cannot do this while cuffed.</span>"
			return 0
