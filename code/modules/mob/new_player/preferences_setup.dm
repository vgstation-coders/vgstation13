			...
			if(job_civilian_high)//I hate how this looks, but there's no reason to go through this switch if it's empty
				switch(job_civilian_high)
					...
					if(CARGOTECH)
						...
						clothes_s=blend_backpack(clothes_s,backbag,"satchel-norm")
					if(MAILMAN)
						clothes_s = new /icon(uniform_dmi, "mailman_s")
						clothes_s.Blend(new /icon(feet_dmi, "mail"), ICON_UNDERLAY)
						clothes_s.Blend(new /icon('icons/mob/head.dmi', "mailman"), ICON_UNDERLAY)
						clothes_s=blend_backpack(clothes_s,backbag,"satchel-mail")
					if(MINER)
