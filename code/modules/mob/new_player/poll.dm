...
/mob/new_player/proc/handle_player_polling()
	...
	if(SSdatabase.IsConnected())
		...
		var/datum/DBQuery/select_query = SSdatabase.NewQuery("SELECT id, question FROM erro_poll_question WHERE [(isadmin ? "" : "adminonly = false AND")] Now() BETWEEN starttime AND endtime")
...
/mob/new_player/proc/poll_player(var/pollid = -1)
	...
	if(SSdatabase.IsConnected())
		...
		var/datum/DBQuery/select_query = SSdatabase.NewQuery("SELECT starttime, endtime, question, polltype, multiplechoiceoptions FROM erro_poll_question WHERE id = [pollid]")
		select_query.Execute()
		...
		switch(polltype)
			...
			if("OPTION")
				var/datum/DBQuery/voted_query = SSdatabase.NewQuery("SELECT optionid FROM erro_poll_vote WHERE pollid = [pollid] AND ckey = '[usr.ckey]'")
				voted_query.Execute()
				...
				var/datum/DBQuery/options_query = SSdatabase.NewQuery("SELECT id, text FROM erro_poll_option WHERE pollid = [pollid]")
			...
			if("TEXT")
				var/datum/DBQuery/voted_query = SSdatabase.NewQuery("SELECT replytext FROM erro_poll_textreply WHERE pollid = [pollid] AND ckey = '[usr.ckey]'")
				voted_query.Execute()
			...
			//Polls with a text input
			if("NUMVAL")
				var/datum/DBQuery/voted_query = SSdatabase.NewQuery("SELECT o.text, v.rating FROM erro_poll_option o, erro_poll_vote v WHERE o.pollid = [pollid] AND v.ckey = '[usr.ckey]' AND o.id = v.optionid")
				voted_query.Execute()
				...
				if(!voted)	//Only make this a form if we have not voted yet
					...
					var/datum/DBQuery/option_query = SSdatabase.NewQuery("SELECT id, text, minval, maxval, descmin, descmid, descmax FROM erro_poll_option WHERE pollid = [pollid]")
...
/mob/new_player/proc/vote_on_poll(var/pollid = -1, var/optionid = -1, var/multichoice = 0)
	...
	if(SSdatabase.IsConnected())
		...
		var/datum/DBQuery/select_query = SSdatabase.NewQuery("SELECT starttime, endtime, question, polltype, multiplechoiceoptions FROM erro_poll_question WHERE id = [pollid] AND Now() BETWEEN starttime AND endtime")
		select_query.Execute()
		...
		var/datum/DBQuery/select_query2 = SSdatabase.NewQuery("SELECT id FROM erro_poll_option WHERE id = [optionid] AND pollid = [pollid]")
		...
		var/datum/DBQuery/voted_query = SSdatabase.NewQuery("SELECT id FROM erro_poll_vote WHERE pollid = [pollid] AND ckey = '[usr.ckey]'")
		voted_query.Execute()
		...
		var/datum/DBQuery/insert_query = SSdatabase.NewQuery("INSERT INTO erro_poll_vote (id ,datetime ,pollid ,optionid ,ckey ,ip ,adminrank) VALUES (null, Now(), [pollid], [optionid], '[usr.ckey]', '[usr.client.address]', '[adminrank]')")
...
/mob/new_player/proc/log_text_poll_reply(var/pollid = -1, var/replytext = "")
	...
	if(SSdatabase.IsConnected())
		...
		var/datum/DBQuery/select_query = SSdatabase.NewQuery("SELECT starttime, endtime, question, polltype FROM erro_poll_question WHERE id = [pollid] AND Now() BETWEEN starttime AND endtime")
		select_query.Execute()
		...
		var/datum/DBQuery/voted_query = SSdatabase.NewQuery("SELECT id FROM erro_poll_textreply WHERE pollid = [pollid] AND ckey = '[usr.ckey]'")
		voted_query.Execute()
		...
		var/datum/DBQuery/insert_query = SSdatabase.NewQuery("INSERT INTO erro_poll_textreply (id ,datetime ,pollid ,ckey ,ip ,replytext ,adminrank) VALUES (null, Now(), [pollid], '[usr.ckey]', '[usr.client.address]', '[replytext]', '[adminrank]')")
		insert_query.Execute()
...
/mob/new_player/proc/vote_on_numval_poll(var/pollid = -1, var/optionid = -1, var/rating = null)
	...
	if(SSdatabase.IsConnected())
		...
		var/datum/DBQuery/voted_query = SSdatabase.NewQuery("SELECT id FROM erro_poll_vote WHERE optionid = [optionid] AND ckey = '[usr.ckey]'")
		voted_query.Execute()
		...
		var/datum/DBQuery/insert_query = SSdatabase.NewQuery("INSERT INTO erro_poll_vote (id ,datetime ,pollid ,optionid ,ckey ,ip ,adminrank, rating) VALUES (null, Now(), [pollid], [optionid], '[usr.ckey]', '[usr.client.address]', '[adminrank]', [(isnull(rating)) ? "null" : rating])")
		insert_query.Execute()
