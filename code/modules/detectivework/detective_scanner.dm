...
/obj/item/device/vampirehead
	...
	desc = "The head of an immortal lord of the night. It won't shut up until you cremate it or feed it blood."
...
/obj/item/device/vampirehead/update_icon()
	icon_state = "vampire_head[smells_blood]"

/obj/item/device/vampirehead/afterattack(atom/A as obj|turf|mob, mob/user as mob)
	if(!in_range(A,user))
		return
	if(loc != user)
		return
	...
	//General
	if(blood_DNA_found.len)
		var/blood = pick(blood_DNA_found)
		commment(blood)

/obj/item/device/vampirehead/proc/extract_blood(atom/A)
	var/list/extracted_blood=list()
	if(A.blood_DNA)
		for(var/blood in A.blood_DNA)
			extracted_blood[blood]=A.blood_DNA[blood]
...
/obj/item/device/vampirehead/proc/comment(var/blood)
	//Let's find out several things about the blood and then chat about one of them.
	//Eye color, hair color, skin tone, assigned job, species
	to_chat(user,"<B>[src]</B> [pick("murmurs","whispers","sighs","hints","gossips")], \"[clue]...\"")