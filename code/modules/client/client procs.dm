...
/client/proc/handle_spam_prevention(var/message, var/mute_type)
	if(config.automute_on && !holder && src.last_message == message)
		...
		if(src.last_message_count >= SPAM_TRIGGER_AUTOMUTE)
			src << "<span class=\"rose\">You have exceeded the spam filter limit for identical messages. An auto-mute was applied.</span>"
			cmd_admin_mute(src.mob, mute_type, 1)
			return 1
		if(src.last_message_count >= SPAM_TRIGGER_WARNING)
			src << "<span class=\"rose\">You are nearing the spam filter limit for identical messages.</span>"
			return 0
	else
...
/client/New(TopicData)
	...
	src << "<span class=\"rose\">If the title screen is black, resources are still downloading. Please be patient until the title screen appears.</span>"
