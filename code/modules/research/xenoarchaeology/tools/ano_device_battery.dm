...
/obj/item/weapon/anodevice/Topic(href, href_list)
	...
	if(href_list["startup"])
		...
		timing = 0
		if(!inserted_battery.battery_effect.activated)
			src.investigation_log(I_ARTIFACT, "|| anomaly battery [inserted_battery.battery_effect.artifact_id] effect ([inserted_battery.battery_effect]) emission started by [key_name(usr)]")
			inserted_battery.battery_effect.ToggleActivate(1)
	...
	if(href_list["starttimer"])
		timing = 1
		archived_time = time
		src.investigation_log(I_ARTIFACT, "|| anomaly battery [inserted_battery.battery_effect.artifact_id] effect ([inserted_battery.battery_effect]) emission timed by [key_name(usr)]")
