...
/obj/item/weapon/anodevice/attackby(var/obj/I as obj, var/mob/user as mob)
	if(istype(I, /obj/item/weapon/anobattery))
		if(!inserted_battery)
			user << "<span class=\"notify\">You insert the battery.</span>"
			user.drop_item()
			I.loc = src
			inserted_battery = I
			UpdateSprite()
...
/obj/item/weapon/anodevice/process()
	if(cooldown > 0)
		...
		if(cooldown <= 0)
			cooldown = 0
			src.visible_message("<span class=\"notify\">\icon[src] [src] chimes.</span>", "<span class=\"notify\">\icon[src] You hear something chime.</span>")
	else if(activated)
...
/obj/item/weapon/anodevice/proc/shutdown_emission()
	if(activated)
		activated = 0
		timing = 0
		src.visible_message("<span class=\"notify\">\icon[src] [src] buzzes.</span>", "\icon[src]<span class=\"notify\">You hear something buzz.</span>")
