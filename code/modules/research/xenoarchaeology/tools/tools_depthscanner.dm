...
/obj/item/device/depth_scanner/proc/scan_atom(var/mob/user, var/atom/A)
	user.visible_message("<span class=\"notify\">[user] scans [A], the air around them humming gently.</span>")
	if(istype(A,/turf/unsimulated/mineral))
		...
		if(M.finds.len || M.artifact_find)
			...
			*/
			...
			for(var/mob/L in range(src, 1))
				L << "<span class=\"notify\">\icon[src] [src] pings.</span>"
		...
		if(B.artifact_find)
			...
			for(var/mob/L in range(src, 1))
				L << "<span class=\"notify\">\icon[src] [src] pings [pick("madly","wildly","excitedly","crazily")]!.</span>"
