...
/obj/item/device/core_sampler/examine()
	...
	if(get_dist(src, usr) < 2)
		usr << "That's \a [src]."
		usr << "<span class=\"notice\">Used to extract geological core samples - this one is [sampled_turf ? "full" : "empty"], and has [num_stored_bags] bag[num_stored_bags != 1 ? "s" : ""] remaining.</span>"
	else
		return ..()
