...
/obj/item/device/core_sampler/examine()
	...
	if(get_dist(src, usr) < 2)
		usr << "That's \a [src]."
		usr << "<span class=\"notify\">Used to extract geological core samples - this one is [sampled_turf ? "full" : "empty"], and has [num_stored_bags] bag[num_stored_bags != 1 ? "s" : ""] remaining.</span>"
	else
		return ..()
...
/obj/item/device/core_sampler/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W,/obj/item/weapon/evidencebag))
		if(num_stored_bags < 10)
			del(W)
			num_stored_bags += 1
			user << "<span class=\"notify\">You insert the [W] into the core sampler.</span>"
		else
			user << "<span class=\"rose\">The core sampler can not fit any more bags!</span>"
	else
		return ..()
...
/obj/item/device/core_sampler/proc/sample_item(var/item_to_sample, var/mob/user as mob)
	...
	if(geo_data)
		if(filled_bag)
			user << "<span class=\"rose\">The core sampler is full!</span>"
		else if(num_stored_bags < 1)
			user << "<span class=\"rose\">The core sampler is out of sample bags!</span>"
		else
			...
			user << "<span class=\"notify\">You take a core sample of the [item_to_sample].</span>"
	else
		user << "<span class=\"rose\">You are unable to take a sample of [item_to_sample].</span>"

/obj/item/device/core_sampler/attack_self()
	if(filled_bag)
		usr << "<span class=\"notify\">You eject the full sample bag.</span>"
		var/success = 0
		...
		usr << "<span class=\"rose\">The core sampler is empty.</span>"
