...
/datum/artifact_trigger/pay2use/proc/owner_attackhand(var/list/event_args, var/source)
	...
	if(context != "TOUCH" && mode != BANK_CARD)
		...
		dat += "20 Seconds - $10 - "
		dat += "<A href='?src=\ref[src];payt1=1'>Pay</a><BR>"

		dat += "1 Minute - $30 - "
		dat += "<A href='?src=\ref[src];payt2=1'>Pay</a><BR>"

		dat += "5 Minutes - $150 - "
		dat += "<A href='?src=\ref[src];payt3=1'>Pay</a><BR>"

		dat += "10 Minutes - $300 - "
		dat += "<A href='?src=\ref[src];payt4=1'>Pay</a><BR>"
...
/datum/artifact_trigger/pay2use/proc/owner_attackby(var/list/event_args, var/source)
	...
	if(context == "MELEE")
		if(mode == COIN && iscoin(item))
			my_artifact.investigation_log(I_ARTIFACT, "|| effect [my_effect.artifact_id]([my_effect]) || [item] inserted [context] to ([my_effect.trigger]) || touched by [key_name(toucher)].")
			...
			if(istype(item, /obj/item/weapon/coin/clown))
				...
				time_left += 150
			qdel(item)
		else if(mode == CREDIT && istype(item, /obj/item/weapon/spacecash))
			my_artifact.investigation_log(I_ARTIFACT, "|| effect [my_effect.artifact_id]([my_effect]) || [item] inserted [context] to ([my_effect.trigger]) || touched by [key_name(toucher)].")
...
/datum/artifact_trigger/pay2use/proc/payviacard(var/dosh = 0, var/mob)
