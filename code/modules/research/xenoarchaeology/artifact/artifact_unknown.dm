...
/obj/machinery/artifact/attack_hand(var/mob/user as mob)
	...
	if(my_effect.trigger == TRIGGER_TOUCH)
		...
		src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by TOUCH([my_effect.trigger]) || Last touched by [user.name] ([user.ckey]).")
		my_effect.ToggleActivate()
	else
	...
	if(prob(25) && secondary_effect && secondary_effect.trigger == TRIGGER_TOUCH)
		src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by TOUCH([my_effect.trigger]) || Last touched by [user.name] ([user.ckey]).")
		secondary_effect.ToggleActivate(0)
	...
	if (istype(W, /obj/item/weapon/reagent_containers/glass) && W.is_open_container() ||\
		...
		if(W.reagents.has_reagent(HYDROGEN, 1) || W.reagents.has_reagent(WATER, 1))
			if(my_effect.trigger == TRIGGER_WATER)
				src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by WATER([my_effect.trigger]) || Splashed with [W] by [user.name] ([user.ckey]).")
				my_effect.ToggleActivate()
			if(secondary_effect && secondary_effect.trigger == TRIGGER_WATER && prob(25))
				src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by WATER([secondary_effect.trigger]) || Splashed with [W] by [user.name] ([user.ckey]).")
				secondary_effect.ToggleActivate(0)
			...
			if(my_effect.trigger == TRIGGER_ACID)
				src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by ACID([my_effect.trigger]) || Splashed with [W] by [user.name] ([user.ckey]).")
				my_effect.ToggleActivate()
			if(secondary_effect && secondary_effect.trigger == TRIGGER_ACID && prob(25))
				src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by ACID([secondary_effect.trigger]) || Splashed with [W] by [user.name] ([user.ckey]).")
				secondary_effect.ToggleActivate(0)
		else if(W.reagents.has_reagent(PLASMA, 1) || W.reagents.has_reagent(THERMITE, 1))
			if(my_effect.trigger == TRIGGER_VOLATILE)
				src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by VOLATILE([my_effect.trigger]) || Splashed with [W] by [user.name] ([user.ckey]).")
				my_effect.ToggleActivate()
			if(secondary_effect && secondary_effect.trigger == TRIGGER_VOLATILE && prob(25))
				src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by VOLATILE([secondary_effect.trigger]) || Splashed with [W] by [user.name] ([user.ckey]).")
				secondary_effect.ToggleActivate(0)
			...
			if(my_effect.trigger == TRIGGER_TOXIN)
				src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by TOXIN([my_effect.trigger]) || Splashed with [W] by [user.name] ([user.ckey]).")
				my_effect.ToggleActivate()
			if(secondary_effect && secondary_effect.trigger == TRIGGER_TOXIN && prob(25))
				src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by TOXIN([secondary_effect.trigger]) || Splashed with [W] by [user.name] ([user.ckey]).")
				secondary_effect.ToggleActivate(0)
		...
		if (my_effect.trigger == TRIGGER_ENERGY)
			src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by ENERGY([my_effect.trigger]) || Energized with [W] by [user.name] ([user.ckey]).")
			my_effect.ToggleActivate()
		if(secondary_effect && secondary_effect.trigger == TRIGGER_ENERGY && prob(25))
			src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by ENERGY([secondary_effect.trigger]) || Energized with [W] by [user.name] ([user.ckey]).")
			secondary_effect.ToggleActivate(0)
		...
		if(my_effect.trigger == TRIGGER_HEAT)
			src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by HEAT([my_effect.trigger]) || Heated with [W] by [user.name] ([user.ckey]).")
			my_effect.ToggleActivate()
		if(secondary_effect && secondary_effect.trigger == TRIGGER_HEAT && prob(25))
			src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by HEAT([secondary_effect.trigger]) || Heated with [W] by [user.name] ([user.ckey]).")
			secondary_effect.ToggleActivate(0)
	else
		..()
		if (my_effect.trigger == TRIGGER_FORCE && W.force >= 10)
			src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by FORCE([my_effect.trigger]) || Attacked with [W] by [user.name] ([user.ckey]).")
			my_effect.ToggleActivate()
		if(secondary_effect && secondary_effect.trigger == TRIGGER_FORCE && prob(25))
			src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by FORCE([secondary_effect.trigger]) || Attacked with [W] by [user.name] ([user.ckey]).")
			secondary_effect.ToggleActivate(0)
	...
	if(istype(M,/obj))
		if(M:throwforce >= 10)
			if(my_effect.trigger == TRIGGER_FORCE)
				src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by THROW FORCE([my_effect.trigger]) || Hit by [M].")
				my_effect.ToggleActivate()
			if(secondary_effect && secondary_effect.trigger == TRIGGER_FORCE && prob(25))
				src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by THROW FORCE([secondary_effect.trigger]) || Hit by [M].")
				secondary_effect.ToggleActivate(0)
...
/obj/machinery/artifact/bullet_act(var/obj/item/projectile/P)
	if(istype(P,/obj/item/projectile/bullet) ||\
		...
		if(my_effect.trigger == TRIGGER_FORCE)
			src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by PROJECTILE FORCE([my_effect.trigger]) || [P] fired by [key_name(P.firer)].")
			my_effect.ToggleActivate()
		if(secondary_effect && secondary_effect.trigger == TRIGGER_FORCE && prob(25))
			src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by PROJECTILE FORCE([secondary_effect.trigger]) || [P] fired by [key_name(P.firer)].")
			secondary_effect.ToggleActivate(0)
		...
		if(my_effect.trigger == TRIGGER_ENERGY)
			src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by BEAM ENERGY([my_effect.trigger]) || [P] fired by [key_name(P.firer)].")
			my_effect.ToggleActivate()
		if(secondary_effect && secondary_effect.trigger == TRIGGER_ENERGY && prob(25))
			src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by BEAM ENERGY([secondary_effect.trigger]) || [P] fired by [key_name(P.firer)].")
			secondary_effect.ToggleActivate(0)
		...
		if(2.0)
			if (prob(50))
				...
				if(my_effect.trigger == TRIGGER_FORCE || my_effect.trigger == TRIGGER_HEAT)
					src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by BOMB DAMAGE([severity])")
					my_effect.ToggleActivate()
				if(secondary_effect && (secondary_effect.trigger == TRIGGER_FORCE || secondary_effect.trigger == TRIGGER_HEAT) && prob(25))
					src.investigation_log(I_ARTIFACT, "|| secondary effect([secondary_effect]) triggered by BOMB DAMAGE([severity])")
					secondary_effect.ToggleActivate(0)
		if(3.0)
			if (my_effect.trigger == TRIGGER_FORCE || my_effect.trigger == TRIGGER_HEAT)
				src.investigation_log(I_ARTIFACT, "|| primary effect([my_effect]) triggered by BOMB DAMAGE([severity])")
