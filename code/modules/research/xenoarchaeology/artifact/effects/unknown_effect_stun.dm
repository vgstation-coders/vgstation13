...
/datum/artifact_effect/stun/DoEffectTouch(var/mob/toucher)
	if(toucher && iscarbon(toucher))
		...
		if(prob(weakness * 100))
			C << "<span class=\"rose\">A powerful force overwhelms your consciousness.</span>"
...
/datum/artifact_effect/stun/DoEffectAura()
	if(holder)
		for (var/mob/living/carbon/C in range(src.effectrange,holder))
			var/weakness = GetAnomalySusceptibility(C)
			if(prob(10 * weakness))
				C << "<span class=\"rose\">Your body goes numb for a moment.</span>"
				C.weakened += 2
				C.stuttering += 2
				if(prob(10))
					C.stunned += 1
			else if(prob(10))
				C << "<span class=\"rose\">You feel numb.</span>"
	...
	if(holder)
		for (var/mob/living/carbon/C in range(src.effectrange,holder))
			...
			if(prob(100 * weakness))
				C << "<span class=\"rose\">A wave of energy overwhelms your senses!</span>"
