...
/obj/item/weapon/archaeological_find/New(loc, var/new_item_type)
	...
	if(new_item)
		...
		origin_tech = Tc_ANOMALY+"=[anomaly_factor]"

		if(talkative)
			listening_to_players = TRUE
			if(prob(25))
				if(!heard_words)
					heard_words = list()
				speaking_to_players = TRUE
