		...
		if(do_after(user, src, rand(10,40)))
			if(machine)
				...
				if(assembling == UNASSEMBLED)
					overlays += image(icon = icon, icon_state = "assembly")
					var/obj/item/paper/instructions = new (get_turf(src))
...
/datum/construction/flatpack_unpack/New(var/atom/A)
	...
	while(((steps.len <= 7) && prob(80)) || steps.len <= 3)
		...
		switch(current_tool)
			if("weldingtool")
				steps[steps.len] = list(Co_KEY=/obj/item/weldingtool,
							...
							Co_DELAY = 30)
			if("screwdriver")
				steps[steps.len] = list(Co_KEY=/obj/item/screwdriver,
							...
							Co_DELAY = 30)
			if("wrench")
				steps[steps.len] = list(Co_KEY=/obj/item/wrench,
							...
							Co_DELAY = 30)
			if("wirecutter")
				steps[steps.len] = list(Co_KEY=/obj/item/wirecutters,
