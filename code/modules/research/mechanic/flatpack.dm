...
/datum/construction/flatpack_unpack/New(var/atom/A)
	...
	while(((steps.len <= 7) && prob(80)) || steps.len <= 3)
		...
		switch(current_tool)
			if("weldingtool")
				steps[steps.len] = list(Co_KEY=/obj/item/weapon/tool/weldingtool,
							...
							Co_DELAY = 30)
			if("screwdriver")
				steps[steps.len] = list(Co_KEY=/obj/item/weapon/tool/screwdriver,
							...
							Co_DELAY = 30)
			if("wrench")
				steps[steps.len] = list(Co_KEY=/obj/item/weapon/tool/wrench,
							...
							Co_DELAY = 30)
			if("wirecutter")
				steps[steps.len] = list(Co_KEY=/obj/item/weapon/tool/wirecutters,
