...
/obj/machinery/r_n_d/reverse_engine/proc/researchQueue()
	...
	if(!research_queue.len)
		visible_message("<span class='notice'>[bicon(src)] \The [src] beeps: 'Successfully researched all designs.'</span>")
...
/obj/machinery/r_n_d/reverse_engine/proc/researchDesign(var/datum/design/design)
	...
	if(design in ready_queue)
		visible_message("<span class='notice'>[bicon(src)] \The [src] beeps:'The [design.name] is already researched.'</span>")
		research_queue -= design
		return
	...
	if(!(design in ready_queue))
		ready_queue += design
		visible_message("<span class='notice'>[bicon(src)] \The [src] beeps: 'Successfully researched \the [design.name].'</span>")
		return 1
	return
...
/obj/machinery/r_n_d/reverse_engine/proc/PrintDesign(var/datum/design/design, var/use_nano = 0)
	if(linked_console)
		...
		visible_message("You need to link this machine to a research console first!")
