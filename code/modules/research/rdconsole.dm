		...
		for(var/name_set in linked_imprinter.part_sets)
			...
			for(var/datum/design/D in files.known_designs)
				if(!(D.build_type & IMPRINTER) || D.category != name_set)
					continue
				imprinter_designs.Add(list(list("name" = D.name, "cost" = linked_imprinter.output_part_cost(D), "command1" = list("imprint" = D.id, "n" = 1), "command5" = list("imprint" = D.id, "n" = 5), "command10" = list("imprint" = D.id, "n" = 10), "command2" = list("imprint" = D.id, "customamt" = 1), "command3" = list("imprint" = D.id, "n" = 1, "now" = 1))))
