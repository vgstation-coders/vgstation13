...
/datum/tg_asset/spritesheet_batched
	_abstract = /datum/tg_asset/spritesheet_batched
	var/name
	/// list("32x32")
	var/list/sizes = list()
...
/datum/tg_asset/spritesheet_batched/proc/should_load_immediately()
...
/datum/tg_asset/spritesheet_batched/should_refresh(yield)
	...
	if (data_out == RUSTG_JOB_ERROR)
		...
		rustg_file_write(cache_data, "[global.log_directory]/spritesheet_cache_debug.[name].json")
		rustg_file_write(entries_json, "[global.log_directory]/spritesheet_debug_[name].json")
...
/datum/tg_asset/spritesheet_batched/proc/insert_icon(sprite_name, datum/universal_icon/entry)
...
/datum/tg_asset/spritesheet_batched/register()
...
/datum/tg_asset/spritesheet_batched/unregister()
	CRASH("unregister() called on batched spritesheet! Bad!")

/// Call insert_icon or insert_all_icons here, building a spritesheet!
/datum/tg_asset/spritesheet_batched/proc/create_spritesheets()
	CRASH("create_spritesheets() not implemented for [type]!")

/datum/tg_asset/spritesheet_batched/proc/insert_all_icons(prefix, icon/I, list/directions, prefix_with_dirs = TRUE)
	if (length(prefix))
		prefix = "[prefix]-"
...
/datum/tg_asset/spritesheet_batched/proc/realize_spritesheets(yield)
	if(fully_generated)
		return
	if(!length(entries))
...
/datum/tg_asset/spritesheet_batched/queued_generation()
	realize_spritesheets(yield = TRUE)

/datum/tg_asset/spritesheet_batched/ensure_ready()
	if(!fully_generated)
		realize_spritesheets(yield = FALSE)
	return ..()

/datum/tg_asset/spritesheet_batched/send(client/client)
	if (!name)
		return
...
/datum/tg_asset/spritesheet_batched/get_url_mappings()
	if (!name)
		return
...
/datum/tg_asset/spritesheet_batched/proc/generate_css()
	var/list/out = list()
...
/datum/tg_asset/spritesheet_batched/proc/read_from_cache()
...
/datum/tg_asset/spritesheet_batched/proc/get_background_url(asset)
	return SSassets.transport.get_asset_url(asset)

/datum/tg_asset/spritesheet_batched/proc/write_cache_meta(input_hash, dmi_hashes)
...
/datum/tg_asset/spritesheet_batched/proc/css_tag()
	return {"<link rel="stylesheet" href="[css_filename()]" />"}

/datum/tg_asset/spritesheet_batched/proc/css_filename()
	return SSassets.transport.get_asset_url("spritesheet_[name].css")

/datum/tg_asset/spritesheet_batched/proc/icon_tag(sprite_name)
...
/datum/tg_asset/spritesheet_batched/proc/icon_class_name(sprite_name)
	var/sprite = sprites[sprite_name]
	if (!sprite)
		return null
	var/size_id = sprite[SPR_SIZE]
 ...
 */
/datum/tg_asset/spritesheet_batched/proc/icon_size_id(sprite_name)
	var/sprite = sprites[sprite_name]
	if (!sprite)
		return null
	var/size_id = sprite[SPR_SIZE]
