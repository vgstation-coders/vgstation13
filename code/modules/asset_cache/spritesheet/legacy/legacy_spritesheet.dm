...
/// Deprecated: Use /datum/tg_asset/spritesheet_batched where possible
/datum/tg_asset/spritesheet
	_abstract = /datum/tg_asset/spritesheet
...
/datum/tg_asset/spritesheet/proc/should_load_immediately()
...
/datum/tg_asset/spritesheet/should_refresh()
	if (..())
		return TRUE
...
/datum/tg_asset/spritesheet/unregister()
...
/datum/tg_asset/spritesheet/regenerate()
	unregister()
	sprites = list()
...
/datum/tg_asset/spritesheet/register()
...
/datum/tg_asset/spritesheet/proc/realize_spritesheets(yield)
	if(fully_generated)
		return
...
/datum/tg_asset/spritesheet/queued_generation()
	realize_spritesheets(yield = TRUE)

/datum/tg_asset/spritesheet/ensure_ready()
	if(!fully_generated)
		realize_spritesheets(yield = FALSE)
	return ..()

/datum/tg_asset/spritesheet/send(client/client)
	if (!name)
		return
...
/datum/tg_asset/spritesheet/get_url_mappings()
	if (!name)
		return
...
/datum/tg_asset/spritesheet/proc/ensure_stripped(sizes_to_strip = sizes)
...
/datum/tg_asset/spritesheet/proc/generate_css()
	var/list/out = list()
...
/datum/tg_asset/spritesheet/proc/css_cache_filename()
	return "[ASSET_CROSS_ROUND_CACHE_DIRECTORY]/spritesheet.[name].css"

/datum/tg_asset/spritesheet/proc/data_cache_filename()
	return "[ASSET_CROSS_ROUND_CACHE_DIRECTORY]/spritesheet.[name].json"

/datum/tg_asset/spritesheet/proc/read_from_cache()
	return read_css_from_cache() && read_data_from_cache()

/datum/tg_asset/spritesheet/proc/read_css_from_cache()
...
/datum/tg_asset/spritesheet/proc/read_data_from_cache()
...
/datum/tg_asset/spritesheet/proc/send_from_cache(client/client)
...
/datum/tg_asset/spritesheet/proc/get_background_url(asset)
	if (generating_cache)
		return "%[asset]%"
	else
		return SSassets.transport.get_asset_url(asset)

/datum/tg_asset/spritesheet/proc/write_to_cache()
	write_css_to_cache()
	write_data_to_cache()

/datum/tg_asset/spritesheet/proc/write_css_to_cache()
	for (var/size_id in sizes)
...
/datum/tg_asset/spritesheet/proc/write_data_to_cache()
	rustg_file_write(json_encode(list(
		"sprites" = sprites,
	)), data_cache_filename())

/datum/tg_asset/spritesheet/proc/get_cached_url_mappings()
...
/datum/tg_asset/spritesheet/proc/create_spritesheets()
	CRASH("create_spritesheets() not implemented for [type]!")

/datum/tg_asset/spritesheet/proc/Insert(sprite_name, icon/inserted_icon, icon_state="", dir=SOUTH, frame=1, moving=FALSE)
	if(should_load_immediately())
		queuedInsert(sprite_name, inserted_icon, icon_state, dir, frame, moving)
	else
		to_generate += list(args.Copy())

/datum/tg_asset/spritesheet/proc/queuedInsert(sprite_name, icon/inserted_icon, icon_state="", dir=SOUTH, frame=1, moving=FALSE)
#ifdef UNIT_TESTS
 ...
 */
/datum/tg_asset/spritesheet/proc/ModifyInserted(icon/pre_asset)
	return pre_asset

/datum/tg_asset/spritesheet/proc/InsertAll(prefix, icon/inserted_icon, list/directions)
	if (length(prefix))
		prefix = "[prefix]-"
...
/datum/tg_asset/spritesheet/proc/css_tag()
	return {"<link rel="stylesheet" href="[css_filename()]" />"}

/datum/tg_asset/spritesheet/proc/css_filename()
	return SSassets.transport.get_asset_url("spritesheet_[name].css")

/datum/tg_asset/spritesheet/proc/icon_tag(sprite_name)
...
/datum/tg_asset/spritesheet/proc/icon_class_name(sprite_name)
	var/sprite = sprites[sprite_name]
	if (!sprite)
		return null
	var/size_id = sprite[SPR_SIZE]
 ...
 */
/datum/tg_asset/spritesheet/proc/icon_size_id(sprite_name)
	var/sprite = sprites[sprite_name]
	if (!sprite)
		return null
	var/size_id = sprite[SPR_SIZE]
...
/// Deprecated: Use /datum/tg_asset/spritesheet_batched where possible
/datum/tg_asset/spritesheet/simple
	_abstract = /datum/tg_asset/spritesheet/simple
	/// Associative list of icon keys (CSS class names) -> PNG filepaths (single quote!)
	/// File paths MUST be PNGs
	var/list/assets

/datum/tg_asset/spritesheet/simple/create_spritesheets()
