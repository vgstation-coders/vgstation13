 ...
 */
/datum/tgui/New(mob/user, datum/src_object, ui_key, interface, title, width = 0, height = 0, datum/tgui/master_ui = null, datum/ui_state/state = global.default_state)
	src.user = user
	...
	var/datum/asset/simple/asset_datum = new /datum/asset/simple/fontawesome
	send_asset_list(user.client, asset_datum.assets)

	asset_datum = new /datum/asset/simple/tgui
	send_asset_list(user.client, asset_datum.assets)
