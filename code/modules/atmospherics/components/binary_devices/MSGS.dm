...
/obj/machinery/atmospherics/binary/msgs/updateUsrDialog()
	...
	if(round(total_moles, 0.01))	//Check if there's total moles to avoid divisions by zero.
		interface.updateContent("oxypercent", Clamp(round(100 * air.gas[GAS_OXYGEN]   / total_moles, 0.1), 0, 100))
		interface.updateContent("nitpercent", Clamp(round(100 * air.gas[GAS_NITROGEN] / total_moles, 0.1), 0, 100))
		interface.updateContent("co2percent", Clamp(round(100 * air.gas[GAS_CARBON]   / total_moles, 0.1), 0, 100))
		interface.updateContent("plapercent", Clamp(round(100 * air.gas[GAS_PLASMA]   / total_moles, 0.1), 0, 100))
		interface.updateContent("n2opercent", Clamp(round(100 * air.gas[GAS_SLEEPING] / total_moles, 0.1), 0, 100))
