/datum/job
	...
	//Job access. The use of minimal_access or access is determined by a config setting: CONFIG_GET(toggle/jobs_have_minimal_access)
...
/datum/job/proc/get_access()
	...
	if(CONFIG_GET(toggle/jobs_have_minimal_access))
		return src.minimal_access.Copy()
	else
...
/datum/job/proc/available_in_days(client/C)
	if(!C)
		return 0
	if(!CONFIG_GET(toggle/use_age_restriction_for_jobs))
		return 0
