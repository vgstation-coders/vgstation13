...
/obj/machinery/singularity/proc/expand(var/force_size = 0, var/growing = 1)
	...
	switch(temp_allowed_size)
		if(STAGE_ONE)
			current_size = 1
			icon = 'icons/obj/singularity.dmi'
			icon_state = modifier + "singularity_s1"
			pixel_x = 0
			pixel_y = 0
		...
		if(STAGE_TWO)
			...
			icon_state = modifier + "singularity_s3"
		...
		if(STAGE_THREE)
			...
			icon_state = modifier + "singularity_s5"
		...
		if(STAGE_FOUR)
			...
			icon_state = modifier + "singularity_s7"
		...
		if(STAGE_FIVE)
			...
			icon_state = modifier + "singularity_s9"
		...
		if(STAGE_SUPER) //SUPERSINGULO
			...
			icon_state = modifier + "singularity_s11" //Uh, whoever drew that, you know that black holes are supposed to look dark right? What's this, the clown's singulo?
		...
		if(STAGE_SSGSS) //SUPER SINGULO GOD SUPER SINGULO
			...
			icon_state = modifier + "singularity_s13" //black holes being black is so 2525
