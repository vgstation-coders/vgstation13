		...
		if(M.stat == CONSCIOUS)
			if (istype(M,/mob/living/carbon/human))
				...
				if(istype(H.glasses,/obj/item/clothing/glasses/meson))
					...
					return
		M << "<span class=\"rose\">You look directly into The [src.name] and feel weak.</span>"
...
/obj/machinery/singularity/narsie/mezzer()
	for(var/mob/living/carbon/M in oviewers(8, src))
		if(M.stat == CONSCIOUS)
			if(!iscultist(M))
				M << "<span class=\"rose\">You feel your sanity crumble away in an instant as you gaze upon [src.name]...</span>"
				M.apply_effect(3, STUN)
...
/obj/machinery/singularity/narsie/proc/acquire(var/mob/food)
	...
	target = food
	if(ishuman(target))
		target << "<span class=\"danger\">NAR-SIE HUNGERS FOR YOUR SOUL</span>"
	else
		target << "<span class=\"danger\">NAR-SIE HAS CHOSEN YOU TO LEAD HIM TO HIS NEXT MEAL</span>"
