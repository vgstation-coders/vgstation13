...
/obj/machinery/shield_gen/Topic(href, href_list[])
	...
	if(href_list["toggle_active"])
		...
		field_radius = clamp(round(input(usr,"Set new field radius","Field radius",field_radius)), MIN_FIELD_RADIUS, MAX_FIELD_RADIUS)
	else if(href_list["set_strengthen_rate"])
		strengthen_rate = clamp(round(input(usr,"Set new strengthen rate","Strengthen rate",strengthen_rate),0.01), MIN_STRENGTHEN_RATE, MAX_STRENGTHEN_RATE)
	else if(href_list["set_field_strength_cap"])
		field_strength_cap = clamp(round(input(usr,"Set new field strength cap","Field strength cap",field_strength_cap)), MIN_FIELD_STRENGTH_CAP, MAX_FIELD_STRENGTH_CAP)
