...
/obj/item/clothing/tie/holobadge/attack_self(mob/user as mob)
	if(!stored_name)
		...
		return
	if(isliving(user))
		user.visible_message("<span class=\"rose\">[user] displays their NanoTrasen Internal Security Legal Authorization Badge.\nIt reads: [stored_name], NT Security.</span>","<span class=\"rose\">You display your NanoTrasen Internal Security Legal Authorization Badge.\nIt reads: [stored_name], NT Security.</span>")

/obj/item/clothing/tie/holobadge/attackby(var/obj/item/O as obj, var/mob/user as mob)

	if (istype(O, /obj/item/weapon/card/emag))
		if (emagged)
			user << "<span class=\"rose\">[src] is already cracked.</span>"
			return
		else
			emagged = 1
			user << "<span class=\"rose\">You swipe [O] and crack the holobadge security checks.</span>"
			return
...
/obj/item/clothing/tie/holobadge/attack(mob/living/carbon/human/M, mob/living/user)
	if(isliving(user))
		user.visible_message("<span class=\"rose\">[user] invades [M]'s personal space, thrusting [src] into their face insistently.</span>","<span class=\"rose\">You invade [M]'s personal space, thrusting [src] into their face insistently. You are the law.</span>")
