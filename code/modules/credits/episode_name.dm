...
/datum/credits/proc/draft_episode_names()
	...
	episode_names += new /datum/episode_name("[pick("HUNT FOR THE GREEN WEENIE", "ALIEN VS VENDOMAT", "SPACE TRACK", "SEX BOMB", "WHOSE IDEA WAS THIS ANYWAY?", "WHATEVER HAPPENED, HAPPENED", "THE GOOD, THE BAD, AND [uppertext(station_name())]", "RESTRAIN YOUR ENJOYMENT")]")
	...
	if(istype(ticker.mode, /datum/gamemode/dynamic))
		...
		switch(mode.threat_level)
			...
			if(30 to 70)
				episode_names += new /datum/episode_name("[pick("THERE MIGHT BE BLOOD", "IT CAME FROM [uppertext(station_name())]!", "THE BALLAD OF [uppertext(station_name())]", "REQUIEM FOR [uppertext(station_name())]", "THE [uppertext(station_name())] INCIDENT", "THE ENEMY WITHIN")]", "Moderate threat level of [mode.threat_level]%.", 200)
			if(70 to 100)
				episode_names += new /datum/episode_name("[pick("ATTACK! ATTACK! ATTACK!", "SPACE 'NAM", "WAR IS THE H-WORD", "CAN'T FIX CRAZY", "SOME LIKE IT HOT", "APOCALYPSE [pick("W", "N", "H")]OW", "A TASTE OF ARMAGEDDON", "OPERATION: ANNIHILATE!", "THE PERFECT STORM")]", "High threat level of [mode.threat_level]%.", 200)
	...
	if(score["gunsspawned"] > 0)
		episode_names += new /datum/episode_name("GUNS, GUNS EVERYWHERE", "[score["gunsspawned"]] guns were spawned this round.", max(300, score["gunsspawned"]*5))
