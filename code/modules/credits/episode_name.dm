...
/datum/credits/proc/draft_episode_names()
	...
	if(istype(ticker.mode, /datum/gamemode/dynamic))
		...
		switch(mode.threat_level)
			if(0 to 30)
				episode_names += new /datum/episode_name("[pick("THE DAY [uppr_name] STOOD STILL", "MUCH ADO ABOUT NOTHING", "WHERE SILENCE HAS LEASE", "RED HERRING", "THIS SIDE OF PARADISE", "HOME ALONE", "[uppr_name]: A SITUATION COMEDY", "THE SILENT SHUFFLE", "GO BIG OR GO [uppr_name]", "PLACEBO EFFECT")]", "Low threat level of [mode.threat_level]%.", 200)
			if(30 to 70)
		...
		if(locate(/datum/dynamic_ruleset/roundstart/malf) in mode.executed_rules)
			episode_names += new /datum/episode_name("[pick("I'M SORRY [uppr_name], I'M AFRAID I CAN'T LET YOU DO THAT", "A STRANGE GAME")]", "Round included a malfunctioning AI.", 250)
	...
	if(score["escapees"] == 0 && emergency_shuttle.location == CENTCOMM_Z)
		episode_names += new /datum/episode_name("[pick("DEAD SPACE", "THE CREW GOES MISSING", "LOST IN TRANSLATION", "[uppr_name]: DELETED SCENES", "WHAT HAPPENS IN [uppr_name], STAYS IN [uppr_name]")]", "There were no escapees on the shuttle.", 200)
	if(score["slips"] > 100)
	...
	if(emergency_shuttle.location == CENTCOMM_Z)
		...
		if(shuttle)
			...
			if(score["escapees"] == 1)
				for(var/mob/living/carbon/human/H in shuttle)
			...
			for(var/mob/living/simple_animal/bee/B in shuttle)
				beecount += B.bees.len
			if(beecount > 20)
				episode_names += new /datum/episode_name("FLIGHT OF THE BUMBLEBEES", "There were [beecount] bees on the shuttle.", min(750, beecount*20))
			...
			if(score["random_soc"] > 7)
				...
				if(nasty_things.len > 3)
					...
					episode_names += new /datum/episode_name(final_string, "Thanks to the Staff of Change, we had quite a diverse shuttle.", 750)
