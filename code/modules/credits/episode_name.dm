...
/datum/credits/proc/draft_episode_names()
	...
	episode_names += new /datum/episode_name("THE CREW [pick("GOES JIHAD", "GOES ON WELFARE", "GIVES BACK", "SELLS OUT", "GETS WHACKED", "BITES THE DUST", "SOLVES THE PLASMA CRISIS", "HITS THE ROAD", "GETS AN ANAL PROBE", "RULES THE WORLD", "RISES", "SAVES THE DAY", "RETIRES", "GOES TO HELL", "GETS NEW WHEELS", "DOES A CLIP SHOW", "GETS AUDITED", "DOES A TV COMMERCIAL", "AFTER HOURS", "GOES TOO FAR", "GIVES UP THE DREAM")]")
	episode_names += new /datum/episode_name("THE CREW'S [pick("DAY OUT", "PUNISHMENT", "BIG GAY ADVENTURE", "LUNCH BREAK", "LAST DAY", "[pick("WILD", "WACKY", "LAME")] VACATION", "CHANGE OF HEART", "BIG BREAK", "BACK IN BUSINESS", "NEW GROOVE", "DONE FOR", "SCHOOL MUSICAL", "HISTORY LESSON", "FLYING CIRCUS")]")
	...
	episode_names += new /datum/episode_name("[pick("BALANCE OF POWER", "SPACE TRACK", "SEX BOMB", "WHOSE IDEA WAS THIS ANYWAY?", "WHATEVER HAPPENED, HAPPENED", "THE GOOD, THE BAD, AND [uppertext(station_name())]", "RESTRAIN YOUR ENJOYMENT", "[uppertext(station_name())]: A NATIONAL CONCERN", "A PUBLIC RELATIONS NIGHTMARE", "WITH APOLOGIES TO THE CREW", "PILOT", "REAL HOUSEWIVES OF [uppertext(station_name())]")]")
	...
	if(istype(ticker.mode, /datum/gamemode/dynamic))
		...
		switch(mode.threat_level)
			...
			if(70 to 100)
				episode_names += new /datum/episode_name("[pick("ATTACK! ATTACK! ATTACK!", "SPACE 'NAM", "WAR IS THE H-WORD", "CAN'T FIX CRAZY", "SOME LIKE IT HOT", "APOCALYPSE [pick("W", "N", "H")]OW", "A TASTE OF ARMAGEDDON", "OPERATION: ANNIHILATE!", "THE PERFECT STORM", "TIME'S UP FOR THE CREW", "THE END OF [uppertext(station_name())] AS WE KNOW IT", "A TOTALLY FUN THING THAT THE CREW WILL NEVER DO AGAIN")]", "High threat level of [mode.threat_level]%.", 200)
