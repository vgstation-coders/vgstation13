...
//Use self to call procs on the reagent instead (remember to inherit the base proc
	...
	if(!istype(self.holder.my_atom, /obj/effect/effect/smoke/chem))
		...
		if(method == TOUCH)
			...
			if(prob(chance) && !block)
				if(M.reagents)
					M.reagents.add_reagent(self.id, self.volume * 0.5) //I guess ?
	return 1
...
/datum/reagent/proc/on_mob_life(var/mob/living/M as mob, var/alien)
	...
	if(!M) //Just in case
		M = holder.my_atom
	if(!isliving(M)) //Make sure we aren't trying to affect a non-living mob
		return
...
/datum/reagent/muhhardcores/on_mob_life(var/mob/living/M)
	...
	if(prob(1))
		if(prob(90))
			M << "<span class='notice'>[pick("You feel quite hardcore","Coderbased is your god", "Fucking kickscammers Bustration will be the best")].</span>"
		else
			...
			if(method == TOUCH)
				...
				if(!M.unacidable)
					if(prob(15) && volume >= 30)
						...
						if(affecting)
							if(affecting.take_damage(0, volume * 2))
								H.UpdateDamageIcon(1)
							H.status_flags |= DISFIGURED
							H.emote("scream", , , 1)
					else
						H.take_organ_damage(0, volume * 2)
			else
				if(!H.unacidable)
					H.take_organ_damage(0, volume * 2)
...
/datum/reagent/slimetoxin/on_mob_life(var/mob/living/M as mob)
	...
	if(iscarbon(M))
		...
		if(istype(C, /mob/living/carbon/human/manifested))
			C << "<span class='warning'>You can feel intriguing reagents seeping into your body, but they don't seem to react at all.</span>"
			C.reagents.del_reagent("mutationtoxin")
			return
		if(ishuman(C))
			var/mob/living/carbon/human/H = C
			if(H.dna.mutantrace == null)
				H << "<span class='warning'>Your flesh rapidly mutates!</span>"
...
/datum/reagent/aslimetoxin/on_mob_life(var/mob/living/M as mob)
	...
	if(iscarbon(M))
		...
		if(istype(C, /mob/living/carbon/human/manifested))
			...
			if(C.monkeyizing)
				return
			C << "<span class='warning'>Your flesh rapidly mutates!</span>"
			C.monkeyizing = 1
			C.canmove = 0
			C.icon = null
			C.overlays.len = 0
			...
			for(var/obj/item/W in C)
				...
				W.loc = M.loc
				W.dropped(M)
			var/mob/living/carbon/slime/new_mob = new /mob/living/carbon/slime(get_turf(C))
	...
	if(ishuman(M))
		...
		if(iscult(H))
			if(prob(10)) //1/10 chance of removing cultist status, so 50 units on average to uncult (half a hole water bottle)
				...
				H << "<span class='danger'>A freezing liquid permeates your bloodstream. Your arcane knowledge is becoming osbscure again.</span>"
		...
		if(isvampire(H))
			if(!(VAMP_MATURE in H.mind.vampire.powers))
				H << "<span class='danger'>A freezing liquid permeates your bloodstream. Your vampiric powers fade and your insides burn.</span>"
				H.take_organ_damage(0, 2*REM) //FIRE
				H.mind.vampire.smitecounter += 2*REM //50 units to catch on fire. Generally you'll get fucked up quickly
			else
				H << "<span class='warning'>A freezing liquid permeates your bloodstream. Your vampiric powers counter most of the damage.</span>"
				H.mind.vampire.smitecounter += 0.4*REM //Basically nothing, unless you drank multiple bottles of holy water (250 units to catch on fire !)
		if(H.species.name == "Grey")
			H.adjustToxLoss(REM)
			H.take_organ_damage(0, 2*REM)
				...
				if(method == TOUCH)
					...
					if(!H.unacidable)
						if(prob(15) && volume >= 30)
							...
							if(affecting)
								if(!(VAMP_MATURE in H.mind.vampire.powers))
									H << "<span class='danger'>A freezing liquid covers your face. It's melting!</span>"
									H.mind.vampire.smitecounter += volume * 2 //Equivalent from metabolizing all this holy water normally
									if(affecting.take_damage(0, volume * 2))
										H.UpdateDamageIcon(1)
									H.status_flags |= DISFIGURED
									H.emote("scream", , , 1)
								else
									H << "<span class='warning'>A freezing liquid covers your face. Your vampiric powers protect you!</span>"
									H.mind.vampire.smitecounter += volume * 0.4 //Ditto above

						else
							if(!(VAMP_MATURE in M.mind.vampire.powers))
								H << "<span class='danger'>You are doused with a freezing liquid. You're melting!</span>"
								H.take_organ_damage(0, volume * 2)
								H.mind.vampire.smitecounter += volume * 2
							else
								H << "<span class='warning'>You are doused with a freezing liquid. Your vampiric powers protect you!</span>"
			...
			if(method == TOUCH)
				...
				if(!H.unacidable)
					if(prob(15) && volume >= 30)
						var/datum/organ/external/affecting = H.get_organ("head")
						if(affecting)
							if(affecting.take_damage(0, volume * 2, 0))
								H.UpdateDamageIcon(1)
							H.status_flags |= DISFIGURED
							H.emote("scream", , , 1)
					else
						H.take_organ_damage(0, volume * 2) //Uses min() and volume to make sure they aren't being sprayed in trace amounts (1 unit != insta rape) -- Doohl
			else
				if(!H.unacidable)
					H.take_organ_damage(0, volume * 2)
...
/datum/reagent/serotrotium/on_mob_life(var/mob/living/M as mob)
	...
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
		if(prob(7))
			H.emote(pick("twitch", "drool", "moan", "gasp"))
		H.druggy = max(H.druggy, 50)
	...
	if(method == TOUCH)
		if(ishuman(M))
			...
			if(MK.wear_mask)
				...
				return
			if(!MK.unacidable)
				MK.take_organ_damage(0, volume * 2)

	else if(!M.unacidable)
		if(ishuman(M))
			var/mob/living/carbon/human/H = M
			if(H.species.name == "Grey")
				return //Greys lurve dem some sacid
			if(prob(15) && volume >= 30)
				...
				M.take_organ_damage(0, volume * 2)
...
/datum/reagent/sacid/reaction_obj(var/obj/O, var/volume)
	...
	if((istype(O, /obj/item) || istype(O, /obj/effect/glowshroom)) && prob(10))
		if(!O.unacidable)
			...
			I.desc = "Looks like this was \a [O] some time ago."
...
/datum/reagent/pacid/reaction_mob(var/mob/living/M, var/method = TOUCH, var/volume)
	...
	if(method == TOUCH)
		if(ishuman(M))
			...
			if(!H.unacidable)
				var/datum/organ/external/affecting = H.get_organ("head")
				if(affecting.take_damage(0, volume * 4))
					H.UpdateDamageIcon(1)
				H.emote("scream", , , 1)
			...
			if(!MK.unacidable)
				MK.take_organ_damage(0, volume * 4) //Same deal as sulphuric acid
	else
		if(!M.unacidable)
			if(ishuman(M))
				var/mob/living/carbon/human/H = M
				...
				M.take_organ_damage(0, volume * 4)
...
/datum/reagent/glycerol/on_mob_life(var/mob/living/M as mob)
	..()
...
/datum/reagent/radium/reaction_turf(var/turf/simulated/T, var/volume)
	..()
	if(volume >= 3)
		if(istype(T, /turf/simulated) && !(locate(/obj/effect/decal/cleanable/greenglow) in T))
...
/datum/reagent/toxin/plantbgone/reaction_mob(var/mob/living/M, var/method = TOUCH, var/volume)
	...
	if(iscarbon(M))
		...
		if(!C.wear_mask) //If not wearing a mask
			C.adjustToxLoss(volume) //4 toxic damage per application, doubled for some reason
		if(ishuman(M))
			var/mob/living/carbon/human/H = M
			if(H.dna)
				if(H.species.flags & IS_PLANT) //Plantmen take a LOT of damage
					H.adjustToxLoss(10*volume)
...
//Copypasta of chloral hydrate, disguised as normal beer for use by emagged brobots
/datum/reagent/beer2
	name = "Beer"
	id = "beer2"
...
/////////////////////////Food Reagents////////////////////////////
//Part of the food code. Nutriment is used instead of the old "heal_amt" code. Also is where all the food, condiments, additives, and such go.
...
/datum/reagent/nutriment/on_mob_life(var/mob/living/M as mob)
	...
	M.nutrition += REM*nutriment_factor	//For hunger and fatness
/*
	//If overeaten - vomit and fall down
	//Makes you feel bad but removes reagents and some effect from your body
	if (M.nutrition > 650)
...
/datum/reagent/soysauce/on_mob_life(var/mob/living/M as mob)
	..()
	M.nutrition += REM*nutriment_factor
...
/datum/reagent/ketchup/on_mob_life(var/mob/living/M as mob)
	..()
	M.nutrition += REM*nutriment_factor
	...
	if(prob(5)) //5% chance of stinking per life()
		for(var/mob/living/carbon/C in oview(stench_radius, M)) //All other carbons in 4 tile radius (excluding our mob)
			...
			if(istype(C.wear_mask))
				...
				if(c_mask.body_parts_covered & MOUTH) //If the carbon's mouth is covered, let's assume they don't smell it
					continue
...
/datum/reagent/cinnamon/on_mob_life(var/mob/living/M as mob)
	..()
	M.nutrition += REM*nutriment_factor
...
/datum/reagent/sprinkles/on_mob_life(var/mob/living/M as mob)
	...
	if(istype(M, /mob/living/carbon/human) && M.job in list("Security Officer", "Head of Security", "Detective", "Warden"))
		M.heal_organ_damage(1, 1)
		M.nutrition += REM*nutriment_factor //Double nutrition
...
//Really ? You realize corn oil is highly flammable right ?
/*
...
/datum/reagent/drink/coffee/on_mob_life(var/mob/living/M as mob)
	...
	if(adj_temp > 0 && holder.has_reagent("frostoil"))
		holder.remove_reagent("frostoil", 100*REAGENTS_METABOLISM)
...
/datum/reagent/ethanol/absinthe/on_mob_life(var/mob/living/M as mob)
	..()
	data++
	M.hallucination += 5
	if(volume > REAGENTS_OVERDOSE)
...
/datum/reagent/hamserum/reaction_mob(var/mob/living/M, var/method=INGEST, var/volume)
	..()
	empulse(get_turf(), 1, 2, 0)
