...
/datum/reagent/honey/on_mob_life(var/mob/living/M)
	if(ishuman(M))
		...
		if(!holder)
			return
		if(H.getBruteLoss(FALSE,FALSE) && prob(60))
			H.heal_organ_damage(quality, 0)
		if(H.getFireLoss(FALSE,FALSE) && prob(50))
...
/datum/reagent/diamond/on_mob_life(var/mob/living/M)
	...
	if(isgolem(M)) //golems metabolize the diamond into very expensive doctor's delight
		if(M.getOxyLoss())
			M.adjustOxyLoss(-2)
		if(M.getBruteLoss(FALSE,FALSE))
			M.heal_organ_damage(2, 0)
		if(M.getFireLoss(FALSE,FALSE))
...
/datum/reagent/polypgelatin/on_mob_life(var/mob/living/M)
	...
	if(M.getFireLoss(FALSE,FALSE) && prob(20))
    ...
    if(ishuman(M))
        ...
        if(H.mind.miming)
            if(M.getOxyLoss() && prob(80))
                M.adjustOxyLoss(-REM)
            if(M.getBruteLoss(FALSE,FALSE) && prob(80))
                M.heal_organ_damage(REM, 0)
            if(M.getFireLoss(FALSE,FALSE) && prob(80))
                M.heal_organ_damage(0, REM)
	...
	if(ishuman(M))
		...
		if(H.mind.GetRole(NINJA))
			...
			if(M.getOxyLoss() && prob(50))
				M.adjustOxyLoss(-2)
			if(M.getBruteLoss(FALSE,FALSE) && prob(60))
				M.heal_organ_damage(2, 0)
			if(M.getFireLoss(FALSE,FALSE) && prob(50))
				M.heal_organ_damage(0, 2)
...
/datum/reagent/drink/coffee/medcoffee/on_mob_life(var/mob/living/M)
	...
	if(M.getBruteLoss() && prob(30))
		M.heal_organ_damage(1, 0)
	if(M.getFireLoss(FALSE,FALSE) && prob(25))
		M.heal_organ_damage(0, 1)
