...
/obj/item/weapon/gun/dartgun/examine()
	...
	if (!(usr in view(2)) && usr!=src.loc)
		return
	if (beakers.len)
		usr << "<span class=\"notice\">[src] contains:</span>"
...
/obj/item/weapon/gun/dartgun/attackby(obj/item/I as obj, mob/user as mob)
	...
	if(istype(I, /obj/item/weapon/reagent_containers/glass))
		...
		if(beakers.len >= max_beakers)
			user << "<span class=\"notice\">[src] already has [max_beakers] vials in it - another one isn't going to fit!</span>"
			return
...
/obj/item/weapon/gun/dartgun/proc/fire_dart(atom/target, mob/user)
	if (locate (/obj/structure/table, src.loc))
		...
		if(!S)
			user << "<span class=\"rose\">There are no darts in [src]!</span>"
			return
		if(!S.reagents)
			user << "<span class=\"rose\">There are no reagents available!</span>"
			return
		cartridge.darts--
		src.update_icon()
