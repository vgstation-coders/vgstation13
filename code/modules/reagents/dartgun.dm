...
/obj/item/gun/dartgun/isHandgun()
	return TRUE
...
/obj/item/gun/dartgun/New()
	...
	if(starting_chems)
		for(var/chem in starting_chems)
			var/obj/item/reagent_containers/glass/beaker/B = new(src)
...
/obj/item/gun/dartgun/canbe_fired()
	if(!cartridge)
		return 0
	else
		return cartridge.darts

/obj/item/gun/dartgun/can_discharge()
	return canbe_fired()
...
/obj/item/gun/dartgun/process_chambered()
	if(canbe_fired())
		if(!in_chamber)
...
/obj/item/gun/dartgun/afterattack(atom/A, mob/living/user, flag, params, struggle = 0)
	if(target == user)
		return
	..()
...
/obj/item/gun/dartgun/updateUsrDialog()
	if(in_use)
		var/is_in_use = 0
...
/obj/item/gun/dartgun/Fire(atom/target, mob/living/user, params, reflex = 0, struggle = 0, var/use_shooter_turf = FALSE)
	if(cartridge)
		..()
	else
