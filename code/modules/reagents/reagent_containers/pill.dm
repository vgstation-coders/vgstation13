...
/obj/item/reagent_containers/pill/afterattack(atom/target, var/mob/user, var/adjacency_flag, var/click_params)
	var/static/list/allowed_targets = list(/obj/item/reagent_containers, /obj/structure/reagent_dispensers/cauldron)
...
/obj/item/reagent_containers/pill/proc/try_feed(mob/target, mob/user)
...
/obj/item/reagent_containers/pill/bite_act(mob/user)
	try_feed(user, user)

//OOP, HO!
/obj/item/reagent_containers/pill/proc/ingest(mob/M as mob)
	if(!reagents)
		return
	if(!M)
		return
...
/obj/item/reagent_containers/pill/fits_in_iv_drip()
	return 1

/obj/item/reagent_containers/pill/should_qdel_if_empty() //If you remove the reagents from this thing via smoke or IV drip or something, it shouldn't like it.
...
/obj/item/reagent_containers/pill/hyperzine
	name = "Hyperzine pill"
	desc = "Gotta go fast!"

	icon_state = "pill7" //grey oblong
/obj/item/reagent_containers/pill/hyperzine/New()
	..()
	reagents.add_reagent(HYPERZINE, 10)

/obj/item/reagent_containers/pill/creatine_safe
	name = "Creatine Pill (26 units)"
	desc = "Become the boss of this Gym."
	icon_state = "pill5" //bright red oblong with stripe

/obj/item/reagent_containers/pill/creatine_safe/New()
	..()
	reagents.add_reagent(CREATINE, 26)

/obj/item/reagent_containers/pill/creatine_supplement
	name = "Creatine Supplement (5 units)"
	desc = "Maintain those massive gains!"
	icon_state = "pill6" //cyan/brown oblong

/obj/item/reagent_containers/pill/creatine_supplement/New()
	..()
	reagents.add_reagent(CREATINE, 5)


/obj/item/storage/pill_bottle/time_release
...
/obj/item/storage/pill_bottle/time_release/New()
	..()
	for(var/i=1 to 7)
		new /obj/item/reagent_containers/pill/time_release(src)
...
/obj/item/reagent_containers/pill/time_release/ingest(mob/M as mob)
	if(!reagents)
		return
	if(!M)
		return
...
/obj/item/storage/pill_bottle/random/New()
	..()
	for(var/i=1 to 14)
...
/obj/item/reagent_containers/pill/random/maintenance/New()
	. = ..()
	name = "\improper [pick( \
...
/obj/item/reagent_containers/pill/nanobot
	name = "nanobot pill"
	desc = "Experimental medication."
	icon_state = "pill7" //grey oblong

/obj/item/reagent_containers/pill/nanobot/New()
	..()
