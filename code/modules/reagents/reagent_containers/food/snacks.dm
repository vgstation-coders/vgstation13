...
/obj/item/weapon/reagent_containers/food/snacks/pickup(mob/user)
	..()
	update_icon()
	var/paintolerance = painful_to_hold(user)
	if(paintolerance)
		spawn(5/paintolerance)
			paintolerance = painful_to_hold(user)
			if(paintolerance && ishuman(user))
				var/mob/living/carbon/human/H = user
				var/hand_index = user.held_items.Find(src)
				if(hand_index)
					switch(hand_index)
						if (GRASP_RIGHT_HAND)
							H.apply_damage(5*paintolerance, BURN, LIMB_RIGHT_HAND)
						if (GRASP_LEFT_HAND)
							H.apply_damage(5*paintolerance, BURN, LIMB_LEFT_HAND)
					if(H.feels_pain())
						to_chat(user,"<span class='danger'>The heat from [src] scalds your hand in pain, causing you to drop it!</span>")
						user.drop_item(src)

/obj/item/weapon/reagent_containers/food/snacks/proc/painful_to_hold(mob/user)
	if(reagents?.chem_temp >= STEAMTEMP) // if item is too hot to pick up
		var/obj/item/clothing/gloves/G = user.get_item_by_slot(slot_gloves)
		if(!G) // and user is not wearing gloves
			return 1
		if(G.heat_conductivity > INS_GLOVES_HEAT_CONDUCTIVITY) // or they don't insulate heat enough (ovenmitts, captain, black or yellow)
			return G.heat_conductivity
	return 0
