...
/proc/get_special_fruits(var/filter=HYDRO_PREHISTORIC|HYDRO_VOX)
	. = list()
	. += /obj/item/reagent_containers/food/snacks/grown
	for(var/T in existing_typesof(/obj/item/reagent_containers/food/snacks/grown))
		var/obj/item/reagent_containers/food/snacks/grown/G = T
		if(initial(G.hydroflags) & filter)
			. += T

/obj/item/reagent_containers/food/snacks/grown/New(atom/loc, custom_plantname)
	..()
	if(custom_plantname)
...
/obj/item/reagent_containers/food/snacks/grown/proc/do_splat_effects(atom/hit_atom)
	if(seed.teleporting)
...
/obj/item/reagent_containers/food/snacks/grown/attack(mob/living/M, mob/user, def_zone)
...
/obj/item/reagent_containers/food/snacks/grown/Crossed(var/mob/living/carbon/M)
	..()
	if(!seed)
		return
	if(!istype(M))
...
/obj/item/reagent_containers/food/snacks/grown/after_consume(var/mob/living/carbon/human/H)
...
/obj/item/reagent_containers/food/snacks/grown/proc/splat_decal(turf/T)
...
/obj/item/reagent_containers/food/snacks/grown/proc/thorns_apply_damage(mob/living/carbon/human/H, datum/organ/external/affecting)
...
/obj/item/reagent_containers/food/snacks/grown/proc/stinging_apply_reagents(mob/living/carbon/human/H)
	if(!seed.stinging)
		return 0
...
/obj/item/reagent_containers/food/snacks/grown/proc/do_fruit_teleport(atom/hit_atom, mob/M, var/potency)	//Does this need logging?
...
/obj/item/reagent_containers/food/snacks/grown/cinnamon
...
/obj/item/reagent_containers/food/snacks/grown/rocknut
	name = "rocknut"
...
/obj/item/reagent_containers/food/snacks/grown/rocknut/New(atom/loc, custom_plantname)
	..()
...
/obj/item/reagent_containers/food/snacks/grown/rocknut/before_consume(mob/living/carbon/eater)
...
/obj/item/reagent_containers/food/snacks/grown/ambrosiavulgaris/cruciatus
	name = "ambrosia vulgaris branch"
	desc = "This is a plant containing various healing chemicals."
	potency = 10
	plantname = "ambrosiacruciatus"

/obj/item/reagent_containers/food/snacks/grown/attackby(var/obj/item/O as obj, var/mob/user as mob)
...
/obj/item/reagent_containers/food/snacks/grown/ambrosiavulgaris/deus/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(istype(O, /obj/item/paper))
		qdel(O)
...
/obj/item/reagent_containers/food/snacks/grown/banana/isHandgun()
	return TRUE

/obj/item/reagent_containers/food/snacks/grown/bluespacebanana
	name = "bluespace banana"
	desc = "It's an excellent prop for a comedy."
	filling_color = "#FCF695"
	plantname = "bluespacebanana"

/obj/item/reagent_containers/food/snacks/grown/bluespacebanana/isHandgun()
	return TRUE

/obj/item/reagent_containers/food/snacks/grown/bluespacebanana/after_consume(var/mob/user, var/datum/reagents/reagentreference)
	var/obj/item/bananapeel/bluespace/peel = new
...
/obj/item/reagent_containers/food/snacks/grown/bluespacetomato/testing
	potency = 100
...
/obj/item/reagent_containers/food/snacks/grown/ghostpepper/spook()
...
/obj/item/reagent_containers/food/snacks/grown/grass/attack_self(mob/user as mob)
...
/obj/item/reagent_containers/food/snacks/grown/vaporsac/attack(mob/living/M, mob/user, def_zone, eat_override = 0)
	pop(user)

/obj/item/reagent_containers/food/snacks/grown/vaporsac/attack_animal(mob/M)
	pop(M)

/obj/item/reagent_containers/food/snacks/grown/vaporsac/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/device/analyzer/plant_analyzer))
		return
	pop(user)

/obj/item/reagent_containers/food/snacks/grown/vaporsac/proc/pop(mob/popper)
	if(popper)
...
/obj/item/reagent_containers/food/snacks/grown/nofruit/New(atom/loc, custom_plantname)
	..()
	available_fruits = existing_typesof(/obj/item/reagent_containers/food/snacks/grown) - get_special_fruits()
	available_fruits = shuffle(available_fruits)

/obj/item/reagent_containers/food/snacks/grown/nofruit/verb/pick_leaf()
...
/obj/item/reagent_containers/food/snacks/grown/nofruit/AltClick(mob/user)
	pick_leaf()

/obj/item/reagent_containers/food/snacks/grown/nofruit/attackby(obj/item/W, mob/user)
	pick_leaf()

/obj/item/reagent_containers/food/snacks/grown/nofruit/proc/randomize()
	switching = 1
	mouse_opacity = 2
	spawn()
		while(switching)
			current_path = available_fruits[counter]
			var/obj/item/reagent_containers/food/snacks/grown/G = current_path
...
/obj/item/reagent_containers/food/snacks/grown/nofruit/proc/getnofruit(mob/user, obj/item/W)
	if(!switching || !current_path)
		return
	verbs -= /obj/item/reagent_containers/food/snacks/grown/nofruit/verb/pick_leaf
	switching = 0
	var/N = rand(1,3)
	if(get_turf(user))
		switch(N)
...
/obj/item/reagent_containers/food/snacks/grown/avocado/can_consume(mob/living/carbon/eater, mob/user)
	if(cant_eat_msg)
		to_chat(user, "<span class='notice'>This [name][cant_eat_msg]</span>")
	else
		return ..()

/obj/item/reagent_containers/food/snacks/grown/avocado/attackby(obj/item/W, mob/user)
	..()
	if(W.sharpness_flags & SHARP_BLADE)
		if(cut && cant_eat_msg)
			...
			if(loc == user)
				if(src in user.held_items)
					user.drop_item(src, force_drop = 1)
					var/obj/item/reagent_containers/food/snacks/grown/avocado/cut/pitted/P = new(get_turf(src))
					user.put_in_hands(P)
					qdel(src)
					return
			new /obj/item/reagent_containers/food/snacks/grown/avocado/cut/pitted(get_turf(src))
			qdel(src)
		else if(!cut)
			user.visible_message("\The [user] slices \the [src] in half with \the [W].","You slice \the [src] in half with \the [W].")
			var/list/halves = list(new /obj/item/reagent_containers/food/snacks/grown/avocado/cut(get_turf(src)), new /obj/item/reagent_containers/food/snacks/grown/avocado/cut/pitted(get_turf(src)))
			if(loc == user)
				if(src in user.held_items)
					user.drop_item(src, force_drop = 1)
...
/obj/item/reagent_containers/food/snacks/grown/avocado/cut
	name = "avocado half"
...
/obj/item/reagent_containers/food/snacks/grown/silverpear
	name = "silver pear"
...
/obj/item/reagent_containers/food/snacks/grown/mustardplant
