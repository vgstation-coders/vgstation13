...
/obj/item/weapon/reagent_containers/syringe
	...
	afterattack(obj/target, mob/user , flag)
		...
		if(mode == SYRINGE_BROKEN)
			user << "<span class=\"rose\">This syringe is broken!</span>"
			return
		...
		switch(mode)
			if(SYRINGE_DRAW)
				...
				if(reagents.total_volume >= reagents.maximum_volume)
					user << "<span class=\"rose\">The syringe is full.</span>"
					return

				if(ismob(target))//Blood!
					if(istype(target, /mob/living/carbon/slime))
						user << "<span class=\"rose\">You are unable to locate any blood.</span>"
						return
					if(src.reagents.has_reagent("blood"))
						user << "<span class=\"rose\">There is already a blood sample in this syringe</span>"
						return
					if(istype(target, /mob/living/carbon))//maybe just add a blood reagent to all mobs. Then you can suck them dry...With hundreds of syringes. Jolly good idea.
						...
						if (B)
							...
							for(var/mob/O in viewers(4, user))
								O.show_message("<span class=\"rose\">[user] takes a blood sample from [target].</span>", 1)
						else
							user.visible_message("<span class=\"rose\">[user] inserts the syringe into [target], draws back the plunger and gets... Nothing?</span>",\
								"<span class=\"rose\">You insert the syringe into [target], draw back the plunger and get... Nothing?</span>")
					...
					if(!target.is_open_container() && !istype(target,/obj/structure/reagent_dispensers) && !istype(target,/obj/item/slime_extract))
						user << "<span class=\"rose\">You cannot directly remove reagents from this object.</span>"
						return
			...
			if(SYRINGE_INJECT)
				...
				if(ismob(target) && target != user)
					...
					for(var/mob/O in viewers(world.view, user))
						O.show_message(text("<span class=\"rose\">[] injects [] with the syringe!</span>", user, target), 1)
		...
		switch(mode)
			...
			if(SYRINGE_INJECT)
				...
				if(ismob(target) && target != user)
					...
					for(var/mob/O in viewers(world.view, user))
						O.show_message(text("<span class=\"rose\">[] injects [] with a giant syringe!</span>", user, target), 1)
					src.reagents.reaction(target, INGEST)
				if(ismob(target) && target == user)
					src.reagents.reaction(target, INGEST)
				spawn(5)
