/obj/item/reagent_containers/glass/mortar
	...
	var/list/blend_items = list (
		...
		/obj/item/grown/nettle         = list(FORMIC_ACID,10),
		/obj/item/grown/deathnettle    = list(PHENOL,10),
		/obj/item/stack/sheet/charcoal        = list("charcoal",20),
		/obj/item/reagent_containers/food/snacks/grown/soybeans   = list(SOYMILK,1),
		/obj/item/reagent_containers/food/snacks/grown/tomato     = list(KETCHUP,2),
		/obj/item/reagent_containers/food/snacks/grown/corn       = list(CORNOIL,3),
		/obj/item/reagent_containers/food/snacks/grown/wheat      = list(FLOUR,5),
		/obj/item/reagent_containers/food/snacks/grown/ricestalk  = list(RICE,5),
		/obj/item/reagent_containers/food/snacks/grown/cherries   = list(CHERRYJELLY,1),
		/obj/item/reagent_containers/food/drinks/soda_cans        = list(ALUMINUM,10),
		...
		/obj/item/cell                 = list(LITHIUM, 10),
		/obj/item/clothing/head/butt          = list(MERCURY, 10),
		/obj/item/rocksliver           = list(GROUND_ROCK,30),
		/obj/item/match                = list(PHOSPHORUS, 2),

		//Recipes must include both variables!
		/obj/item/reagent_containers/food = list("generic",0),
...
/obj/item/reagent_containers/glass/mortar/attackby(var/obj/item/O as obj, var/mob/user as mob)
...
/obj/item/reagent_containers/glass/mortar/attack_hand(mob/user as mob)
...
/obj/item/reagent_containers/glass/mortar/attack_self(mob/user as mob)
	if(!crushable)
	...
	if(is_type_in_list(crushable, juice_items))
		...
		if(!id)
			return
		var/obj/item/reagent_containers/food/snacks/grown/juiceable = crushable
		...
		if(!id)
			return
		if(istype(crushable, /obj/item/reagent_containers/food/snacks)) //Most growable food
			if(id[1] == "generic")
				crushable.reagents.trans_to(src,crushable.reagents.total_volume)
			else
				reagents.add_reagent(id[1],min(id[2], space))
		else if(istype(crushable, /obj/item/stack/sheet) || istype(crushable, /obj/item/seeds) || /obj/item/device/flashlight/flare || /obj/item/stack/cable_coil || /obj/item/cell || /obj/item/clothing/head/butt) //Generic processes
			reagents.add_reagent(id[1],min(id[2], space))
		else if(istype(crushable, /obj/item/grown)) //Nettle and death nettle
			crushable.reagents.trans_to(src,crushable.reagents.total_volume)
		else if(istype(crushable, /obj/item/rocksliver)) //Xenoarch
			var/obj/item/rocksliver/R = crushable
