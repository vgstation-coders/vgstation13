...
/obj/item/gun/syringe/isHandgun()
	return FALSE
...
/obj/item/gun/syringe/afterattack(atom/A, mob/living/user, flag, params, struggle = 0)
	if(target == user)
		return
	..()

/obj/item/gun/syringe/canbe_fired()
	return syringes.len

/obj/item/gun/syringe/can_discharge()
	return canbe_fired()
...
/obj/item/gun/syringe/process_chambered()
	if(canbe_fired())
		if(!in_chamber)
			var/S = syringes[1]
...
/obj/item/gun/syringe/Fire(atom/target, mob/living/user, params, reflex = 0, struggle = 0, var/use_shooter_turf = FALSE)
