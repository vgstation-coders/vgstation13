...
datum/admins/proc/DB_ban_unban(var/ckey, var/bantype, var/job = "")
	...
	if(ban_number == 0)
		usr << "<span class=\"rose\">Database update failed due to no bans fitting the search criteria. If this is not a legacy ban you should contact the database admin.</span>"
		return

	if(ban_number > 1)
		usr << "<span class=\"rose\">Database update failed due to multiple bans fitting the search criteria. Note down the ckey, job and current time and contact the database admin.</span>"
		return

	if(istext(ban_id))
		ban_id = text2num(ban_id)
	if(!isnum(ban_id))
		usr << "<span class=\"notice\">Database update failed due to a ban ID mismatch. Contact the database admin.</span>"
		return
...
datum/admins/proc/DB_ban_unban_by_id(var/id)
	...
	if(ban_number == 0)
		usr << "<span class=\"rose\">Database update failed due to a ban id not being present in the database.</span>"
		return

	if(ban_number > 1)
		usr << "<span class=\"rose\">Database update failed due to multiple bans having the same ID. Contact the database admin.</span>"
		return
...
/datum/admins/proc/DB_ban_panel(var/playerckey = null, var/adminckey = null)
	...
	establish_db_connection()
	if(!dbcon.IsConnected())
		usr << "<span class=\"rose\">Failed to establish database connection</span>"
		return
