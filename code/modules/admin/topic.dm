...
/*				if("shockwave")
			...
			if("immovablebig")
				...
				message_admins("[key_name_admin(usr)] has sent an immovable big rod to the station.", 1)
				immovablerod(1)
			if("immovablehyper")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","IRH")
				message_admins("[key_name_admin(usr)] has sent an immovable hyper rod to the station. That one's gonna hurt.", 1)
				immovablerod(2)
			if("prison_break")
				feedback_inc("admin_secrets_fun_used",1)
