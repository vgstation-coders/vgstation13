...
//				message_admins("[key_name_admin(usr)] has triggered a battle to the death (only one)")
			...
			if("nuclearblowout")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","NB")
				var/choice = input("You sure you want to cause a blowout? Misuse of this could result in removal of flags or halarity.") in list("CHEEKI BREEKI IV DAMKE", "Cancel")
				if(choice == "CHEEKI BREEKI IV DAMKE")
					message_admins("[key_name_admin(usr)] has caused a nuclear blowout, fellow spessman.")
			...
			if("fakealerts")
				...
				if(choice == "Malfunction") //BLOW EVERYTHING
					...
					to_chat(world, sound('sound/AI/aimalf.ogg'))//AI got valid
