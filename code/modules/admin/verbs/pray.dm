...
/mob/verb/pray(msg as text)
	...
	send_prayer_to_admins(msg, 'sound/effects/prayer.ogg', "Prayer", key_name(usr), get_turf(usr))
...
/proc/Centcomm_announce(var/text , var/mob/Sender)
	...
	send_prayer_to_admins(msg, 'sound/effects/msn.ogg', "Centcomm", key_name(Sender, 1), get_turf(Sender))
...
/proc/send_prayer_to_admins(var/msg,var/sound,var/type,var/key,var/jumploc)
	...
	if(admin_number_present <= 0)
		if(!admin_number_afk)
			...
			send2admindiscord("**[type]**: [key]: `[replacetext(msg, "`", "\\`")]` - **No admins online**", TRUE)

		else
			send2adminirc("[uppertext(type)] [key]: [msg] - All admins AFK ([admin_number_afk])")
			send2admindiscord("**[type]**: [key]: `[replacetext(msg, "`", "\\`")]` - **All admins AFK** ([admin_number_afk])", TRUE)

	else
		send2adminirc("[uppertext(type)] [key]: [msg]")
		send2admindiscord("**[type]**: [key]: `[replacetext(msg, "`", "\\`")]` - **[admin_number_present]** Active admins, **[admin_number_afk]** AFK admins.")
	
	log_admin("[uppertext(type)]: [key] at [formatJumpTo(jumploc)]: [msg]")
