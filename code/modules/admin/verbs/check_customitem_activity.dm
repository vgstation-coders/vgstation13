...
/proc/populate_inactive_customitems_list(var/client/C)
	...
	if(ckeys_with_customitems.len)
		var/datum/DBQuery/query_inactive = SSdatabase.NewQuery("SELECT ckey, lastseen FROM erro_player WHERE datediff(Now(), lastseen) > 60")
	...
	if(ckeys_with_customitems.len)
		for(var/cur_ckey in ckeys_with_customitems)
			var/datum/DBQuery/query_inactive = SSdatabase.NewQuery("SELECT ckey FROM erro_player WHERE ckey = '[cur_ckey]'")
			query_inactive.Execute()
			if(!length(query_inactive.query.CurrentRow()))
