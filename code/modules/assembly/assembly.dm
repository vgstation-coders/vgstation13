...
//Called when this assembly is pulsed by another one
/obj/item/device/assembly/Activate()
	if(cooldown > 0)
		return 0
	cooldown = 2
...
//Call this via spawn(10) to have it count down the cooldown var
/obj/item/device/assembly/Process_cooldown()
	cooldown--
	if(cooldown <= 0)
		return 0
...
//Called when an assembly holder attempts to attach, sets src's loc in here
/obj/item/device/assembly/Attach_Holder(var/obj/H, var/mob/user)
	if(!H)
		return 0
