...
/obj/item/sword/weaponcraft
	var/obj/item/reagent_containers/hypospray/hypo = null

/obj/item/sword/suicide_act(mob/user)
...
/obj/item/sword/weaponcraft/attack_self(mob/user as mob)
	if(!hypo)
		return
...
/obj/item/sword/weaponcraft/update_icon()
	overlays.len = 0
	if(hypo)
		var/image/hypo_icon = image('icons/obj/weaponsmithing.dmi', src, "sword_hypo_overlay")
		overlays += hypo_icon

/obj/item/sword/weaponcraft/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/metal_blade))
		to_chat(user, "You attach \the [W] to \the [src].")
		if(src.loc == user)
			user.drop_item(src, force_drop = 1)
			var/obj/item/sword/executioner/I = new (get_turf(user))
			user.put_in_hands(I)
		else
			new /obj/item/sword/executioner(get_turf(src.loc))
		qdel(src)
		qdel(W)
	if(W.type == /obj/item/reagent_containers/hypospray || W.type == /obj/item/reagent_containers/hypospray/creatine)
		to_chat(user, "You wrap \the [src]'s grip around \the [W], affixing it to the base of \the [src].")
		user.drop_item(W, force_drop = 1)
		hypo = W
		W.forceMove(src)
		update_icon()
	if(hypo && istype(W, /obj/item/aluminum_cylinder))
		to_chat(user, "You affix \the [W] to the bottom of \the [src]'s [hypo.name].")
		if(src.loc == user)
			user.drop_item(src, force_drop = 1)
			var/obj/item/sword/venom/I = new (get_turf(user))
			...
			new /obj/item/sword/venom(get_turf(src.loc))
		qdel(src)
		qdel(W)

/obj/item/sword/weaponcraft/Destroy()
	if(hypo)
		qdel(hypo)
		hypo = null
	..()
...
/obj/item/sword/venom
	...
	var/beaker = null
	var/obj/item/reagent_containers/hypospray/HY = null
...
/obj/item/sword/venom/verb/set_inject_amount()
...
/obj/item/sword/venom/examine(mob/user)
	..()
	if(beaker)
...
/obj/item/sword/venom/proc/update_color()
...
/obj/item/sword/venom/attack_self(mob/user as mob)
	if(!beaker)
		return
	else
		var/obj/item/reagent_containers/glass/beaker/B = beaker
...
/obj/item/sword/venom/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/reagent_containers/glass/beaker))
		if(beaker)
			...
			return
		var/obj/item/reagent_containers/glass/beaker/B = W
	...
	if(iscrowbar(W))
		...
		var/obj/item/sword/weaponcraft/I = new (get_turf(src))
		if(HY)
			I.hypo = HY
			HY.forceMove(I)
			HY = null
		else
			var/obj/item/reagent_containers/hypospray/H = new (I)
		...
		if(src.loc == user)
			user.drop_item(src, force_drop = 1)
			user.put_in_hands(I)
		new /obj/item/aluminum_cylinder(get_turf(src))
		qdel(src)

/obj/item/sword/venom/attack(mob/M as mob, mob/user as mob)
...
/obj/item/sword/executioner //takes a long time to swing and doesn't do much more damage than the sword, however it has a much higher chance to sever a limb
...
/obj/item/sword/executioner/afterattack(null, mob/living/user as mob|obj, null, null, null)
	if(complete)
		user.delayNextAttack(30) //thrice the regular attack delay

/obj/item/sword/executioner/attack_self(mob/user as mob)
	if(complete)
		return
	to_chat(user, "You detach the metal blade from \the [src].")
	new /obj/item/metal_blade(get_turf(user.loc))
	new /obj/item/sword(get_turf(user.loc))
	qdel(src)

/obj/item/sword/executioner/attackby(obj/item/W, mob/user)
