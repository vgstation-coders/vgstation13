...
/obj/item/gun/banannon/New()
	..()
	chamber_if_possible()

/obj/item/gun/banannon/update_icon()
...
/obj/item/gun/banannon/examine(mob/user)
	..()
	to_chat(user, "<span class='info'>It has [current_ammo] round\s remaining.</span>")

/obj/item/gun/banannon/proc/chamber_if_possible()
	if(current_ammo > 0)
		if(!process_chambered())
			in_chamber = new /obj/item/projectile/bullet/sabonana(src)

/obj/item/gun/banannon/afterattack(atom/A, mob/living/user, flag, params, struggle = 0)
	if(flag)
...
/obj/item/gun/banannon/Fire(atom/target, mob/living/user, params, reflex = 0, struggle = 0, var/use_shooter_turf = FALSE)
...
/obj/item/gun/banannon/process_chambered()
	return in_chamber

/obj/item/gun/banannon/attackby(obj/item/W, mob/user)
	..()
...
/obj/item/gun/banannon/can_discharge()
	if(current_ammo)
		return 1
