...
/obj/item/weapon/gun/proc/TargetActed(var/mob/living/T)
	...
	if(can_fire())
		...
		if(firing_check > 0)
			...
			M << "<span class=\"rose\">They can't be hit from here!</span>"
			told_cant_shoot = 1
			spawn(30)
				told_cant_shoot = 0
	else
...
mob/living/proc/Targeted(var/obj/item/weapon/gun/I) //Self explanitory.
	...
	drag and drop items in or into the map, speak, and click on interface buttons. Clicking on the map objects (floors and walls are fine), their items \
	 (other than a weapon to de-target), or moving will result in being fired upon. <span class=\"rose\"> The aggressor may also fire manually, \
	...
	if(T)
		...
		if(m_intent == "run" && T.client.target_can_move == 1 && T.client.target_can_run == 0 && (ishuman(T)))
			src << "<span class=\"rose\">Your move intent is now set to walk, as your targeter permits it.</span>"  //Self explanitory.
			set_m_intent("walk")
			...
			for(var/mob/living/M in G.target)
				if(target_can_move)
					...
					if(!target_can_run && (ishuman(M)))
						M << "<span class=\"rose\">Your move intent is now set to walk, as your targeter permits it.</span>"
						M.set_m_intent("walk")
				else
