...
/obj/machinery/account_database/attack_hand(mob/user as mob)
	...
	if(.)
		return
	var/datum/role/malfAI/MR = user.mind?.GetRole(MALF)
	if(isAdminGhost(user) || malf_owner == MR || (ishuman(user) && !user.stat && get_dist(src,user) <= 1))
		...
		if(access_level > 0 || isAdminGhost(user) || malf_owner == MR)
