...
/obj/machinery/prism/beam_connect(var/obj/effect/beam/emitter/B)
	if(istype(B))
		...
		..()
		B.lazy_register_event(/lazy_event/beam_power_change, src, .proc/on_power_change)
		update_beams(B)
...
/obj/machinery/prism/beam_disconnect(var/obj/effect/beam/emitter/B)
	if(istype(B))
		if(B.HasSource(src))
			return // Prevent infinite loops.
		..()
		B.lazy_unregister_event(/lazy_event/beam_power_change, src, .proc/on_power_change)
		update_beams(B)
