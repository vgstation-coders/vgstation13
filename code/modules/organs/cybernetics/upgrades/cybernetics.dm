...
/obj/item/cybernetics/afterattack(var/obj/item/O)
	if(!Adjacent(usr))
		return
	...
	if(istype(O, /obj/item/organ))
		...
		if(!(I.robotic == 2))
			...
			return
		to_chat(usr, "DEBUG - Succesfull application of [src] to [I].")
		apply(I)
		...
		if(is_type_in_list(E, Coveredlimbs))
			to_chat(usr, "DEBUG - Succesfull application of [src] to [E].")
			apply(E)
		else
			to_chat(usr, "\the [E] has no current surgical applications for carbon reconstruction, so is not supported for cybernetic enhancement.")
			return
	else
		to_chat(usr, "\the [src] won't work on \the [O].")
		return
		//Remember to delete this little chunk, right now it's just used for debug

/obj/item/cybernetics/proc/apply(var/obj/item/O)
	to_chat(usr, "Succesfully applied \the [src].")
	//Nothing to see here