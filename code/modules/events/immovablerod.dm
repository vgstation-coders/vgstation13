...
//One of the turfs in the 60x60 square in the center of the zlevel
	...
	new /obj/item/projectile/immovablerod(locate(startx, starty, 1), locate(endx, endy, 1), hyperRod)
...
/obj/item/projectile/immovablerod/New(atom/start, atom/end, hyperRod = 0)
	..()
	fuckMyShitUp = hyperRod
...
/obj/item/projectile/immovablerod/throw_at(atom/end)
	for(var/mob/dead/observer/people in observers)
		to_chat(people, "<span class = 'notice'>Immovable [fuckMyShitUp ? "hyper " : ""]rod has been thrown at the station, <a href='?src=\ref[people];follow=\ref[src]'>Follow it</a></span>")
	original = end
	starting = loc
	current = loc
	OnFired()
