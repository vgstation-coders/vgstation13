...
/obj/item/clothing/suit/space/space_adv/proc/depl() //Body cheking
	...
	if(istype(usr:wear_suit, /obj/item/clothing/suit/space/space_adv))
		if(!istype(usr:head, /obj/item/clothing/head))                                      //Head cheking


			usr << "\green <B>Starting</B>\n"


			affecting = usr
			depl = 1
			canremove = 0
		//	usr:head = new/obj/item/clothing/head/helmet/space/space_adv //Head create
			equip(new/obj/item/clothing/head/helmet/space/space_adv, slot_head)
			n_hood = usr:head
			n_hood.canremove = 0
