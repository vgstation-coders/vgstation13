...
/obj/machinery/atm/process()
	...
	if(linked_db && ( (linked_db.stat & NOPOWER) || !linked_db.activated ) )
		...
		src.visible_message("<span class=\"rose\">\icon[src] [src] buzzes rudely, \"Connection to remote database lost.\"</span>")
		updateDialog()
...
/obj/machinery/atm/attack_hand(mob/user as mob)
	if(istype(user, /mob/living/silicon))
		user << "<span class=\"rose\">Artificial unit recognized. Artificial units do not currently receive monetary compensation, as per NanoTrasen regulation #1005.</span>"
		return
					...
					if(!authenticated_account)
						...
						if(previous_account_number == tried_account_num)
							if(number_incorrect_tries > max_pin_attempts)
								...
								usr << "<span class=\"rose\">\icon[src] Incorrect pin/account combination entered, [max_pin_attempts - number_incorrect_tries] attempts remaining.</span>"
								previous_account_number = tried_account_num
								playsound(src, 'sound/machines/buzz-sigh.ogg', 50, 1)
						else
							usr << "<span class=\"rose\">\icon[src] incorrect pin/account combination entered.</span>"
							number_incorrect_tries = 0
					else
