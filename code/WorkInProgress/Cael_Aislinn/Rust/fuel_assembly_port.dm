...
/obj/machinery/rust_fuel_assembly_port/attackby(var/obj/item/I, var/mob/user)
	if(istype(I,/obj/item/weapon/fuel_assembly) && !opened)
		if(cur_assembly)
			user << "<span class=\"rose\">There is already a fuel rod assembly in there!</span>"
		else
			cur_assembly = I
			user.drop_item()
			I.loc = src
...
/obj/machinery/rust_fuel_assembly_port/attack_hand(mob/user)
	...
	if(cur_assembly)
		if(try_insert_assembly())
			...
			if(eject_assembly())
				user << "<span class=\"rose\">\icon[src] [src] ejects it's fuel assembly. Check the fuel injector status.</span>"
			else if(try_draw_assembly())
				user << "<span class=\"notice\">\icon[src] [src] draws a fuel rod assembly from an injector.</span>"
	else if(try_draw_assembly())
		user << "<span class=\"notice\">\icon[src] [src] draws a fuel rod assembly from an injector.</span>"
	else
		user << "<span class=\"rose\">\icon[src] [src] was unable to draw a fuel rod assembly from an injector.</span>"
