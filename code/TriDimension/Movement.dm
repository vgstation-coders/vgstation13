...
/obj/item/weapon/tank/jetpack/proc/move_z(cardinal, mob/user as mob)
	if (user.z > 1)
		user << "<span class=\"rose\">There is nothing of interest in that direction.</span>"
		return
		...
		switch(cardinal)
				...
				if(user.z > maxZ) // If we aren't at the very top of the ship
					...
					if(T && istype(T, /turf/space))
						...
						for(var/atom/A in T.contents)
							if(T.density)
								blocked = 1
								user << "<span class=\"rose\">You bump into [T.name].</span>"
								break
						if(!blocked)
							user.Move(T)
					else
						user << "<span class=\"rose\">You bump into the ship's plating.</span>"
				else
					user << "<span class=\"rose\">The ship's gravity well keeps you in orbit!</span>" // Assuming the ship starts on z level 1, you don't want to go past it
			...
			if (DOWN) // Going down!
				if (user.z < 1) // If we aren't at the very bottom of the ship, or out in space
					...
					user << "<span class=\"rose\">The ship's gravity well keeps you in orbit!</span>"